# WAVE06 Alpha Candidate Campaign — Predeclaration (W06-A-CAMPAIGN-PREDECLARATION-01, repaired by W06-A-CAMPAIGN-PREDECLARATION-AUTHORITY-REPAIR-01)
...
## Advancement policy (Finding 2)

`PREDECLARED_CAMPAIGN.json`'s `advancement_policy` block freezes every gate a candidate must clear, numerically, before any Wave06 result exists: absolute economic solvency, benchmark-relative excess (`> 0.0`), matched-placebo excess (`> 0.20` net Sharpe), long-short-vs-long-only excess (`> 0.0`), DSR (`>= 0.0`, reusing the metric's own literature-standard midpoint), PBO (`<= 0.5`, same), the genuine shuffled placebo (`mqk_research.ml.genuine_shuffled_placebo_cli`), the P9 robustness gauntlet (`dsr_pbo_sensitivity_cli`'s full entry-threshold sweep), and P7A/P7B economic replay stress (`p7a_p7b_economic_replay_stress_cli`, 40% max-drawdown ceiling). No banned vague word ("non-negligible", "materially", "meaningfully", etc.) survives in either candidate's `falsification_condition` — each now points at this policy instead. This development-stage policy is explicitly **not** a promotion bypass: it never weakens the canonical production promotion gate.
