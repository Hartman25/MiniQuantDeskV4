...
- **Deployed universe/promotion truth — UNRESOLVED.** `sys_strategy_registry` contains 41 rows; 40 read as leftover test fixtures (`strat-srg05_*`, `sup_*`, `disabled_*`, `dup_*`, `fullpath_*`, `disarmed_*`) and exactly one (`intraday_scalper`, kind=`native`, enabled=true) reads as a genuine production strategy. `sys_strategy_promotion_transitions` has zero rows for `intraday_scalper` — no promotion-transition evidence could be found for the one real deployed strategy. This may simply mean native/built-in strategies are authorized through a path other than the research-promotion table, but this read-only pass could not confirm that. Also note: today's (`2026-09-18`) autonomous daily operation (`352ea5d7...`) never started a run (`start_attempt_count=0`) and was closed to `evidence_degraded` at end-of-day rollover — consistent with the runtime remaining disarmed all day, not a separate scheduling defect.

**Result: M1.9 PARTIAL.** Seven of eight verification sub-items (daemon identity, Paper DB identity, config truth, provider/data freshness, scheduler registration, risk/arm/reconcile truth, OMS truth) are internally consistent and truthfully verified against the actual deployed system. The deployed-universe/promotion-authorization sub-item remains `UNKNOWN_NEEDS_PROOF` — no promotion-transition record could be found for the sole real deployed strategy, and the production strategy registry table also contains substantial leftover test-fixture rows whose presence in a production DB was not evaluated further (out of this read-only pass's scope). M1.9 does **not** close on this evidence; it remains **OPERATIONAL_ONLY / PARTIAL** pending either a promotion-record source outside this table or an explicit operator decision on how `intraday_scalper`'s authorization is tracked.
