...
- **CONFIRMED — day/time context, reject-window id, and kill-switch context are equally frozen at construction, not just equity.** The same one-time `RiskInput` built in `RuntimeRiskGate::from_run_config` fixes `day_id` (run-start day), `reject_window_id` (run-start minute bucket), and `kill_switch: None` for the life of the run (`mqk-runtime/src/runtime_risk.rs:30-44`), alongside the frozen `equity_micros` above; `evaluate_gate`/`evaluate_gate_for_request` reuse this one stored input on every call, overriding only `request`/`is_risk_reducing`. No production call site refreshes `day_id` (day-rollover detection is therefore inert), advances `reject_window_id` (reject-window rollover is therefore inert), or ever constructs a non-`None` `kill_switch` value — the kill-switch branch of `mqk_risk::evaluate()` is unreachable from the daemon today, independent of the reject-storm defect above. Verified directly at `mqk-runtime/src/runtime_risk.rs:30-44`.
- **CONFIRMED — the daemon's ordinary risk-config seam never supplies max-drawdown, so the fail-open default above is not an edge case on the daemon-created-run path.** `mqk-daemon/src/state/orchestrator_build.rs::load_risk_env`/`effective_run_config_for_risk` read exactly two env-derived values (`MQK_RISK_DAILY_LOSS_LIMIT` and an equity figure) into the run's `config_json` `/risk` subtree; there is no equivalent path that ever populates `/risk/max_drawdown`. Combined with the fail-open default immediately above, this means max-drawdown is disabled on the ordinary daemon-created-run path today, not merely on some unusual hand-authored config — a dynamic-equity repair alone (item D.1) would not make max-drawdown operational without also threading a max-drawdown value through this same seam. Verified directly at `mqk-daemon/src/state/orchestrator_build.rs:75-79, 634-646`.
...
**Why this matters for the mission's finish line:** CLAUDE.md frames this repository as one where "real money will be deployed." A risk gate whose engine logic exists (`mqk_risk::evaluate()` is real and production-reachable) but whose runtime state/config authority is not wired — frozen equity, frozen day/reject-window context, a kill-switch input that can never be non-`None`, a reject-count mutator with zero production callers, a hardcoded-ok PDT context, and a max-drawdown control that is disabled by default on the ordinary daemon-created-run path — is not a cosmetic gap. It is the account-level capital-preservation controls silently not doing their stated job for the life of any run, Paper or Live, even though the gate they sit inside is genuinely wired into every order submission. This is listed as a NOW-lane critical-path item below and in the final report's deterministic-defects list, not filed as a routine backlog item.
...
This path is sequenced, not a flat backlog — a fresh soak session does not precede fixing the code blocker; it follows it.

**NOW — engineering blocker (must close before any countable session):**

1. **`RUNTIME-RISK-DYNAMIC-STATE-AUTHORITY-01` — bind the live risk gate to real runtime state (equity, day/time, reject window, reject count, PDT, kill-switch, max-drawdown config).** Status: OPEN. Why required: `mqk_risk::evaluate()` is genuinely production-reachable through `RuntimeRiskGate` (every order submission is gated by it), but its input binding is static — frozen equity, frozen `day_id`, frozen `reject_window_id`, a `kill_switch` that can never be non-`None`, a reject-count mutator with zero production callers, a hardcoded-ok PDT context, and a daemon config seam that never supplies max-drawdown at all — so the account-level capital-preservation controls this gate exists to provide cannot structurally fire today. Evidence: `mqk-runtime/src/runtime_risk.rs:30-49` (frozen `RiskInput`), `mqk-risk/src/types.rs::record_reject` (zero production callers), `PdtContext::ok()` hardcoded, `mqk-daemon/src/state/orchestrator_build.rs:75-79,634-646` (`load_risk_env`/`effective_run_config_for_risk` supply only equity + daily-loss). Acceptance: a live-equity-drop test proves the daily-loss/max-drawdown gate denies; a reject-storm negative control proves the reject-window gate denies; day-rollover is proven to reset the daily-loss baseline; PDT context reflects real state (or is explicitly, visibly disabled rather than silently no-op'd); the kill-switch input either has a real runtime-authoritative source or is explicitly documented as intentionally not wired; max-drawdown config is threaded through the daemon's ordinary run-config seam. Patch class: CODE + TEST. This repair must first identify authoritative, freshness-safe sources for each input — see §M for the recommended decomposition; do not assume `mqk-execution`'s `PortfolioState` is authoritative current equity without code proof, since the external broker snapshot may be the more appropriate source.

**PRE-SOAK OPERATIONAL PROOF (before the next session, independent of the code repair above):**
...
3. **Fix the operational cause of, or mitigate, `DAEMON-EXIT-20260824`.** Status: OPEN (UNKNOWN_NEEDS_PROOF root cause per the ledger itself). Why required: an unattended autonomous session must survive ordinary operational faults; a host entering sleep mid-session and the daemon going silent afterward is a soak-session-invalidating event if it recurs, and would contaminate the one fresh validation session in item 4 below if unaddressed first. Acceptance: disable host sleep/Modern-Standby during autonomous sessions (operational config) and/or add a watchdog/crash-detection mechanism. Patch class: OPERATIONAL-PROOF (config) or CODE (watchdog).

**THEN — one fresh post-repair validation session (not yet countable toward the 10-20 session soak):**

4. **Run one fresh autonomous daily session on the post-repair binary, with items 2-3 above already satisfied, and confirm it both re-validates the 2026-08-24/25 stale-operation fix and shows the repaired risk gate actually reacting to a real (Paper) state change.** Status: OPEN. Why required: the stale-operation fix is code-closed and independently accepted, but no session in evidence has actually run a fresh Paper day against it — §33 says so explicitly, and this audit found no newer local evidence; separately, item D.1's repair itself needs one live confirmation beyond its own unit/integration tests before soak sessions built on top of it are treated as meaningful safety evidence. Acceptance: one clean autonomous daily session, on the current (post-D.1) binary, that reaches `strategy_evaluation_count > 0` (or a legitimate no-signal verdict distinguishable per the `evaluation_count`/outcome-classifier mechanism confirmed above), with no `GapDetected`/unresolved reconcile drift/unexplained exit. Patch class: OPERATIONAL-PROOF.

**Only after item 4 succeeds does the 10-20 session countable soak begin** — see §K.

**Governance (parallel-track — process gap, not a runtime-safety or Paper-mechanics blocker; does not need to precede D.1-D.4):**

5. **Enable GitHub branch protection on `main`.** Status: OPEN (CONFIRMED via `gh api`, HTTP 404 "Branch not protected"; re-confirmed 2026-08-30 by `MASTER-AUDIT-TRUTH-CORRECTION-01`, including an empty `gh api repos/Hartman25/MiniQuantDeskV4/rulesets` result — no ruleset-based protection either). Why required: CI passing is currently advisory only; nothing prevents a direct unreviewed push to the branch this whole ledger treats as ground truth. This is a repository-governance gap, not an economic/runtime Paper-safety blocker, and does not gate D.1-D.4 above. Acceptance: required status checks configured for at least the `rust`, `guards`, and `gui-contract` jobs. Patch class: CONFIG (GitHub settings, not code).

**NEXT (required before/for controlled Paper, not before the risk-gate repair):**

6. **Decide and, if needed, mitigate the live corporate-action/event-risk gap.** Status: OPEN. Why required: the live path has no earnings-calendar feed and no pre-event flattening gate (only an operator-declared static blackout file), materially weaker than the backtest engine's `ForbidPeriods`. Acceptance: either an explicit, documented operator process that covers this during active soak sessions, or a scoped patch. Patch class: DOC (near-term) or CODE (if a live gate is built).
...
1. **Live risk gate uses run-start-frozen equity** — daily-loss-limit and max-drawdown checks cannot fire against a real intraday loss. `mqk-runtime/src/runtime_risk.rs:30-49`. Severity: HIGH (capital-preservation control silently inert).
2. **Live risk gate also freezes `day_id` and `reject_window_id` at construction** — day-rollover (would reset the daily-loss baseline) and reject-window rollover cannot occur during a run. `mqk-runtime/src/runtime_risk.rs:30-44`. Severity: HIGH (same class as defect 1; both are the same frozen-`RiskInput` root cause).
3. **Reject-storm detection has zero production call sites** — `RiskState::record_reject()` is only called from `mqk-risk`'s own test, so the frozen `reject_window_id` above is compounded by nothing ever incrementing the reject count it would key off of. Severity: HIGH (a documented kill-switch cannot trigger).
4. **PDT guard is a hardcoded no-op** — `PdtContext::ok()` unconditionally. Severity: MEDIUM (relevant only to accounts subject to PDT rules, but silently non-functional regardless of config).
5. **`kill_switch` is hardcoded `None` at construction, with no production call site that ever constructs a non-`None` value** — the kill-switch branch of `mqk_risk::evaluate()` is unreachable from the daemon today. `mqk-runtime/src/runtime_risk.rs:44`. Severity: HIGH (a distinct input channel from the reject-storm kill switch in defect 3; both are inert, for different reasons).
6. **`max_drawdown` fails open (silently disabled) on missing config, inconsistent with `daily_loss_limit`'s fail-closed sibling behavior.** Severity: LOW-MEDIUM.
7. **The daemon's ordinary risk-config seam (`load_risk_env`/`effective_run_config_for_risk`) never supplies `/risk/max_drawdown` at all** — defect 6's fail-open default is therefore the *normal* outcome on a daemon-created run, not an edge case; fixing defect 6 alone (fail closed on absence) would make every ordinary run reject-on-start unless this seam is also extended to thread a max-drawdown value through. `mqk-daemon/src/state/orchestrator_build.rs:75-79,634-646`. Severity: MEDIUM (root-cause pairing with defect 6).
8. **`buying_power` aliased to `cash` in the portfolio summary route** — inaccurate for a margin account. Severity: MEDIUM, but out of near-term scope (Live-gated).
9. **`main` has no GitHub branch protection or ruleset** (re-confirmed 2026-08-30: `gh api .../branches/main/protection` → 404; `gh api .../rulesets` → empty list). Severity: MEDIUM (process/governance gap — does not gate Paper/Live runtime safety; tracked on a separate governance track from defects 1-7, see §D).
10. **Live corporate-action/event-risk handling has no earnings-calendar feed and no pre-event flattening gate**, unlike the backtest engine. Severity: MEDIUM, self-documented in code, not hidden.
11. **`DAEMON-EXIT-20260824` root cause remains `UNKNOWN_NEEDS_PROOF`** (Windows sleep correlation, not a proven code defect) — listed here as a tracked open risk, not asserted as a confirmed code defect.

Proposed patch IDs (not implemented, per mission scope): `RUNTIME-RISK-DYNAMIC-STATE-AUTHORITY-01` (defects 1-3, 5, 7 — renamed from the earlier `RUNTIME-RISK-LIVE-STATE-BINDING-01` working name to avoid the "Live" term being misread as Live-capital-mode-only; this defect set affects the production runtime used by Paper as well), `RUNTIME-RISK-DRAWDOWN-FAIL-CLOSED-01` (defect 6, sequenced after `RUNTIME-RISK-DYNAMIC-STATE-AUTHORITY-01` per defect 7's coupling), `LIVE-PORTFOLIO-BUYING-POWER-AUTHORITY-01` (defect 8, Full-V4/Live lane), `REPO-GOVERNANCE-BRANCH-PROTECTION-01` (defect 9, config not code, governance track), `LIVE-EVENT-RISK-CALENDAR-GATE-01` (defect 10, if a code fix is chosen over an operator-process mitigation).
...
| Risk gate (structural wiring) | CONFIRMED | CONFIRMED | N/A | N/A | UNKNOWN — likely-inert per defects 1-3, 5 | CONFIRMED (tests pass, but don't exercise live-equity-change scenario) |
...
**BLOCKED FOR COUNTABLE PAPER SOAK / FORWARD VALIDATION.**

The one previously-tracked hard blocker (stale-operation release) is code-closed and independently accepted, and no reproducible defect blocks a deliberately-authorized, explicitly-diagnostic Paper run from being started today. But "Paper capital is not real" is not sufficient reason to call the frozen-equity/frozen-day/frozen-reject-window/inert-kill-switch/disabled-max-drawdown risk-gate defect set (§Domain: Risk, §G defects 1-3, 5-7) non-blocking, because the soak's entire purpose is to validate that the account-level safety controls work — and right now they provably do not fire. Concretely:

- A run started today may be performed as an explicitly-authorized diagnostic session, but it does not count toward the 10-20 valid-session countable soak sequence (§K).
- No countable session should begin until `RUNTIME-RISK-DYNAMIC-STATE-AUTHORITY-01` (§D item 1) is accepted, migration `0067`'s Paper-DB status is confirmed (§D item 2), and host sleep is disabled for the session's duration (§D item 3) — in that order, per §D's sequencing.
- Only the one fresh post-repair validation session described in §D item 4 may begin the countable sequence, and only once it passes.
...
**The countable 10-20 session sequence has not started, and no session may be counted toward it until `RUNTIME-RISK-DYNAMIC-STATE-AUTHORITY-01` (§D item 1) is accepted and the one fresh post-repair validation session (§D item 4) has passed** — see §I. Sessions run before that point may be genuine, deliberately-authorized diagnostic Paper runs, but they do not count.

Once counting begins, for a session to count as VALID it must show, via durable evidence (not daemon-uptime alone):
...
- The risk gate's dynamic-state inputs (equity, day rollover, reject window, kill-switch) were genuinely live for that session, not still running on the pre-repair frozen `RiskInput` — a session on the pre-repair binary cannot count regardless of how clean it otherwise looks, since its own safety net was provably not evaluating real state during the session.
- Given the confirmed frozen-equity risk-gate defect, even after the repair, a session where a real drawdown occurred but the risk gate did not react should be flagged, not silently counted as a clean pass.

Per the evidence gathered, **zero sessions since the 2026-08-25 stale-operation fix landed have been confirmed VALID in this audit's evidence trail**, and — independent of that — **zero sessions of any kind have run against a risk gate with dynamic-state authority**, since that repair does not yet exist. Both conditions must be satisfied before the first countable session.

## L. Master Remaining Ledger

**NOW** (required, in sequence, before any countable equity/ETF Paper soak session): D.1 (`RUNTIME-RISK-DYNAMIC-STATE-AUTHORITY-01` — engineering blocker, must close first) → D.2 + D.3 (migration 0067 confirmation, `DAEMON-EXIT-20260824` operational fix/host-sleep mitigation — pre-soak operational proof) → D.4 (one fresh post-repair validation session — first potentially-countable session).

**GOVERNANCE** (parallel-track, does not gate the NOW sequence above): D.5 (GitHub branch protection on `main`).

**NEXT** (required before/for controlled Paper, not before D.1-D.4): D.6-D.7 (event-risk mitigation decision, multi-symbol-in-production confirmation).
...
**`RUNTIME-RISK-DYNAMIC-STATE-AUTHORITY-01`** (renamed from the earlier working name `RUNTIME-RISK-LIVE-STATE-BINDING-01` — "Live" could be misread as Live-capital-mode-only; this defect set affects the production runtime used by Paper as well) — bind `RuntimeRiskGate` to real, current runtime state instead of values frozen at run start. Before implementing, this repair must first identify authoritative, freshness-safe sources for each of the following (this audit does not decide those sources — that is a future production-patch discovery question, not a docs-mission decision):

- **Current account equity** — feed actual current equity into the risk evaluator on each gate check so daily-loss-limit and max-drawdown can genuinely fire. Do not assume `mqk-execution`'s `PortfolioState` is authoritative current equity merely because it is convenient; prove it has authoritative mark-to-market equity, or use the external broker snapshot if that proves to be the more appropriate source.
- **Day/time authority** — refresh `day_id` so day-rollover genuinely resets the daily-loss baseline instead of running against a run-start day for the life of the process.
- **Reject event + current reject window** — call `RiskState::record_reject()` from the real order-rejection path, and refresh `reject_window_id` so reject-storm detection is live, not frozen at run-start.
- **PDT/account applicability state** — replace the hardcoded `PdtContext::ok()` with real account PDT state, or, if PDT truly does not apply to this account/asset class today, replace it with an explicit, visible "not applicable" state rather than a silent always-pass.
- **Kill-switch/safety-event context** — either identify a real runtime-authoritative source for `kill_switch` (currently hardcoded `None`), or explicitly document which `mqk_risk` kill-switch branches are intentionally not runtime-authoritative today, rather than leaving the gap undocumented.
- **Max-drawdown configuration** — thread a max-drawdown value through the daemon's ordinary risk-config seam (`load_risk_env`/`effective_run_config_for_risk`), which today only ever supplies equity and daily-loss-limit.

This is the single highest-leverage finite blocker found in this audit: it is the one deterministic, safety-critical, currently-inert set of controls standing between "the mechanics work" and "the mechanics are actually protected the way CLAUDE.md's priority ordering requires," and every other NOW-lane item (migration confirmation, a fresh soak run) is either a precondition-free operational step rather than an engineering patch, or (branch protection) reclassified onto a separate governance track that does not gate this repair. Recommend running it as a CLAUDE.md-style autonomous wave with sequential patches (equity binding → day/reject-window rollover → reject-storm wiring → PDT context → kill-switch decision → max-drawdown config threading), each with its own RED/GREEN proof against a live-equity-change / day-rollover / synthetic-reject-burst negative control, since each is independently testable and none should be merged bundled with the others.
