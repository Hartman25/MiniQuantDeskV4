...
| `ETF-RISK-01` Correlation exposure controls | CLOSED (`ETF-RISK-CLOSURE-01`) | 100% | S | `PORTFOLIO-LIVE-WEIGHTS-01` | Live, bps-based `evaluate_sector_risk` wired pre-outbox as Gate 1h in `decision.rs`; default-off via `MQK_SECTOR_EXPOSURE_LIMITS_BPS`; see §16 closure note | 2 | Closed once the live mark/NAV/weight dependency (`PORTFOLIO-LIVE-WEIGHTS-01`) existed. |
