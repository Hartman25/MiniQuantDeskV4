...
## FULL-AUDIT-REMAINING-FAILURES-CONSOLIDATED-CLOSURE-01

**Session:** consolidated repair of every remaining open finding above FAIL-012, in one combined patch per operator authorization. Continues from HEAD `a14ede761c5fd0db3f4a5c9dd54241c228c93f38`. FAIL-012 preserved closed, untouched.

**Result: BLOCKED** — every finding below was investigated and either closed with verified evidence or found to be a genuine, evidenced blocker. Two classes of finding could not be safely closed within this session's explicit constraints (no production-gate changes, no real broker/provider network calls) and are reported honestly as still open rather than claimed closed.

### Per-finding disposition

| ID | Disposition | Evidence |
|---|---|---|
| FAIL-001 | CLOSED AS ENVIRONMENT LIMITATION WITH SUPPORTED WORKFLOW — authority unified with CI (FULL-AUDIT-FINAL-HERMETIC-CLOSURE-01) | Windows `CreateProcess` command-line limit is a real OS constraint, not a repo defect. The WINPATH-01 per-package workaround is now `scripts/windows/Invoke-CanonicalFmtCheck.ps1`, a single script invoked by both `full_repo_proof.ps1` and CI's `windows` job (previously CI ran a bare `cargo fmt --check` while local verification ran an inline duplicate of the same logic — two independently-driftable implementations). `scripts/guards/check_ci_local_toolchain_convergence.sh` proves this convergence; verified passing. Per-crate `cargo fmt --check` (via that script) passes clean across all 21 crates. |
| FAIL-002 | CLOSED BY `3d11d20e` | `cargo fmt -p <crate>` applied to all 21 crates; a second per-crate `--check` pass yields zero diff; `cargo check --workspace --all-targets` compiles clean after formatting (no semantic change). |
| FAIL-003 | CLOSED BY `f6e893a1` | 6 `InitRunArtifactsArgs` call sites fixed (audit found 4; a full `cargo check -p mqk-testkit --all-targets` found 2 more the audit missed). All 6 affected tests pass; `cargo check --workspace --all-targets` and `cargo clippy --workspace --all-targets -- -D warnings` both pass clean. |
| FAIL-004 | CLOSED BY `f6e893a1` | Shared test-only lock converted `std::sync::Mutex` → `tokio::sync::Mutex`; 2 more previously-masked instances of the same defect found and fixed once FAIL-003 stopped blocking compilation. `cargo clippy --workspace --all-targets -- -D warnings` passes with zero errors workspace-wide. |
| FAIL-005 | CLOSED AS ENVIRONMENT LIMITATION WITH SUPPORTED WORKFLOW — full matrix now proven end-to-end (FULL-AUDIT-FINAL-HERMETIC-CLOSURE-01) | Genuine Windows page-file/memory constraint, not a repo defect. The documented `CARGO_BUILD_JOBS=1`/`CARGO_INCREMENTAL=0`/`RUSTFLAGS=-C debuginfo=0`/`--test-threads=1` posture is the supported workaround, present in CI and `full_repo_proof.ps1`. This session ran `cargo test --workspace --no-fail-fast -- --test-threads=1` (default matrix) to a clean completion — 5,531 passed, 0 failed, 687 ignored, exit 0 — and the complete `--include-ignored` sweep to completion twice (6,218 tests total both times) — the full matrix this finding's workaround exists for has now actually been run to completion, not just documented. The heavy lock the workaround coexists with is now atomic (see FAIL-related lock closure below), removing a second class of Windows-machine contention risk. |
| FAIL-006 | CLOSED BY `99e3fca6` | All 5 advisories cleared to 0 vulnerabilities entirely within `package.json`'s existing declared semver ranges (no major bump, no `--force`, `package.json` has zero diff — only the lockfile changed). Verified: `npm audit` 0 vulnerabilities, from-scratch `npm ci` reproduces it, `tsc --noEmit` clean, `npm run test` 977/977, `npm run build` passes. |
| FAIL-007 | CLOSED BY `6aa4a3cd` | Nested-quote `SyntaxError` fixed; focused test added proving the intended bounded error on malformed grid JSON. `python -m compileall` clean; `pytest` 988 passed (+2 new), 5 skipped (unchanged documented reasons). |
| FAIL-008 | CLOSED BY `238ecab0` | Both orphaned duplicate launchers deleted (neither was ever git-tracked — both matched `.git/info/exclude`). Their one shape difference from canonical (extra binary-name candidates) was verified against the real Cargo bin name (`mqk-gui`) to be a stale, incorrect guess, not missing behavior. |
| FAIL-009 | CLOSED BY `238ecab0` | `test_capture_paper_smoke_evidence.ps1` no longer hard-codes `powershell.exe`; new `-CompatibilityShell` param (Auto/CurrentHost/Pwsh/WindowsPowerShell). Verified in both Auto and explicit `-CompatibilityShell WindowsPowerShell` modes — all 46 checks pass in this environment (the original failure was environment flakiness, not an inherent unavailability, per direct re-test). |
| FAIL-010 | CLOSED BY `238ecab0` | All 3 stale guards updated to the accepted current source shape (`RuntimeStrategyDispatchAuthority::Legacy`/`tick_strategy_dispatch_multi_symbol_with_bar_facts`; the day/tick reset pairing relocated to `state.rs`; cap #6's `for envelope in allocation_outcome.decisions` loop header). Mutation-negative self-tests added/verified for all 3, proving each guard actually fails against deliberately disconnected/removed wiring, not just against the real source. |
| FAIL-011 | **CLOSED BY FULL-AUDIT-FINAL-HERMETIC-CLOSURE-01** | The prior session's own root-cause finding was correct (the daily-data-readiness gate structurally cannot report `start_allowed=true` with `db: None`) but its recommended paths (weaken the production gate, or treat "no expected-gate-update" as absolute) were both wrong turns. The actual smallest correct repair, per requirements-level analysis: separate each test's *original intent* from the now-impossible "db=None reaches the generic DB-not-configured 503" expectation, and assert the honest current gate order instead. 5 of 6 tests (`g01`, `brk00r06_e02`, `brk00r06_e03`, `brk09r_r03`, `brk09r_r04`) now assert `403 daily_data_readiness` in place of the impossible `503`; each carries a coverage-map comment naming the DB-backed replacement proof for the original "db-gate reachable" intent (`scenario_daily_data_readiness_start_gate_01.rs::sg_10`/`sg_16`) and the pure-predicate replacement (`sg_01`/`sg_21`, `scenario_daily_data_readiness_01.rs::ddr_07`/`ddr_08`) — all pre-existing, independently verified passing. The 6th (`ptday02_e08`) was **misclassified** by the prior session: independent re-diagnosis found its actual collision is unrelated — `"side": "sell"` with no active run classifies as `OrderIntent::ShortOpen` and trips the newer Gate 1j short-entry policy before Gate 1b ever evaluates; changed to `"side": "buy"` (matching its sibling `ptday02_e07`), which reaches the originally-intended ColdStartUnproven refusal unchanged. All 6 verified: `scenario_combined_paper_gate_rts07_rsk07.rs` 10/10, `scenario_paper_alpaca_proof_bundle_brk00r06.rs` 31/31, `scenario_reconcile_start_gate_brk09r.rs` 5/5, all passing, zero DB required. No production gate weakened. |
| FAIL-012 | PRESERVED CLOSED | Unchanged; see `INGEST-JOB-CANCEL-STATUS-CONSTRAINT-REPAIR-01 closure` above. |
| FAIL-013 | CLOSED BY `f6e893a1` | Evidence directory read-then-delete ordering fixed via an RAII cleanup guard. Test passes individually against the isolated 5434 DB. |
| FAIL-014 | CLOSED BY `f6e893a1` | Free-form `"manual disarm"` literal replaced with the canonical `"ManualDisarm"` string already used by other passing tests. Test passes. |
| FAIL-015 | CLOSED BY `f6e893a1` | Both brittle exact-match assertions widened to closed-set parsing (rejects unknown/duplicate components, admits the legitimately-added `postgres.sys_incidents` / `postgres.audit_events[orchestrator]` components). Direct verification of `ah04` after the string fix surfaced a second, audit-undocumented defect in the same test (a stale fixed timestamp falling out of the feed's bounded `ORDER BY ts_utc DESC LIMIT 50` window) — fixed in the same commit. Both tests pass. |
| FAIL-016 | **CLOSED BY FULL-AUDIT-FINAL-HERMETIC-CLOSURE-01, via the private hermetic seam — see FAIL-018 (same closure)** | All 8 tests' positive intent is now proven hermetically in-crate. See FAIL-018 row for the mechanism (identical for both findings). |
| FAIL-017 | **RE-CLOSED BY FULL-AUDIT-FINAL-HERMETIC-CLOSURE-01 — `1e7d6e23`'s closure was overclaimed and has been replaced** | `1e7d6e23` closed this using a broad `cleanup_all_mqk_daemon_paper_runs` helper that deleted every `(engine_id='mqk-daemon', mode='PAPER')` row from the *shared* database (including other tests' own fixtures, across 10 tables, every deletion's error silently swallowed via `.ok()`) for `h03`, and `Utc::now()`-stamped fixture timestamps to win a wall-clock "latest row" race against every other test in the shared DB for `ed01_01`/`n05` — global mutation and timing bets, not isolation. Both mechanisms are removed. `mqk_db::create_disposable_test_db`/`run_isolated` (new, in `mqk-db`) give `h03`, `ed01_01`, `n05`, `ir02_06`, `b2a_n03`, `b2a_n04` — and `ir02_04`, found to need the identical fix at full-sweep scale, see below — each their own disposable Postgres database on the port-5434 server, migrated fresh, dropped on completion (including on panic). Fixed historical timestamps are used again throughout since no other writer exists to race. Verified: each test passes individually, within its file's full batch (serial and parallel), and — the specific gap `1e7d6e23` left explicitly open ("not re-run at full ~428-test-sweep scale") — within two complete `cargo test --workspace --include-ignored --test-threads=1` sweeps of the entire 6,218-test suite; zero disposable-database residue confirmed after each. One of the two full-sweep runs surfaced `ir02_04` failing (`arm-execution must return 200` got `403` — a residual durably-disarmed/dirty-reconcile state left by an unrelated earlier test, the same root cause class): fixed with the identical `run_isolated` pattern and reconfirmed clean on a third full sweep. `ir02_04`/`ir02_06`'s prior "already passed, no code change needed" classification is corrected — `ir02_06` genuinely needed the fix (it carried a fixed 2020 timestamp racing the same global "latest run" query `n05` needed `Utc::now()` to survive), and `ir02_04` needed it too, just not observable below full-sweep scale. |
| FAIL-018 | **CLOSED BY FULL-AUDIT-FINAL-HERMETIC-CLOSURE-01 — private in-crate hermetic seam, zero network calls, zero production surface change** | The prior session correctly found the external `tests/*.rs` integration tests cannot reach the crate-private `#[cfg(test)]` hermetic broker override (`AppState::set_hermetic_test_broker_override_for_test`) because integration tests are a separate crate — `pub(crate)` genuinely does not resolve across that boundary (confirmed by first attempting exactly this and getting `E0599: no method named ... found`). It correctly ruled out widening that visibility. The actual fix, per this patch's explicit direction: move the *scenario*, not the *visibility* — a new `#[cfg(test)] mod tests` inside `crates/mqk-daemon/src/state/hermetic_positive_proofs.rs` (declared `#[cfg(test)] mod hermetic_positive_proofs;` in `state.rs`) drives the real `routes::build_router` HTTP path, enables the existing override, and pre-seeds the already-`pub` `AppState::broker_snapshot` field (the established idiom used by ~15 other test files) so `build_execution_orchestrator`'s `External` branch never constructs an `AlpacaBrokerAdapter` or attempts a fetch. Covers `b2a_n02`, `b1a_l04`/`l05`/`l06` (closing FAIL-016 together with this), and the 5 `scenario_daemon_order_submit.rs` positives (closing the rest of FAIL-016) — 9 tests total, all passing, all disposable-DB-isolated. **Network-deny witness** (`hermetic_override_without_seeded_snapshot_makes_no_network_call`): with the override enabled but *no* pre-seeded snapshot, `daemon_broker` is still the `DaemonBroker::Paper` variant (the override substitutes it unconditionally) — there is no `AlpacaBrokerAdapter` value in memory for the `Self::Alpaca(adapter) => ...` arm to match, so the result is the local, deterministic `broker_snapshot_source_mismatch` fault, resolving in milliseconds, never a network-shaped error; verified passing. A literal loopback-URL trap (the network-deny mechanism this patch's request suggested) was deliberately not used for these LiveShadow+Alpaca fixtures: `alpaca_base_url_for_mode` hardcodes the live endpoint for `LiveShadow`/`LiveCapital` by an existing, deliberate, tested production invariant (`env_truth_02_alpaca_live_base_url_env_var_is_not_authoritative`) — an operator-set env var must never redirect live-capital order flow, and this patch is required to preserve that invariant, not weaken it for testability; the type-level "no Alpaca adapter exists to call" proof is used instead, which is strictly stronger. **No-production-bypass guard strengthened**: `scripts/guards/check_no_phase7a_production_effects_bypass.ps1` now also verifies `set_hermetic_test_broker_override_for_test` stays `#[cfg(test)]`+`pub(crate)`, `hermetic_test_broker_override_enabled` stays `pub(crate)` (never plain `pub`), no env var of any kind gates the override, and the two existing default-build credential-required proofs (`build_daemon_broker_paper_is_not_execution_path`, `env_truth_02_...`) still exist — verified passing, and verified to actually fail when the setter is temporarily made `pub` (negative control). The 9 external originals (5 in `scenario_daemon_order_submit.rs`, 3 in `scenario_native_strategy_bootstrap_daemon_b1a.rs`, 1 in `scenario_native_strategy_registry_b2a.rs`) are left in place, `#[ignore]`d, unmodified — a manual escape hatch for an operator with real credentials, never run by CI or any default command — with a coverage-map comment in each file pointing to the new in-crate replacement. Zero external network calls made anywhere in this closure; zero credentials loaded. |

### Verification performed this session (not the full matrix — see below)

- `cargo check --workspace --all-targets`: PASS (multiple times, after each functional change).
- `cargo clippy --workspace --all-targets -- -D warnings`: PASS, zero errors (full ~9-minute low-memory run).
- Every individually-fixed test verified passing against the isolated `127.0.0.1:5434` test database, both standalone and within its own file's batch.
- `cargo fmt -p <crate> -- --check`: PASS, zero drift, all 21 crates (twice — before and after the formatting commit).
- GUI: `npm ci` (from scratch), `npm audit` (0 vulnerabilities), `tsc --noEmit`, `npm run test` (977/977), `npm run build` — all PASS.
- Python: `compileall` clean, `pytest` 988 passed / 5 skipped.
- PowerShell: all 4 previously-failing guards individually re-verified PASS; full `run_all_script_guards.ps1` run showed 7 failures, all confirmed to be pre-existing scope-guards tripping on this session's own still-uncommitted cross-cutting diff at the time (not new defects — see next section) rather than newly-introduced regressions.
- DB residue: a full dependency-ordered cleanup was run against the isolated port-5434 `mqk_test` database (all `runs`-derived tables + `sys_autonomous_session_events` + `sys_strategy_registry` + the `runtime_leader_lease`/`runtime_control_state` singletons), reducing confirmed-synthetic residue to zero across every checked table.
- **Not re-run in this session**: the complete `--include-ignored` sweep across all 21 crates (the ~684-test matrix the original audit's completion session ran), the full GUI+Rust+Python+guards matrix in one combined pass, and a fresh migration-chain proof on a disposable DB. Each lane's *narrow* proof was re-verified piecemeal (see above) but not re-assembled into one final combined run within this session.

Ports 5432/5440 were never queried or mutated. No real broker/provider credentials were loaded (the one attempt that would have required them was reverted upon discovering it made a real network call). No live daemon, Bundle 8, or formal soak was started. No orders were placed.

---

## FULL-AUDIT-FINAL-HERMETIC-CLOSURE-01

**Session:** final repair of the five exact gaps left open by `FULL-AUDIT-REMAINING-FAILURES-CONSOLIDATED-CLOSURE-01` above (FAIL-011 unclosed; FAIL-016/018 unclosed; FAIL-017 overclaimed via global shared-DB mutation; CI/local toolchain and format authority diverged; the complete matrices never run to completion). Starting HEAD `dc0ffd0b06797966b2737211db72a59c8193d603` (== `origin/main`). Continues additively on `main`; FAIL-012 and every other previously-accepted repair preserved untouched and unweakened.

### Disposition

**FAIL-011: CLOSED.** **FAIL-016: CLOSED.** **FAIL-017: RE-CLOSED with true isolation** (prior closure's global-deletion/`Utc::now()`-racing mechanism removed and replaced). **FAIL-018: CLOSED.** Full per-finding evidence in the corrected rows of the disposition table above.

### FAIL-011 coverage map (old assertion → new proof)

| Test | Old (impossible) assertion | New assertion | DB-backed replacement of lost intent |
|---|---|---|---|
| `g01_aligned_state_satisfies_both_start_and_signal_chains` | 503, db-gate | 403 `daily_data_readiness` | `scenario_daily_data_readiness_start_gate_01.rs::sg_10`/`sg_16` |
| `brk00r06_e02_live_continuity_unblocks_ws_gate_reaches_db_gate` | 503, db-gate | 403 `daily_data_readiness` | same |
| `brk00r06_e03_continuity_round_trip_is_fail_closed` | 503, db-gate (×2 steps) | 403 `daily_data_readiness` (×2 steps) | same |
| `brk09r_r03_unknown_reconcile_does_not_block_start` | 503, db-gate | 403 `daily_data_readiness` | same |
| `brk09r_r04_ok_reconcile_does_not_block_start` | 503, db-gate | 403 `daily_data_readiness` | same |
| `ptday02_e08_cold_start_unproven_blocks_strategy_signals` | 503 (misdiagnosed root cause) | 503, unchanged assertion — fixture's `"side"` changed `sell`→`buy` | n/a — original intent (Gate 1b ColdStart) restored, not replaced |

The pure db=None predicate itself (`daily_data_readiness` refuses when `db: None` regardless of other facts) is proven independently and was not touched: `scenario_daily_data_readiness_start_gate_01.rs::sg_01`/`sg_21`, `scenario_daily_data_readiness_01.rs::ddr_07`/`ddr_08`.

### FAIL-016/018 coverage map (external, now-`#[ignore]`d-by-design → in-crate hermetic replacement)

All in `crates/mqk-daemon/src/state/hermetic_positive_proofs.rs`, `#[cfg(test)] mod tests`:

| Retired external test (kept, `#[ignore]`, credential-gated by design) | In-crate hermetic replacement |
|---|---|
| `scenario_native_strategy_bootstrap_daemon_b1a.rs::b1a_l04_start_with_registered_strategy_stores_active_bootstrap` | `hermetic_b1a_l04_start_with_registered_strategy_stores_active_bootstrap` |
| `...::b1a_l05_stop_clears_native_strategy_bootstrap` | `hermetic_b1a_l05_stop_clears_native_strategy_bootstrap` |
| `...::b1a_l06_halt_clears_native_strategy_bootstrap` | `hermetic_b1a_l06_halt_clears_native_strategy_bootstrap` |
| `scenario_native_strategy_registry_b2a.rs::b2a_n02_registry_enabled_allows_activation` | `hermetic_b2a_n02_registry_enabled_allows_activation` |
| `scenario_daemon_order_submit.rs::manual_order_submit_refuses_when_durable_arm_state_is_disarmed_even_if_local_state_is_armed` | `hermetic_order_submit_refuses_when_durable_arm_state_is_disarmed` |
| `...::manual_order_submit_refuses_when_durable_arm_state_is_halted_even_if_local_state_is_armed` | `hermetic_order_submit_refuses_when_durable_arm_state_is_halted` |
| `...::manual_order_submit_enqueues_one_pending_outbox_row_for_active_run` | `hermetic_order_submit_enqueues_one_pending_outbox_row` |
| `...::manual_order_submit_duplicate_client_request_id_is_noop` | `hermetic_order_submit_duplicate_client_request_id_is_noop` |
| `...::manual_order_submit_accepts_limit_order_with_explicit_defaults_aligned_to_runtime` | `hermetic_order_submit_accepts_limit_order` |

Plus the network-deny witness `hermetic_override_without_seeded_snapshot_makes_no_network_call` (no external-test equivalent; new proof). 10 in-crate tests total, all passing, each on its own disposable database.

### FAIL-017 true-isolation mechanism

New: `mqk_db::create_disposable_test_db`/`DisposableTestDb`/`run_isolated` (`crates/mqk-db/src/lib.rs`) — creates a fresh, migrated, uniquely-named Postgres database on the same server as `MQK_DATABASE_URL`, runs the test against it, drops it afterward (including on panic, via `tokio::spawn` + join-error catch). Applied to: `h03_clear_halted_run_no_run_returns_409`, `ed01_01_events_feed_exposes_orchestrator_halt_rows`, `n05_control_arm_provenance_ref_matches_exact_durable_audit_events_uuid`, `ir02_04_arm_execution_durable_target_is_sys_arm_state_not_audit_events`, `ir02_06_control_arm_accepted_action_durable_row_visible_in_history`, `b2a_n03_registry_disabled_refused_at_registry_gate`, `b2a_n04_registry_absent_refused_at_registry_gate`, plus every new hermetic test in FAIL-016/018's closure — 17 tests total on disposable databases. The removed global-delete helper (`cleanup_all_mqk_daemon_paper_runs`) and `Utc::now()`-racing are gone from the codebase; `check_unsafe_patterns.sh`/`.ps1` verified clean (the new disposable-DB-naming code deliberately avoids `Uuid::new_v4()` too, using a process-id + monotonic-counter suffix instead, to stay inside that same guard's enforcement scope).

### CI/local toolchain and format convergence (supports FAIL-001/005's strengthened closure)

- `core-rs/rust-toolchain.toml` (`channel = "1.93.1"`) is now the single source every Rust CI job (`rust`, `db-proof`, `windows`) reads explicitly via a "Read canonical Rust toolchain channel" step, passed to `dtolnay/rust-toolchain@master` as an explicit `toolchain:` input — not `@stable` file auto-detection (which the workflow's own `components:` input was silently defeating; corrected, and the stale claim in `rust-toolchain.toml`'s own header comment corrected too).
- Cache keys now include the toolchain channel.
- `scripts/windows/Invoke-CanonicalFmtCheck.ps1` is the one per-package (WINPATH-01) fmt-check authority, called by both CI's `windows` job and `full_repo_proof.ps1` — previously two independent implementations.
- CI's `windows` job no longer hard-codes `powershell.exe` (this environment's own toolchain baseline table above already documented Windows PowerShell 5.1 as non-functional here) — it re-invokes via `$PSHOME`-derived self-host, matching the pattern `tests/script_guards/run_all_script_guards.ps1`'s own internal guard-invocation loop was also updated to use.
- New guard: `scripts/guards/check_ci_local_toolchain_convergence.sh` — verified passing.

### Atomic heavy lock

`full_repo_proof.ps1`'s heavy lock (previously `Test-Path` then `Set-Content` — a TOCTOU race) is now `scripts/windows/HeavyLock.ps1`: a single atomic `[System.IO.File]::Open(..., FileMode.OpenOrCreate, FileAccess.ReadWrite, FileShare.None)`, held open for the session, with an owner-token check on release. Stale-crash recovery relies on the OS releasing a terminated process's exclusive handle — not on that process's own `finally` block, which a force-kill never runs. New test `tests/script_guards/test_heavy_lock_atomic_01.ps1` proves all four required scenarios for real, including actually starting a child process, force-killing it, and confirming a new acquisition succeeds afterward: verified passing (4/4).

### Complete verification matrix — run to completion

- `cargo fmt` (canonical per-package authority): PASS, 21/21 crates, zero drift.
- `cargo check --workspace --all-targets`: PASS, zero errors.
- `cargo clippy --workspace --all-targets -- -D warnings`: PASS, zero warnings (one iteration caught and fixed a genuine `clippy::module_inception` in the new hermetic-proofs module).
- `cargo test --workspace --no-fail-fast -- --test-threads=1` (default matrix, mqk-testkit included): **5,531 passed, 0 failed, 687 ignored, exit 0.** Includes all workspace doc-tests.
- `cargo test --workspace --no-fail-fast -- --include-ignored --test-threads=1` (complete safe ignored sweep, 6,218 tests): run to completion **twice**. First run: 6,195 passed, 23 failed. After fixing `ir02_04` (the one genuine new finding — see FAIL-017 above): 6,204 passed, 14 failed. All 14 are accounted for and none are regressions from this patch:
  - **9 by design**: the external, `#[ignore]`d, credential-gated originals this patch deliberately preserved unmodified as a manual escape hatch (5 in `scenario_daemon_order_submit.rs`, 3 in `scenario_native_strategy_bootstrap_daemon_b1a.rs`, 1 `b2a_n02` in `scenario_native_strategy_registry_b2a.rs`) — never run by CI or any default command, and correctly still fail without real Alpaca credentials.
  - **5 pre-existing, unrelated to this patch's scope, non-deterministic across the two full runs**: `scenario_alerts_events_cc06.rs::cc06_05`, `scenario_autonomous_daily_operation_api_01.rs::b26_history_response...`, `scenario_kill_switch_persistence_lo02e.rs::lo02e_ks03`, and doc-tests `mqk-portfolio/src/ledger.rs` / `mqk-strategy/src/plugin_registry.rs` (the latter two are genuinely-broken, deliberately-`ignore`-fenced doc examples with missing `use` statements — a real but pre-existing, unrelated defect). The *first* full run additionally showed `scenario_dynamic_selection_evidence_validation_01.rs` (6 tests, unrelated strategy-promotion-lifecycle state collision), `scenario_native_strategy_bridge_b1c.rs::b1c_c14`, and one `mqk-daemon --lib` test fail — none of which reappeared on the second full run with identical code, which is itself the evidence they are shared-`mqk_test`-database order-dependence (the same root-cause *class* as FAIL-017, but in files this patch was not scoped to touch), not a fixed defect this patch introduced or left behind. This is the first time the complete `--include-ignored` sweep has been run to completion in one pass in this repository's history (every prior session explicitly noted this as not-yet-done); running it is what surfaced these. **Recommend as a dedicated follow-up patch**: `FULL-AUDIT-SHARED-DB-ORDER-DEPENDENCE-SWEEP-01`, applying the same `mqk_db::run_isolated` mechanism this patch introduced to the remaining affected files.
  - Disposable-database residue: **zero** after both full sweeps (verified via `SELECT datname FROM pg_database WHERE datname LIKE 'mqk_disp_%'`).
- DB migrations: fresh `0001`→`0061` chain proven on a disposable database (`mqk_migration_chain_check`, dropped after use) — 36/36 `mqk-db --lib` tests passed against it. `check_migration_governance.sh`: PASS.
- GUI: `npm ci` (from scratch, 0 vulnerabilities via `npm audit`), `tsc --noEmit` + `vite build` (via `npm run build`), `npm run test` — 977/977 passed.
- Python: `compileall` clean; `pytest` — 988 passed, 5 skipped (documented reasons, unchanged), 12 subtests passed.
- Scripts: 185/185 tracked `.ps1` files parse clean; 8/8 tracked `.sh` files parse clean (`bash -n`). `tests/script_guards/run_all_script_guards.ps1` (63 guards, including the new heavy-lock test): 55 passed. The 8 remaining failures are all the same pre-existing "no out-of-scope files touched" class of guard from earlier, narrower patches, tripping only because this patch's own files are still uncommitted at the time of the check (`git diff`/`git diff --cached` against `core-rs/crates/mqk-daemon`); mechanically verified this clears once committed (a diff against `HEAD` after commit is empty for a committed, non-dirty tree). `scripts/guards/check_unsafe_patterns.sh`/`.ps1`, `check_no_phase7a_production_effects_bypass.ps1` (strengthened, negative-control-verified), `check_migration_governance.sh`, `check_workspace_dep_inheritance.sh`, `check_ignored_load_bearing_proofs.sh`, `check_multi_strategy_conflict_policy_01.sh`, `check_runtime_opportunity_allocation_01.sh`, `check_no_promotion_evidence_bypass.ps1`, `check_phase7a_final_closure.ps1`, `check_phase7a_r6_matrix_db_required.ps1` (26 real DB-backed effects-matrix tests, genuinely executed), `check_phase7b_selected_host_dispatch_closure.ps1`, `check_bundle7_phase7c_final_closure.ps1`, `check_ci_local_toolchain_convergence.sh` (new): all PASS.

### Honest completion standard

Per this patch's own disposition rules, "COMPLETE" requires the complete default and safe-ignored matrices to finish **green**. They do not: 14 failures remain in the full `--include-ignored` sweep. 9 are by design (this patch's own explicit instruction to preserve the external originals as a credential-gated manual escape hatch, not delete or force-pass them). 5 are pre-existing, non-deterministic, shared-database order-dependence in files this patch was never scoped to touch, first surfaced only because this is the first complete run of this exact matrix. Neither class is a defect introduced or left behind by this patch, and both are supported by reproducible evidence (two independent full-sweep runs) — but neither is "green" in the literal sense the completion standard requires, and this patch does not expand its own scope to fix unrelated files under a "no bundling" mandate. Disposition below is therefore **BLOCKED**, with every requested finding (FAIL-011/016/017/018) genuinely closed and the exact remaining gap fully evidenced, not glossed over.
