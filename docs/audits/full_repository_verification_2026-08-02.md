...
|---|---|---|
| FAIL-001 | CLOSED AS ENVIRONMENT LIMITATION WITH SUPPORTED WORKFLOW | Windows `CreateProcess` command-line limit is a real OS constraint, not a repo defect. `full_repo_proof.ps1` already carried the per-package fmt-check workaround (WINPATH-01) before this session; commit `d0ff02c4` added `-CargoTargetDir`, heavy-lock acquire/release, and the pinned toolchain around it. Verified: per-crate `cargo fmt --check` passes clean across all 21 crates. |
...
| FAIL-005 | CLOSED AS ENVIRONMENT LIMITATION WITH SUPPORTED WORKFLOW | Genuine Windows page-file/memory constraint, not a repo defect. The documented `CARGO_BUILD_JOBS=1`/`CARGO_INCREMENTAL=0`/`RUSTFLAGS=-C debuginfo=0`/`--test-threads=1` posture is the supported workaround, already present in CI and `full_repo_proof.ps1`; commit `d0ff02c4` strengthened the harness around it (explicit target dir, heavy-lock). |
...
| FAIL-011 | **NOT CLOSED — BLOCKED, architectural** | Investigated in depth (see below). Genuine finding: the daily-data-readiness gate's `evaluate_assignment` can only report `readiness_state="ready"` via a real `Some(&PgPool)` bar-provenance query (`daily_data_readiness.rs`); with `db: None` (these 6 tests' deliberate design — pure in-process, proving the *pre-DB* gate boundary), the gate can never return `start_allowed=true`, so it always 403-blocks before the tests' intended 503 DB-gate boundary. Confirmed empirically (not just by static reading): adding valid `MQK_STRATEGY_SYMBOL`/`MQK_STRATEGY_MD_TIMEFRAME` config changes *which* blocker fires (`required_assignments_missing` → per-assignment `db_unavailable`) but never reaches `start_allowed=true`. Closing this within "fixture repair only" scope would require either weakening the production gate (explicitly forbidden by this patch's own PRODUCTION BOUNDARY section) or converting these tests to genuine DB-backed fixtures with real seeded bar data (a materially larger change than "fixture repair," and contradicts the test files' explicit "pure in-process, no MQK_DATABASE_URL required" design documented in their own header comments). Recommend as a follow-up decision for the operator: either accept a narrowly-scoped production change (route a sole-blocker-is-db_unavailable readiness verdict to the existing `db_pool()?` 503 path instead of a 403), or accept these 6 tests' originally-provable boundary no longer exists post-DAILY-DATA-READINESS-01C-ENFORCEMENT-01 and rewrite their assertions (out of this patch's "no expected-gate-update" instruction). |
...
| FAIL-016 | **PARTIALLY CLOSED — 3 of 8 tests hit the same wall as FAIL-018** | `b1a_l04`/`l05`/`l06` (`scenario_native_strategy_bootstrap_daemon_b1a.rs`): their originally-diagnosed blocker (`strategy_registry_missing`) is genuinely fixed by `b408e796` (seeds a matching enabled `sys_strategy_registry` row, mirroring the proven `b2a_n02`/`n03` pattern) — verified this advances the failure past that gate. All three then hit a second, deeper wall identical to FAIL-018 (see below), which could not be safely closed. The other 5 tests (`scenario_daemon_order_submit.rs`, `deployment_mode_unproven`) were not attempted this session: closing them requires satisfying both this Paper+Alpaca deployment gate *and* the same daily-data-readiness machinery FAIL-011 investigated, but unlike FAIL-011's fixtures these do carry a real DB pool, so — unlike FAIL-011 — a genuine fix (seeded registries + real bar data, mirroring `scenario_daily_data_readiness_start_gate_01.rs`'s established pattern) is plausible but was not attempted within this session's remaining time. |
| FAIL-017 | CLOSED BY `1e7d6e23` | All 8 originally-listed tests pass, individually and within their own file's `#[ignore]`d batch, against the current isolated-DB state. 2 were re-diagnosed as genuine fixture-content bugs, not DB residue (the audit's own classification was incorrect for these two — see commit message); 2 (`h03`, `n05`) had real order-dependence bugs fixed (both relied on a global "latest run for this engine" resolution that a fixed-timestamp or narrowly-scoped cleanup couldn't guarantee); 4 (`b2a_n03`/`n04`, `ir02_04`/`ir02_06`) already passed cleanly against the current DB state and did not require a code change — re-verified, not re-broken and silently left alone. Not independently reproduced as flaky at the audit's original full ~428-test-sweep scale (this session did not re-run the complete ignored sweep to confirm the flakiness signature is fully gone at that scale, only within each affected test's own file). |
| FAIL-018 | **NOT CLOSED — BLOCKED, same root cause as FAIL-016's partial** | Investigated exhaustively; no safe fix exists within this session's constraints. `b2a_n02` uses `LiveShadow+Alpaca`, whose `BrokerSnapshotTruthSource` is `External` — reaching a 200 response requires constructing a real `AlpacaBrokerAdapter` and driving it through at least one real HTTP call. Two potential mitigations were investigated and rejected: (1) an obviously-fake `ALPACA_API_KEY_LIVE`/`SECRET` plus pre-seeding `AppState::broker_snapshot` (to skip the known `fetch_broker_snapshot` call site) was tried and *reverted* after direct testing showed a second, different code path (`build_execution_orchestrator`'s initial-tick `fetch_events()`) still made a real blocking HTTP call to Alpaca's live API and received a genuine "unauthorized" response from Alpaca's real server — a real network call, forbidden regardless of whether the credentials are fake; the stray halted `runs` row this left in the shared DB was identified and cleaned. (2) The codebase's own existing hermetic-broker test seam (`hermetic_test_broker_override_enabled`/`set_hermetic_test_broker_override_for_test`, which swaps in an in-process `LockedPaperBroker` with zero network I/O) is `#[cfg(test)]`-gated at the source level — it does not even compile into the library artifact that external `tests/*.rs` integration tests link against, so it is structurally unreachable from `b2a_n02` without removing that `#[cfg(test)]` gate, which would be a genuine production-surface change (a live daemon binary would then contain an always-present, if defaulted-off, "swap in a fake broker" switch) explicitly out of this patch's scope. The negative-test half of Part 6's requirement is already satisfied and unaffected: `state::tests::build_daemon_broker_alpaca_live_shadow_requires_credentials` (existing, `mqk-daemon --lib`) already proves missing credentials fail closed, verified passing. Recommend as a follow-up decision for the operator: either authorize a scoped, explicit production change to widen the hermetic-broker seam's visibility for exactly this purpose, or accept `b2a_n02`/`b1a_l04`-`l06` as permanently `#[ignore]`d-and-unrunnable-without-real-credentials by design. |
