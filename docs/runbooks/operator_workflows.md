...
# Paper — full startup (delegates to Launch-VeritasLedger.ps1 + the daemon's
# required-universe market-data scheduler + reconcile + halt-recovery; never
# calls start-system)
...
**Market-data authority (PAPER-OPS-AUTOFRESH-LAUNCHER-INTEGRATION-01):** both
`-Mode Paper` and `-Mode Paper -Scheduled` rely on the daemon's own
required-universe market-data scheduler
(`POST`/`GET /api/v1/market-data/required-universe/{start,status}`) as their
sole ongoing market-data maintenance authority, established before
reconcile/halt-recovery/arm and verified fail-closed (a `200`/`409` response
is never itself treated as proof). The official launcher does **not** start
or own a `Refresh-IntradayMarketData.ps1` child process during normal Paper
startup, and does not run `Prep-PremarketMarketData.ps1` — the daemon
scheduler's own immediate cycle already covers strict readiness evaluation,
bounded historical bootstrap, latest-bar repair, and provider mapping.
...
| Script | Purpose |
|--------|---------|
| `Start-MiniQuantDesk.ps1` | **Official entrypoint** — Paper/Live mode selection, orchestrates `Launch-VeritasLedger.ps1` and the daemon's required-universe market-data scheduler |
...
| `Prep-PremarketMarketData.ps1` | Manual/compatibility market-data prep or check — no daemon, no orders. Not invoked by `Start-MiniQuantDesk.ps1`'s official Paper startup (the daemon required-universe scheduler covers that role); still used directly by `Launch-VeritasLedger.ps1 -PrepMarketData`/`-CheckMarketData` below |
| `Refresh-IntradayMarketData.ps1` | Manual/compatibility recurring intraday bar refresh loop. Not started by `Start-MiniQuantDesk.ps1`'s official Paper startup (the daemon required-universe scheduler is the sole ongoing authority there); remains available for standalone/manual use and carries its own conflict guard against a running required-universe scheduler |
