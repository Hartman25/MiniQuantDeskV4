...
All eleven sub-slices are `CLOSED_LOCAL`; the parent `BACKTEST-MULTIPLIER-MARGIN-01`
label itself has never been marked closed in the ledger — every sub-slice's
closing paragraph explicitly states it "remains `PARTIAL`."
...
| CLI `mqk backtest csv-sweep` | **No** | `BacktestCmd::CsvSweep` (`mqk-cli/src/main.rs`) has no `contract_multiplier`/`initial_margin_micros`/`maintenance_margin_micros` fields. `run_sweep_csv` (`mqk-cli/src/commands/bkt.rs`) constructs `BacktestEngine::new(cfg)` per sweep point and never calls `.with_economics(...)`. Every sweep run — regardless of what an operator might want — is silently forced to default equity economics (multiplier=1). This is the one confirmed gap; see §6. |
...
| Source | Status |
|---|---|
| Explicit CLI flags (`csv`, `db`) | Live, tested, fail-closed on non-positive multiplier. |
| Explicit CLI flags (`csv-sweep`) | **Absent** — the confirmed gap. |
...
| Default equity economics | `BacktestEngine::new(cfg)` without `.with_economics(...)` defaults to `BacktestInstrumentEconomics::equity()` (multiplier=1, no margin) everywhere — byte-identical to pre-economics behavior; this is what every `csv-sweep` run gets today regardless of operator intent. |
...
- **Sweep support.** `mqk backtest csv-sweep` has no economics flags and cannot produce a multiplier-aware sweep run today. This is a backtest-only gap (see §6/§9 of the closure decision).
...
**`PARTIAL / SAFE-GAPS-REMAIN`** as of this audit's start (before Phase B).
The exact remaining safe gap is `mqk backtest csv-sweep`'s missing economics
flags (§3, §6) — genuinely backtest-only, additive, opt-in, and closable
without touching any forbidden file. See
[`backtest_multiplier_margin_01_closure_decision.md`](backtest_multiplier_margin_01_closure_decision.md)
for the final status after Phase B/C of this session.

---

## 8. Recommended next patch

If Phase B closes the sweep gap cleanly: proceed to Phase C's closure
decision, most likely landing on `CLOSED_LOCAL / BACKTEST-COMPLETE` for
backtest-side multiplier/margin economics, with margin enforcement and a
real non-equity registry-v2 data source named as explicitly deferred,
separate future patches. That in turn satisfies prerequisite #1 of
`ASSET-CORE-01H`'s production-cutover boundary (`docs/specs/asset_core_01h_instrument_registry_v2_consumption_boundary_decision.md`),
though it does not by itself authorize `REGISTRY-V2-PRODUCTION-CUTOVER-DECISION-01`
