...
| `ASSET-CORE-03` (Asset-Aware Risk Router) | `CLOSED_LOCAL / PARTIAL` (per `ASSET-CORE-03-RISK-ROUTER-FOUNDATION-01-COMBINED`; see `docs/specs/asset_core_02_03a_current_completion_audit.md` for current-session re-audit) | Production-consumption-open, `ASSET-CORE-04`-dependent for graduated live enforcement | Two tested fail-closed gates exist (Gate 0, routing guard) **plus** a graduated per-class static policy model (`mqk_execution::asset_risk_policy`) — this corrects the "zero graduated per-class policy behind them" claim previously recorded on this line; that policy model already existed before this reconcile pass but was not reflected here. It is not yet operator-surfaced by any daemon route or GUI panel, and true live enforcement (margin/multiplier/NAV blocking real orders) requires `ASSET-CORE-04` live accounting, not yet wired. |
