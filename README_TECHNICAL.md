...
current-operation panel rendered blockers). **F1 (plus this repair) is now
accepted.** On top of the accepted F1 head,
`AUTONOMOUS-DAILY-PAPER-OPERATIONS-01F2-OPERATOR-RUNBOOK-CORRECTION` updates
the canonical operator runbook (`docs/runbooks/autonomous_paper_ops.md`) in
place: a safety-boundary section (Paper + Alpaca only, single-symbol
long-only US equity/ETF, active operator supervision required, unattended
soak not started, live capital not ready), an operating-vs-test-vs-
reality-test database port table, the five authoritative read-only routes
with their full truth-state/finalization-status/evidence vocabulary, a
before-session checklist, during-session supervision guidance, bounded
recovery procedures that never invent a manual finalization command or DB
rewrite, stop/emergency posture, end-of-day evidence capture, and restart
distinctions (before finalization / after a terminal commit / after an
evidence blocker). Documentation and validation only — no daemon, GUI, or
migration file is touched. **F2 (plus the final operational-safety repair
below) is now accepted.** On top of F2,
`AUTONOMOUS-DAILY-PAPER-OPERATIONS-01F3-SUPERVISED-SOAK-EVIDENCE-PREPARATION`
prepares read-only evidence-capture tooling for future supervised sessions —
`scripts/soak/capture_autonomous_paper_session_evidence.ps1` (GET-only,
fail-closed to a local daemon host, no `.env.local` read, no credential
handling, `-ValidateOnly`/`-FixturePath` safe modes),
`scripts/soak/validate_autonomous_paper_session_evidence.ps1` (schema,
truth-state, null-count, and secret-pattern validation), the
`autonomous-paper-soak-evidence-v1` manifest template, an operator
...
final-repair-implementation complete, awaiting final ChatGPT/operator
acceptance.** On top of F3,
...
behavior, migration, or real external call. **F2 is now accepted; F3 and
Phase G are each repair-implementation complete, awaiting final ChatGPT and
operator acceptance.**

Independent proof-integrity review of that repair then found: the E2A-E4
guards' own "Phase E: ACCEPTED"/E5's "E5: ACCEPTED" forbidden-claim entries
had gone stale the instant Phase E was genuinely accepted (each guard
permanently failed against the truthful, required README status line); the
Phase G guard only ever transitively re-ran F1→F2→F3 and never explicitly
invoked a single D-guard or E-guard; the Phase G committed-range proof used
a permanently-widening `4b6eec72..HEAD` window instead of a fixed range; and
the F3 evidence validator accepted several coercible/unsafe shapes
(`operator_supervised` coercing from `1`, `deployment_mode`/`adapter_id`
accepting a case- or whitespace-mismatched string, `live_routing_enabled`
accepting a non-Boolean, `repository_commit` accepting any nonempty string
including raw Git error text, and a `Double`/negative count silently passing
as valid).
...
forbidden-claim entries are retired from the E2A/E2B/E3/E4/E5 guards with
every genuine implementation invariant preserved; the Phase G guard now
explicitly invokes the full D1/D4/E1-E5/F1-F3 guard matrix plus
`check_unsafe_patterns.ps1` by exact filename, reporting any failure by
exact filename and exit code; the Bundle 3 committed-range proof is now a
fixed, self-locating range bounded by the exact commit subject `fix: close
bundle 3 proof and evidence gaps` (never `..HEAD`), covering both the
`4b6eec72..<repair commit>` no-migration/no-production-Rust proof and the
post-F1 `bd7336d4..<repair commit>` no-GUI-behavior-change proof; the F3
capture script now captures `repository_commit` via
...
checks, a strict integer-or-null count helper covering every retained
`query_failed` row, a bounded `capture_errors` shape check, and an
independent `daemon_base_url` path check. The local-fixture suite grew from
16 to 35 scenarios. No production Rust, daemon API, migration, GUI behavior,
broker, provider, trading, order, or live-capital change.
**BUNDLE 3: ACCEPTED — COMPLETE.** Bundle 3 (D1 through Phase G, including
the final guard-and-evidence-integrity repair) has now received independent
ChatGPT/operator acceptance as a whole. This repository does not mark
Bundle 3 "closed" (a distinct, narrower claim) — only accepted-complete. On
top of Bundle 3's now-accepted implementation, `DURABLE-PAPER-PORTFOLIO-AND-PNL-01-COMBINED`
(Bundle 4, B4-0 through B4-G — durable paper portfolio snapshot persistence,
restart-safe FIFO fill accounting, read-only API/GUI visibility, and an
integrated closure proof, all against the isolated port-5434 test database
with real production seams, no synthetic broker events, no fabricated
fills/marks/P&L) has now received independent ChatGPT/operator acceptance —
**BUNDLE 4: ACCEPTED — COMPLETE**, accepted head
`750b52809c569982dfe773c52be7c015227ff048` (see § "Active Bundle 4 boundary"
...
### Active Bundle 3 boundary (accepted complete)

`AUTONOMOUS-DAILY-PAPER-OPERATIONS-01-COMBINED` is **ACCEPTED — COMPLETE**
(D1 through Phase G, including the final guard-and-evidence-integrity
repair) — independent ChatGPT/operator acceptance received. This repository
does not mark it "closed" (a distinct, narrower claim). The narrative below
records the implementation history; see § "Active Bundle 4 boundary" for the
current bundle built on top of it.
...
E closure proof) are all accepted — Phase E is accepted complete in full. Phase F1 (the read-only
GUI daily-operation truth projection), Phase F2 (operator runbook correction), F3 (supervised
soak-evidence preparation), and Phase G (the final Bundle 3 closure audit, plus its final
operational-safety and guard-and-evidence-integrity repairs) are all accepted. **Bundle 3 has
received independent ChatGPT/operator acceptance — BUNDLE 3: ACCEPTED — COMPLETE.** This repository
does not mark it "closed" (a distinct, narrower claim it has not made).

### Active Bundle 4 boundary

On top of Bundle 3's implementation, `DURABLE-PAPER-PORTFOLIO-AND-PNL-01-COMBINED` closes the durable,
restart-surviving portfolio and P&L truth gap for paper + Alpaca, single-symbol long-only US
equity/ETF, supervised. Eight phases, one commit per phase, each proven against the isolated
port-5434 test database with real production seams:

- **B4-0** stabilizes `scenario_autonomous_completed_bar_driver_01.rs`'s `standard_timing()` fixture
  to a floored `Utc::now()` anchor (previously a fixed calendar date that aged out from under the
  real-wall-clock staleness gate), and fixes a cross-test DB race in the same file's cleanup helper
- **B4-A** is a read-only current-truth audit and binding contract: no existing funnel durably
  persists the real broker snapshot; `oms_inbox` is reused as the durable fill-event source (no new
  fill table); realized P&L must be explicitly incomplete, never fabricated, when fill history is
  partial
- **B4-B** adds migration `0053` (`sys_paper_portfolio_snapshots`/`_positions`,
  `sys_paper_portfolio_accounting_state`) and the `mqk-db::paper_portfolio` idempotent, fail-closed
  store module
- **B4-C** wires `accept_external_broker_snapshot` into the run-start cold-fetch, periodic refresh,
  and terminal-fill-expiry-refresher call sites as the sole writer of the durable snapshot
- **B4-D** adds durable FIFO fill accounting (`replay_paper_portfolio_accounting`), reusing
  `recover_oms_and_portfolio`'s own duplicate-fill guard rather than a second replay algorithm
- **B4-E** adds three read-only routes (`durable-summary`/`durable-positions`/`durable-snapshots`)
  and wires `GET /api/v1/execution/paper-lifecycle`'s portfolio/P&L truth states to the same durable
  tables
- **B4-F** adds the operator GUI's `DurablePortfolioSection`, runbook sections, and extends the
  Bundle 3 soak-evidence capture/validate tooling to the new durable truth
- **B4-G** is the integrated closure proof (snapshot durability, exactly-once accounting,
  restart-identical replay, fabrication-free incomplete-history handling, independently
  positive/negative/zero realized P&L, and the full chain proven through the real
  `paper-lifecycle` route) and the final Bundle 4 audit/guard

No synthetic broker lifecycle events, no fabricated fills/marks/baselines/positions/cash/equity/P&L,
and no live-capital, multi-symbol, or unattended-soak change anywhere in Bundle 4. **B4-0 through
B4-G, including the final read-side authority repair and closure proof (below), have now received
independent ChatGPT/operator acceptance — BUNDLE 4: ACCEPTED — COMPLETE, accepted head
`750b52809c569982dfe773c52be7c015227ff048`.**
...
B4-B through B4-G, including the final read-side authority repair and closure proof, have now
received independent ChatGPT/operator acceptance — **BUNDLE 4: ACCEPTED — COMPLETE**, accepted head
`750b52809c569982dfe773c52be7c015227ff048`.

### Operational meaning

Bundle 3 and Bundle 4 (durable paper portfolio and P&L truth) are both now accepted. Acceptance
...
1. Bundle 3 — **accepted complete**
2. Bundle 4 durable paper portfolio and P&L truth — **accepted complete**; accounting may now be
   treated as restart-safe
3. run final targeted and full locked-snapshot proof
4. start with operator-watched, **supervised** Paper + Alpaca sessions (authorized, not yet started)
5. collect roughly 10–20 clean supervised sessions; only then seek separate authorization for an
   unattended soak
6. close real-fill, reconcile, Discord, restart, and repeated-cycle evidence
...
The path is now in **supervised-soak-authorized posture, with both Bundle 3 and Bundle 4 accepted**.
Phase D (D1–D4) is accepted complete in full; the Phase E1 contract
audit (four-times-corrected) is **accepted complete**; Phase E2A (durable coverage-anchor/run-lineage
evidence foundation), plus both repairs, is **accepted complete**; Phase E2B (strict outcome
classifier and finalization CAS) is **accepted complete**; Phase E3 (coordinator finalization
integration and notification) is **accepted complete**; Phase E4 (the read-only daily-operation API,
plus both repairs) is **accepted complete**; Phase E5 (the integrated Phase E closure proof) is
**accepted complete — Phase E is accepted complete in full**. Phase F1 (the read-only GUI
daily-operation truth projection), Phase F2 (operator runbook correction), Phase F3 (supervised
soak-evidence preparation), and Phase G (final Bundle 3 closure audit) — including the final
operational-safety repair
(`AUTONOMOUS-DAILY-PAPER-OPERATIONS-01-F2-F3-G-FINAL-OPERATIONAL-SAFETY-REPAIR`) and the further
guard-and-evidence-integrity repair
(`AUTONOMOUS-DAILY-PAPER-OPERATIONS-01-BUNDLE-3-FINAL-GUARD-AND-EVIDENCE-INTEGRITY-REPAIR`) — are all
**accepted complete**. **Bundle 3 has received independent ChatGPT/operator acceptance —
BUNDLE 3: ACCEPTED — COMPLETE.** This repository does not mark it "closed" (a distinct, narrower
claim it has not made).

On top of Bundle 3's implementation, `DURABLE-PAPER-PORTFOLIO-AND-PNL-01-COMBINED` (Bundle 4, B4-0 through
B4-G) has now received independent ChatGPT/operator acceptance — **BUNDLE 4: ACCEPTED — COMPLETE**,
accepted head `750b52809c569982dfe773c52be7c015227ff048` (see § "Active Bundle 4 boundary"). This
authorizes a **supervised** Paper + Alpaca soak; it does not authorize live capital, does not claim
any soak session has occurred, and does not authorize the unattended 10–20-session soak.

### What Bundle 4 acceptance means operationally

With Bundle 4 accepted, the daemon should be able to remain running across supported NYSE sessions,
manage the daily Paper + Alpaca lifecycle without a person manually starting and stopping each run,
and maintain trustworthy durable paper cash, positions, lots, cost basis, and daily realized/
unrealized P&L across restarts.
...
Use the first sessions as a controlled, **supervised** soak (authorized, not yet started). The
project plan is roughly 10–20 clean supervised sessions before separately seeking authorization for
an unattended soak and any broader rollout.
...
- Bundle 4 (`DURABLE-PAPER-PORTFOLIO-AND-PNL-01-COMBINED`, durable paper cash/positions/lots/cost basis/P&L truth, B4-0 through B4-G) is **accepted complete** (independent ChatGPT/operator acceptance received, accepted head `750b52809c569982dfe773c52be7c015227ff048`)
- this acceptance authorizes a **supervised** Paper + Alpaca soak under active operator supervision (authorized, not yet started); it does not authorize an unattended soak (still not yet authorized) and does not authorize live capital; Bundle 5 is not started and must occur in a separate branch/worktree, not against the frozen soak baseline
