...
# D-R2-R4: the canonical local Paper Postgres classification (the ONE narrow
# exemption below) is loaded from a shared helper so this pre-stage scan and
# Invoke-MiniQuantDeskOffsiteBackup.ps1's independent post-restore re-scan
# can never semantically drift from each other -- see lib\CanonicalPaperDbCanary.ps1
# for the exact structural contract.
