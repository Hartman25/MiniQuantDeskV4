...
was 21 at the moment of the Phase N commit and is 3 as of this update.
...
#### CLOSED (this normalization pass, updated through the burn phase) = 43
...
`MD-ALPACA-FETCH-RETRY-BACKOFF-01`, `DISCORD-CHANNEL-ROUTING-01`,
`DISCORD-DATA-STALENESS-ALERT-01`, `DISCORD-DAILY-SUMMARY-PUSH-01` (all
eighteen: burn-phase closures, 2026-09-07 — see above),
...
**Sum check (as of the burn-phase update above):** 3 + 8 + 0 + 3 + 4 + 4 + 7
+ 1 + 43 = **73**. Matches the `04-active-row-table.csv` row count exactly —
every row accounted for in exactly one bucket, no hidden subset counts.

**`ACTIVE_IMPLEMENTATION_LEDGER = 3`** is the operative denominator as of the
...
**Status:** OPEN — newly discovered while validating W1-07, not fixed in this wave · **Priority:** P3 · **Paper Impact:** GREEN (test-only code; no production/trading path) · **Subsystem:** mqk-cli / CI guards
