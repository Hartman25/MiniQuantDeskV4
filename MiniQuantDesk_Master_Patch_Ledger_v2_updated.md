...
**Status:** OPEN (new, discovered `MQK-LEDGER-BURN-CONTROLLER-02` W2-B, 2026-09-06) · **Priority:** P2 · **Paper Impact:** GREEN (test-only; no production behavior change made or proposed) · **Subsystem:** `mqk-daemon` test suite / `required_universe_scheduler`
**Current Source Truth:** `core-rs/crates/mqk-daemon/tests/scenario_market_data_autofresh_required_universe_01.rs`, `stop_start_generation_race_old_cycle_cannot_overwrite_new_owner` — fails 4/4 in isolation on branch `post-wave06-ledger-burn-01` at HEAD `889db5f0`, always at the same line (`call_started.notified()` timeout, `A's provider call must start within 10s: Elapsed(())`), ~10.1s per run. The sibling test `concurrent_scheduler_start_admits_exactly_one_starter` (same file, same `start_required_universe_scheduler` entrypoint, no `BarrierDelayedProvider`) passes in 0.04s in the same session — isolates the defect to this test's barrier-delayed-provider concurrent-start path, not the scheduler function generally.
**Problem:** Either (a) a genuine regression in `start_required_universe_scheduler`'s historical-bootstrap provider-call path specific to this branch/HEAD, (b) a `BarrierDelayedProvider`/`tokio::sync::Notify` test-fixture race (e.g. a lost `notify_one` permit), or (c) an environment/DB-schema drift between this worktree's disposable `mqk_test` DB and what this branch's scheduler code now expects (unconfirmed — not diagnosed this session, per this investigation row's own no-fix-here scope).
**Dependencies:** None. References `PAPER-SOAK-RUST-TIMING-TEST-HARDENING-01` (the investigation that discovered this) and `PAPER-OPS-AUTOFRESH-LAUNCHER-INTEGRATION-01` (original discrepancy record, different branch).
**In Scope (future patch):** Root-cause exactly which of (a)/(b)/(c) above is the real cause, using deterministic barriers/logging (no sleep/timeout increases, no retries, no `#[ignore]`), then fix the real cause. **Out of Scope (this row, as filed):** Any Rust source or test change — none was made or attempted.
**Required Validation (future patch):** `stop_start_generation_race_old_cycle_cannot_overwrite_new_owner` passes deterministically, repeated at least 10x with zero failures, with a clear root-cause note distinguishing (a)/(b)/(c).
**Exact CLOSED End State:** CLOSED when the real cause is identified and fixed (or, if (c), when the disposable test DB / migration state is confirmed and reconciled) and the target test passes reliably.
**Acceptance History:** PENDING / PENDING / PENDING / PENDING.
...
**Status:** IMPLEMENTATION_COMPLETE_OPERATOR_VALIDATION_DEFERRED (`MQK-LEDGER-BURN-CONTROLLER-02` W2-E, 2026-09-06, commit `0e98a9ed`) — engineering portion only; see `DEFERRED_OPERATOR_VALIDATION` note below · **Priority:** P1 · **Paper Impact:** GREEN (additive tooling, zero shared runtime, LiveShadow = no capital risk) · **Subsystem:** Ops tooling
**W2-E finding/implementation:** `scripts\windows\Start-LiveShadowSmoke.ps1` (new) wraps the canonical launcher (`Start-MiniQuantDesk.ps1 -Mode Live`) with LiveShadow-specific framing and deterministic evidence capture, rather than reimplementing broker/daemon-start logic. Investigation found that launcher's own `-Mode Live` path is itself documented read-only/report-only today ("this patch NEVER starts a live daemon process, submits an order, or mutates a live account"), with or without `-CheckOnly` — so this wrapper cannot itself cause any real daemon start, broker call, or order regardless of which path is invoked; a genuine real-operational LiveShadow smoke remains blocked on that upstream capability not existing yet, not on anything this row could add. Forces `MQK_DAEMON_DEPLOYMENT_MODE=live-shadow` (the file's one and only assignment to that variable, verified structurally); never calls `Invoke-WebRequest`/`Invoke-RestMethod`/`curl` directly; never references an order/arm/halt/reconcile route literal. `CheckOnly` is the fail-closed default. Proven by `tests/script_guards/test_live_shadow_smoke.ps1`, 8/8 (structural guards plus one real, safe `-CheckOnly` invocation whose manifest confirms `real_daemon_start_performed`/`real_broker_call_performed`/`real_order_submitted` all `false`). **DEFERRED_OPERATOR_VALIDATION:** the row's own second half — "perform a real operational smoke" — remains genuinely blocked (not merely postponed by this session) until a future patch adds real LiveShadow daemon-start capability to the canonical launcher; this script is built to delegate to that capability once it exists rather than growing a parallel implementation.
