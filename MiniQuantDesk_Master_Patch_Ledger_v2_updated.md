...
| A3B `LIVE-TINY-CAPITAL-SMOKE-01` | Engineering now genuinely `IMPLEMENTATION_COMPLETE_OPERATOR_VALIDATION_DEFERRED` (not merely re-asserted) — `Start-LiveShadowSmoke.ps1` now delegates to the real `-Mode LiveShadow` (closing Controller-02 defect: it previously delegated to LiveCapital's inert `-Mode Live`, which structurally could not run a full smoke and inherited `Confirm-LiveIntent`'s prompt). `manifest.json` no longer hardcodes `real_daemon_start_performed`/`real_broker_call_performed`/`real_order_submitted` to a fabricated `$false`; it now distinguishes wrapper-static-contract facts (provably `$false`, unconditionally) from observed-runtime-evidence (`'not_run'`/`'observed_true'`/`'observed_false'`/`'not_observed'`, derived from the canonical launcher's own JSON log, never invented). 11/11 guard proofs, including one real `-CheckOnly` invocation. Only the real operational LiveShadow smoke remains deferred. | `9d99b407` |
