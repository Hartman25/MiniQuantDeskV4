...
cross-sectional rank hypothesis (LIQ-01: the Amihud illiquidity risk premium
-- Amihud (2002), "Illiquidity and Stock Returns: Cross-Section and
Time-Series Effects", Journal of Financial Markets 5(1):31-56; Pastor &
Stambaugh (2003), "Liquidity Risk and Expected Stock Returns", Journal of
Political Economy 111(3):642-685: investors demand compensation for holding
names that are costly to trade, so illiquid names have historically earned a
return premium), run as a matched LONG-ONLY control / LONG-SHORT candidate
pair plus a causal same-horizon placebo, using the existing accepted
Research -> registered walk-forward -> economic evaluation ->
multiple-testing-judge pipeline, over the SAME 88-symbol broad Research
universe seed used by SHORT-WAVE-03 and DISCOVERY-01 (see SEED_UNIVERSE.json;
identical universe_id, reused unchanged, not regenerated).
...
DATA WINDOW: byte-for-byte reuse of DISCOVERY-01's own frozen data window
(2016-01-01..2025-05-01, ASOF=2025-05-01). DISCOVERY-01 (commit 3cf711cf)
already discovered, PRE-OUTCOME, that this exact 88-symbol universe's shared
bars pipeline fail-closed CorporateActionReviewRequired gate refuses any
end_utc at or after 2025-05-27 (unresolved 2025-2026 "name_change"
corporate-action events on CCL, RKLB, XOM). Reusing that already-established
safe boundary here -- rather than re-declaring a later end_utc that would
just re-hit the identical, already-documented gate -- is the correct
pre-outcome choice; it is not a reaction to any Wave06 economic outcome
(none has been observed at predeclaration time), and it preserves this
campaign's single narrow pre-outcome data-boundary amendment allowance for a
genuinely new discovery, not a re-run of an already-known one. The reserved
final holdout window is 2024-11-01..2025-05-01 (WF_HOLDOUT_MONTHS=6) and
remains genuinely untouched -- its holdout_id is content-bound to this
candidate's own feature/bars dataset identity (see
mqk_research.ml.holdout_ledger.compute_holdout_id) and is therefore
structurally distinct from DISCOVERY-01's own reserved holdout even though
the calendar window is identical.
"""
...
# module docstring "DATA WINDOW" for the full pre-outcome provenance account
# (DISCOVERY-01 commit 3cf711cf discovered this exact universe's fail-closed
# CorporateActionReviewRequired boundary at RKLB's 2025-05-27 name_change
# event; reusing that already-known boundary here is not a reaction to any
# Wave06 outcome, since none has been observed at predeclaration time).
...
FAMILIES: dict[str, HypothesisFamily] = {
    "LIQ-01": HypothesisFamily(
        key="LIQ-01", feature_column=FEATURE_COLUMN,
        strategy_id="pooled_single_feature_xs_amihud_illiquidity_direct_rank_v1",
        hyp_long_only="wave06_liq01_amihud_illiquidity_long_only_v1",
        hyp_long_short="wave06_liq01_amihud_illiquidity_long_short_v1",
        hyp_placebo="wave06_liq01_amihud_illiquidity_placebo_v1",
    ),
}
...
def ensure_full_features(bars: pd.DataFrame) -> pd.DataFrame:
    ...
    already-verified in-memory `bars`, never existence-cached."""
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not argv:
        print("usage: run_wave.py {check|liq01|judge} [--execute]", file=sys.stderr)
        raise SystemExit(2)
    stage = argv[0]
