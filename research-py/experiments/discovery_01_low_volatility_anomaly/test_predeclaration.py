"""DISCOVERY-01-LOW-VOLATILITY-ANOMALY-PREDECLARATION-01 -- proves the
predeclaration is internally consistent, network-free in check mode, and
hard-guarded against accidental real execution. Adapted from
short_wave_03_broad_direct_rank/test_predeclaration.py (same required-test
numbering convention), trimmed to this experiment's single RISK-01 family.
...
def test_exactly_two_real_candidates() -> None:
    ...
    family (RISK-01) -> exactly 2 real candidates (long_only, long_short)."""
    decl = _decl()
...
def test_feature_is_new_and_distinct_from_every_prior_family() -> None:
    """REQUIRED TEST 12: the single predeclared feature (vol_rank_20) is not
    any of the five feature columns already tested by ALPHA-01/SHORT-01/
    SHORT-WAVE-02/03 (momentum_score, slope_60, ret_rank_20, ret_5,
    gap_pct_1) -- guards against silently relabeling a retuned variant of a
    REJECTED_NOT_ADVANCED / INCONCLUSIVE mechanism as a new discovery."""
    decl = _decl()
    feature_columns = decl["hypotheses"]["RISK-01"]["feature_columns"]
    assert feature_columns == ["vol_rank_20"]
    already_tested = {"momentum_score", "slope_60", "ret_rank_20", "ret_5", "gap_pct_1"}
...
def test_placebo_seed_is_frozen() -> None:
    """REQUIRED TEST 16."""
    assert _decl()["placebo_seed"] == 4242
    assert run_wave.PLACEBO_SEED == 4242
