"""W06-A-CAMPAIGN-PREDECLARATION-01 -- development-stage Research experiment
driver for ONE predeclared, genuinely new single-feature OOS-classifier-score
cross-sectional rank hypothesis (VOL-01: the high-volume / investor-attention
return premium -- Gervais, Kaniel & Mingelgrin (2001), "The High-Volume
Return Premium", Journal of Finance 56(3):877-919; Barber & Odean (2008),
"All That Glitters", Review of Financial Studies 21(2):785-818: a stock
trading at unusually high volume relative to its own recent history draws
disproportionate investor attention, producing a documented subsequent
short-horizon return premium), run as a matched LONG-ONLY control /
LONG-SHORT candidate pair plus a causal same-horizon placebo, using the
existing accepted Research -> registered walk-forward -> economic
evaluation -> multiple-testing-judge pipeline, over the SAME 88-symbol
broad Research universe seed used by SHORT-WAVE-03, DISCOVERY-01, and this
campaign's own LIQ-01 candidate (see SEED_UNIVERSE.json; identical
universe_id, reused unchanged, not regenerated).

This driver is forked, with minimal targeted identity/config/feature edits
only, from the already-reviewed, already-executed
research-py/experiments/discovery_01_low_volatility_anomaly/run_wave.py
(itself forked from short_wave_03_broad_direct_rank/run_wave.py). All shared
...
derivation) is REUSED VERBATIM AND UNCHANGED from that proven lineage --
...
The underlying statistic (`vol_ratio`, a symbol's own current-day volume
divided by that SAME symbol's own current-bar-INCLUSIVE trailing 20-day
rolling-mean volume -- feature_set_v1.py computes this via pandas
`vol.rolling(20, min_periods=20).mean()`, whose window includes the current
row) is NOT new code -- it is already unconditionally computed by the
existing, unmodified mqk_research.features.feature_set_v1.build_feature_set_v1,
but has never been used, in any form, as a ranking/classifier feature in any
prior REGISTERED MiniQuantDesk research experiment: ALPHA-01 (momentum_score),
SHORT-01 (slope_60), SHORT-WAVE-02/03 (ret_rank_20, ret_5, gap_pct_1),
DISCOVERY-01 (vol_rank_20), or this campaign's own LIQ-01
(illiquidity_amihud_daily_xs_rank) -- vol_ratio is a temporally-RELATIVE
own-history statistic (demand-shock/attention signal), definitionally
distinct from LIQ-01's cross-sectional LEVEL statistic (price-impact cost)
even though both use volume data. (The separate, non-registered
research-py/experiments/exp_penny screener already uses volume/ADV data
outside this hypothesis-registration lineage, so this is not the first use
of volume data in the repository's history overall.) Only the
cross-sectional percentile-rank transform (`vol_ratio_rank_20`, computed
locally by `add_cross_sectional_rank` below using the exact same
...
LITERATURE SCOPE: Gervais, Kaniel & Mingelgrin (2001) report unusually
high/low trading volume over a day/week followed by subsequent return
differences over roughly the next month. This exact candidate uses current
volume / current-inclusive rolling-20-day mean volume, then a same-date
cross-sectional rank and classifier -- label it a
Gervais-Kaniel-Mingelgrin-inspired implementation, not a literal
reproduction of the paper's portfolio construction.
...
CAMPAIGN ORDER: this candidate is order_position=2 in
../wave06_campaign/PREDECLARED_CAMPAIGN.json. main() calls
campaign_order_guard.require_authorized_to_execute("VOL-01", ...) before
any real execution -- this refuses to run unless LIQ-01 already has a
registry-verified closeout classified REJECTED_NOT_ADVANCED or INCONCLUSIVE
under the frozen advancement_policy. See wave06_campaign/campaign_order_guard.py.
...
the same way SHORT-WAVE-02/03/DISCOVERY-01's own run_wave.py inlined it
(portability convention) so this driver is runnable from a bare checkout of
this branch alone. Semantics unchanged.

BENCHMARK: family-specific DYNAMIC equal-weight comparator, identical
formula/implementation to SHORT-WAVE-03's/DISCOVERY-01's own
`build_dynamic_rankable_benchmark` (already implemented and exercised there
-- see run_01 local evidence's
benchmark_type="dynamic_equal_weight_causally_rankable_fold_reset_v1"),
reused verbatim unchanged.

DATA WINDOW: byte-for-byte reuse of DISCOVERY-01's / this campaign's own
LIQ-01's frozen data window (2016-01-01..2025-05-01, ASOF=2025-05-01).
DISCOVERY-01 (commit 3cf711cf) already discovered, PRE-OUTCOME, that this
exact 88-symbol universe's shared bars pipeline fail-closed
CorporateActionReviewRequired gate refuses any end_utc at or after
2025-05-27 (unresolved 2025-2026 "name_change" corporate-action events on
CCL, RKLB, XOM). Reusing that already-established safe boundary here --
rather than re-declaring a later end_utc that would just re-hit the
identical, already-documented gate -- is the correct pre-outcome choice; it
is not a reaction to any Wave06 economic outcome (none has been observed at
predeclaration time), and it preserves this campaign's single narrow
pre-outcome data-boundary amendment allowance for a genuinely new discovery,
not a re-run of an already-known one. The reserved final holdout window is
2024-11-01..2025-05-01 (WF_HOLDOUT_MONTHS=6) and remains genuinely
untouched -- its holdout_id is content-bound to this candidate's own
feature/bars dataset identity (see
mqk_research.ml.holdout_ledger.compute_holdout_id) and is therefore
structurally distinct from DISCOVERY-01's and LIQ-01's own reserved
holdouts even though the calendar window is identical.
"""
...
# in the SHORT-WAVE-03/DISCOVERY-01 driver lineage this was forked from): the
...
# NOTE: byte-for-byte reuse of DISCOVERY-01's / LIQ-01's own frozen
# END_UTC/ASOF -- see module docstring "DATA WINDOW" for the full
# pre-outcome provenance account (DISCOVERY-01 commit 3cf711cf discovered
# this exact universe's fail-closed CorporateActionReviewRequired boundary
# at RKLB's 2025-05-27 name_change event; reusing that already-known
# boundary here is not a reaction to any Wave06 outcome, since none has
# been observed at predeclaration time).
...
# The single production feature_set_v1 column this candidate's rank feature
...
SOURCE_FEATURE_COLUMN = "vol_ratio"
FEATURE_COLUMN = "vol_ratio_rank_20"
...
FAMILIES: dict[str, HypothesisFamily] = {
    "VOL-01": HypothesisFamily(
        key="VOL-01", feature_column=FEATURE_COLUMN,
        strategy_id="pooled_single_feature_xs_volume_surprise_direct_rank_v1",
        hyp_long_only="wave06_vol01_volume_surprise_long_only_v1",
        hyp_long_short="wave06_vol01_volume_surprise_long_short_v1",
        hyp_placebo="wave06_vol01_volume_surprise_placebo_v1",
    ),
}
...
def assert_driver_agrees_with_predeclaration() -> None:
    ...
    for key, fam in FAMILIES.items():
        ...
        assert h["source_production_column"] == SOURCE_FEATURE_COLUMN
...
def add_cross_sectional_rank(feats: pd.DataFrame, *, source_column: str, rank_column: str) -> pd.DataFrame:
    """Locally computes the cross-sectional percentile rank (per end_ts,
    method="average") of an EXISTING, unmodified
    mqk_research.features.feature_set_v1.build_feature_set_v1 output column
    that production code already computes but has never itself ranked. The
    formula is an exact, literal reproduction of that same module's own
    vol_rank_20/atr_rank_14/ret_rank_w ranking step
    (`feats.groupby(end_ts)[col].rank(pct=True, method="average")") -- not a
    new statistical method, and this function does not modify
    research-py/src; it is a pure downstream, read-only transform applied
    experiment-side, exactly analogous to how isolate_feature already
    selects (rather than recomputes) an existing column."""
    if source_column not in feats.columns:
        raise RuntimeError(
            f"feature isolation failed: FeatureSetV1 output missing required source column {source_column!r}"
        )
    out = feats.copy()
    end_ts_dt = pd.to_datetime(out["end_ts"], utc=True)
    out[rank_column] = out.groupby(end_ts_dt)[source_column].rank(pct=True, method="average")
    return out


def ensure_full_features(bars: pd.DataFrame) -> pd.DataFrame:
    """DEFAULT FeatureSetV1Spec already produces vol_ratio (unranked
    current-day-volume / trailing-20-day-average-volume ratio) -- no spec
    customization required. This candidate's own feature, FEATURE_COLUMN
    (the cross-sectional rank of that existing column), is added locally
    via add_cross_sectional_rank; see module docstring. R2: see
    ensure_real_targets -- always recomputed from the already-verified
    in-memory `bars`, never existence-cached."""
    feats = build_feature_set_v1(bars)
    feats = add_cross_sectional_rank(feats, source_column=SOURCE_FEATURE_COLUMN, rank_column=FEATURE_COLUMN)
    RUN_ROOT.mkdir(parents=True, exist_ok=True)
...
def run_family_judge() -> dict:
    ...
    already-attempted LIQ-01 (or vice versa)."""
    return campaign_judge.run_campaign_judge(registry_db=REGISTRY_DB)


EXECUTE_REQUIRED_STAGES = frozenset({"vol01", "judge"})


def main(argv: list[str] | None = None) -> None:
    argv = sys.argv[1:] if argv is None else argv
    if not argv:
        print("usage: run_wave.py {check|vol01|judge} [--execute]", file=sys.stderr)
        raise SystemExit(2)
    stage = argv[0]
