"""W06-A-CAMPAIGN-PREDECLARATION-01 (VOL-01) -- proves the predeclaration is
...
def test_exactly_two_real_candidates() -> None:
    ...
    family (VOL-01) -> exactly 2 real candidates (long_only, long_short)."""
    decl = _decl()
...
def test_feature_is_new_and_distinct_from_every_prior_family() -> None:
    """REQUIRED TEST 12: the single predeclared feature (vol_ratio_rank_20)
    is not any of the seven feature columns already tested by ALPHA-01/
    SHORT-01/SHORT-WAVE-02/03/DISCOVERY-01/WAVE06-LIQ-01 (momentum_score,
    slope_60, ret_rank_20, ret_5, gap_pct_1, vol_rank_20,
    illiquidity_amihud_rank_20) -- guards against silently relabeling a
    retuned variant of a REJECTED_NOT_ADVANCED / INCONCLUSIVE mechanism, or
    of this same campaign's own LIQ-01 candidate, as a new discovery."""
    decl = _decl()
    feature_columns = decl["hypotheses"]["VOL-01"]["feature_columns"]
    assert feature_columns == ["vol_ratio_rank_20"]
    already_tested = {
        "momentum_score", "slope_60", "ret_rank_20", "ret_5", "gap_pct_1", "vol_rank_20",
        "illiquidity_amihud_rank_20",
    }
...
def test_source_production_column_is_existing_unmodified_feature() -> None:
    ...
    decl = _decl()
    source_column = decl["hypotheses"]["VOL-01"]["source_production_column"]
    assert source_column == "vol_ratio"
    assert run_wave.SOURCE_FEATURE_COLUMN == source_column
    assert run_wave.FEATURE_COLUMN == "vol_ratio_rank_20"
    ...
    assert "vol_ratio_rank_20" not in feats.columns  # NOT ranked by production code

    ranked = run_wave.add_cross_sectional_rank(feats, source_column=source_column, rank_column="vol_ratio_rank_20")
    valid = ranked["vol_ratio_rank_20"].dropna()
    assert len(valid) > 0
...
def test_placebo_seed_is_frozen() -> None:
    """REQUIRED TEST 16."""
    assert _decl()["placebo_seed"] == 60602
    assert run_wave.PLACEBO_SEED == 60602
