...
> **Scope note** (updated by `MT-01C`, 2026-09-07): "Source of Truth" above refers to GUI-specific patch detail (exact files/tests per GUI patch) within this narrow domain only. For overall repository program/backlog/status — including which GUI-touching patches are currently open — `MiniQuantDeskV4_Master_Program_Plan_and_Ledger.md` is the canonical current authority; keep this GUI tracker consistent with that master when GUI patch status changes.
