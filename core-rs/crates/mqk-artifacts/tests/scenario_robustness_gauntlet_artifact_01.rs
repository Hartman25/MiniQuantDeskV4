...
fn rga01d_content_tampered_after_write_rejected() {
    ...
    let output = run_robustness_gauntlet(&report, &config, &bars, || {
        Box::new(HoldFlat { name: "RgaTamper" })
    });
    mqk_artifacts::write_canonical_robustness_gauntlet(&run_dir, &output).unwrap();

    tamper(&run_dir, |v| {
        v["scenarios"][0]["passed"] =
            serde_json::Value::Bool(!v["scenarios"][0]["passed"].as_bool().unwrap());
    });
...
fn rga01e_cross_candidate_swap_rejected() {
    ...
    assert!(matches!(
        err,
        RobustnessGauntletArtifactError::RunIdMismatch { .. }
    ));
...
fn rga01i_loaded_artifact_complete_after_dsr_pbo_merged() {
    ...
    assert!(
        loaded.is_complete(),
        "must be complete once every deferred scenario is merged in"
    );
...
fn fake_sensitivity(passed: bool) -> mqk_backtest::RobustnessScenarioOutcome {
    mqk_backtest::RobustnessScenarioOutcome {
        ...
        reason: if passed {
            ...
            Some("dsr_range exceeded ceiling".to_string())
        },
...
fn fake_p7a_p7b_stress(passed: bool) -> mqk_backtest::RobustnessScenarioOutcome {
    mqk_backtest::RobustnessScenarioOutcome {
        ...
        reason: if passed {
            ...
            Some("stressed max_drawdown exceeded ceiling".to_string())
        },
        detail: "test-fabricated finalize-seam outcome".to_string(),
        research_trial_id: Some("fake_sensitivity_trial".to_string()),
        evidence: None,
...
fn fake_genuine_shuffled_placebo(passed: bool) -> mqk_backtest::RobustnessScenarioOutcome {
    mqk_backtest::RobustnessScenarioOutcome {
        ...
        reason: if passed {
            ...
            Some("placebo performed as well or better".to_string())
        },
        detail: "test-fabricated finalize-seam outcome".to_string(),
        research_trial_id: Some("fake_sensitivity_trial".to_string()),
        evidence: None,
...
fn rga01j_finalize_merges_sensitivity_into_incomplete_artifact() {
    ...
    let output = run_robustness_gauntlet(&report, &config, &bars, || {
        Box::new(HoldFlat {
            name: "RgaFinalizeBasic",
        })
    });
    mqk_artifacts::write_canonical_robustness_gauntlet(&run_dir, &output).unwrap();
    assert!(!load_canonical_robustness_gauntlet(&run_dir)
        .unwrap()
        .is_complete());
    ...
    assert!(
        loaded.is_complete(),
        "must be complete after merging every deferred scenario"
    );
    assert!(
        !loaded
            .failed_scenario_descriptions()
            .iter()
            .any(|d| d.starts_with("dsr_pbo_sensitivity")),
...
fn rga01k_finalize_is_idempotent_on_identical_replay() {
    ...
    assert_eq!(
        loaded.scenarios_run(),
        7,
        "replay must not duplicate the scenario"
    );
...
fn rga01l_finalize_rejects_conflicting_sensitivity() {
    let (report, run_dir, config, bars) =
        run_and_persist("finalize_conflict", "RgaFinalizeConflict");
    let output = run_robustness_gauntlet(&report, &config, &bars, || {
        Box::new(HoldFlat {
            name: "RgaFinalizeConflict",
        })
    });
    mqk_artifacts::write_canonical_robustness_gauntlet(&run_dir, &output).unwrap();

    finalize_canonical_robustness_gauntlet_with_sensitivity(&run_dir, &fake_sensitivity(true))
        .unwrap();

    let err =
        finalize_canonical_robustness_gauntlet_with_sensitivity(&run_dir, &fake_sensitivity(false))
            .unwrap_err();
    assert_eq!(
        err,
        RobustnessGauntletFinalizeError::ConflictingSensitivityResult
    );
...
fn rga01m_finalize_rejects_wrong_scenario_name() {
    let (report, run_dir, config, bars) =
        run_and_persist("finalize_wrong_name", "RgaFinalizeWrongName");
    let output = run_robustness_gauntlet(&report, &config, &bars, || {
        Box::new(HoldFlat {
            name: "RgaFinalizeWrongName",
        })
    });
    mqk_artifacts::write_canonical_robustness_gauntlet(&run_dir, &output).unwrap();
    ...
    let err =
        finalize_canonical_robustness_gauntlet_with_sensitivity(&run_dir, &wrong).unwrap_err();
    assert_eq!(
        err,
...
fn rga01n_finalize_on_missing_artifact_fails() {
    let (_report, run_dir, _config, _bars) =
        run_and_persist("finalize_missing", "RgaFinalizeMissing");
...
fn rga01o_interrupted_finalization_leaves_no_falsely_complete_artifact() {
    let (report, run_dir, config, bars) =
        run_and_persist("finalize_interrupt", "RgaFinalizeInterrupt");
    let output = run_robustness_gauntlet(&report, &config, &bars, || {
        Box::new(HoldFlat {
            name: "RgaFinalizeInterrupt",
        })
    });
    mqk_artifacts::write_canonical_robustness_gauntlet(&run_dir, &output).unwrap();
