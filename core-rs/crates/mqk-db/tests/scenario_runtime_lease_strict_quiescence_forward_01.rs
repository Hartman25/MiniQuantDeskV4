//! RUNTIME-LEASE-STRICT-QUIESCENCE-FORWARD-01: migration-level proofs
//! for 0070's quiescence-gated reconciliation of a legacy (`run_id IS NULL`)
...
//! migration 0070, which is exactly the kind of global/singleton-table setup
...
// ---------------------------------------------------------------------------
// 6. 0069 database, legacy lease + a RUNNING run whose heartbeat is stale
//    (past the deadman window): quiescence must be judged by run status
//    alone, never by heartbeat freshness, so 0070 must ALSO refuse here.
...
async fn mig_06_running_run_with_stale_heartbeat_still_blocks_migration() {
    ...
    migrate_to(&pool, 69)
        .await
        .expect("database must reach exactly 0069");
...
// ---------------------------------------------------------------------------
// 8. 0069 database, legacy lease + a RUNNING run with NULL heartbeat (the
//    realistic shape immediately after begin_run, before the first
//    heartbeat_run call ever lands): 0070 must refuse -- NULL heartbeat is
...
// ---------------------------------------------------------------------------
// 9. 0069 database, legacy lease + a HALTED run with a stale heartbeat: 0070
...
// ---------------------------------------------------------------------------
// 10. 0069 database, legacy lease + a CREATED-only run (never armed): 0070
...
async fn mig_12_begin_run_blocks_while_migration_lock_is_held() {
    ...
    migrate_fresh(&pool)
        .await
        .expect("fresh chain through 0070 must apply cleanly");
    ...
    sqlx::query("LOCK TABLE runs IN SHARE MODE")
        ...
        .await
        .expect("acquire SHARE lock, simulating migration 0070's in-flight window");
