//! Patch 2 — restart quarantine for DISPATCHING rows.
//!
//! # PROOF LANE
//!
...
const RUN_ID_STR: &str = "29200001-0000-0000-0000-000000000000";
...
#[tokio::test]
async fn restart_quarantines_dispatching_row_and_refuses_dispatch() -> Result<()> {
    let url = require_db_url();
    let pool = require_pool(&url).await;
    mqk_db::migrate(&pool).await?;

    let run_id: Uuid = RUN_ID_STR.parse().unwrap();
    let idem = "patch2-dispatching-ord-001";
    ...
    let err = orch
        .tick()
        .await
        .expect_err("tick must quarantine DISPATCHING row");
    ...
    assert!(
        msg.contains("DISPATCHING"),
        "error must mention DISPATCHING evidence, got: {msg}"
    );
