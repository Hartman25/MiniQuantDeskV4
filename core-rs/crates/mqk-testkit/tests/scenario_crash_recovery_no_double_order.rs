...
async fn crash_recovery_does_not_double_submit_when_broker_already_has_order() -> anyhow::Result<()>
{
    ...
    assert!(
        dispatching,
        "must transition CLAIMED -> DISPATCHING before submit"
    );

    // Simulate the "submit to broker" step happening…
    // …and then a crash BEFORE we ever mark ACKED (SENT+map already durable).
    ...
    // Record durable dispatch evidence (SENT + broker map), but do NOT ack.
    let sent = mqk_db::outbox_mark_sent_with_broker_map(
        &pool,
        &claimed_row.row.idempotency_key,
        &format!("broker-{}", claimed_row.row.idempotency_key),
        chrono::Utc::now(),
    )
    .await?;
    assert!(
    ...
    assert!(
        ambiguous
            .iter()
            .all(|row| row.idempotency_key != claimed_row.row.idempotency_key),
        "SENT row with broker-map evidence must not be quarantined"
    );
