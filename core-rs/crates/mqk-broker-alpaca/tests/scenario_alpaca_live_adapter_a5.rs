...
use mqk_broker_alpaca::{
    activity_to_trade_update, build_replace_body, build_submit_body, micros_to_price_str,
    normalize::normalize_trade_update,
    types::{AlpacaOrderActivity, AlpacaOrderFull, AlpacaSubmitResponse},
...
// ---------------------------------------------------------------------------
// L14 — fetch_events fails on unreachable URL (not silent empty-vec stub)
// ---------------------------------------------------------------------------

#[test]
fn l14_adapter_fetch_events_fails_on_unreachable_url_not_silent_stub() {
    let adapter = unreachable_adapter();
    let token = BrokerInvokeToken::for_test();

    let result = adapter.fetch_events(None, &token);

    // Must not return Ok(empty vec) — that was the stub behaviour.
    // A real implementation makes a network call; an unreachable host must error.
    assert!(
        result.is_err(),
        "fetch_events to unreachable URL must return Err, not Ok(empty vec)"
    );
    match result.unwrap_err() {
        BrokerError::Transport { .. } | BrokerError::Transient { .. } => {}
        other => {
            panic!("fetch_events to unreachable URL must be Transport or Transient, got {other:?}")
        }
    }
}
