...
//! updated for CRYPTO-DATA-01AB-AC-KRAKEN-CONTENT-DIFF-SYNC-BUNDLE-01-
//! COMBINED.
//!
...
//!   writing anything,
//! - insert a genuinely missing/new completed row while updating a
//!   previously-stored row whose content has changed,
//! - never write the forming (not-yet-committed) candle,
//! - preserve Kraken's scaled (x1e8) fractional base-asset volume across
//!   readback exactly,
//! - re-run idempotently with **true** content-diff skip (no insert, no
//!   update, no write-helper call at all once content matches),
//! - leave zero `oms_outbox` side effects,
//! - clean up to zero leftover rows.
//!
//! 01Z-AA honestly documented that the underlying
//! `ingest_provider_bars_to_md_bars_with_provider_metadata` write helper
//! (reused unchanged here) always performs an unconditional
//! `ON CONFLICT DO UPDATE`, so it could not distinguish "existing and
//! unchanged" from "existing and changed" -- the default policy upserted
//! every completed bar unconditionally. 01AB-AC closes that gap: the CLI
//! now reads existing rows for the exact candidate `end_ts` keys
//! (`mqk_db::md::fetch_md_bars_for_provider_sync_keys`) and compares content
//! (`mqk_db::md::provider_bar_matches_existing`) before deciding to write.
//! This file proves the updated semantics; the fuller
//! missing/changed/unchanged/no-update-existing matrix lives in
//! `scenario_cli_kraken_ohlc_content_diff_sync_db_01abac.rs`.
//!
...
async fn ksz02_kraken_fixture_sync_inserts_missing_updates_existing_and_cleans_up(
    ...
    assert!(
        stdout.contains("rows_skipped_if_known=0"),
        "changed+missing rows are both written under default policy: {stdout}"
    );
    assert!(
    ...
    assert!(
        ...
        "the pre-existing (earlier) row must be updated because its content changed: {stdout}"
    );
    ...
    assert_eq!(
        earlier.6.as_deref(),
        Some("provider_sync"),
        "sync-written rows carry provider_sync"
    );
    ...
    // rows as unchanged (content-diff, not just presence) and must not call
    // the write helper at all. ---
