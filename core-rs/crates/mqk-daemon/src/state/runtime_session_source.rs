...
//! as a candidate production session source, while leaving the real,
//! currently-wired legacy session source (`NyseWeekdaysProvider` via
//! `session_controller.rs` / `start_execution_runtime`) completely untouched.
//!
//! # Honesty contract
//!
//! - This is a scaffold/proof patch, not a production cutover. No caller in
//!   `session_controller.rs`, `start_execution_runtime`, or any trading/risk/
//!   OMS/broker path reads [`RuntimeSessionSourceMode`] or
//!   [`RuntimeSessionSourceEvaluation`] for any decision.
//! - Default (no env set, or `MQK_RUNTIME_SESSION_SOURCE` unset/empty):
//!   [`RuntimeSessionSourceMode::Legacy`]. `runtime_uses_session_v2` and
//!   `trading_uses_session_v2` are always `false` regardless of mode — this
//!   patch proves candidate parity, it does not flip either flag to `true`.
//! - An invalid/unrecognized `MQK_RUNTIME_SESSION_SOURCE` value fails closed
//!   for the *candidate* evaluation (mode falls back to `Legacy`, candidate
//!   side reports `activation_refusal_reason`) but never aborts daemon
//!   startup — mirrors `parse_deployment_mode`/`BrokerKind::parse` in
//!   `state/env.rs` and `state/broker.rs`, which return `None`/fallback
//!   rather than panicking on unrecognized env values.
...
//!   reports itself as the active session source.
...
/// Env var selecting the candidate runtime session-source mode.
///
...
/// Any other value is invalid and falls back to `Legacy` (fail-closed for the
/// candidate side; never crashes daemon startup).
pub const RUNTIME_SESSION_SOURCE_ENV: &str = "MQK_RUNTIME_SESSION_SOURCE";

/// Candidate runtime session-source mode — ASSET-CORE-05D.
///
/// `Legacy` is the default and is the only mode that matches current
...
/// trading, runtime, or routing decision.
...
pub enum RuntimeSessionSourceMode {
    ...
    /// only — never wired into any trading/runtime gate by this patch.
    V2EquityShadow,
}
        ...
        match self {
            Self::Invalid { raw } => Some(format!(
                "{RUNTIME_SESSION_SOURCE_ENV}={raw:?} is not a recognized runtime session \
                 source mode (expected \"legacy\" or \"v2_equity_shadow\"); falling back to \
                 legacy — the candidate v2 source was NOT activated"
            )),
            _ => None,
        }
    }
...
pub struct RuntimeSessionSourceSummary {
    ...
    /// Always `false` in this patch. No default production cutover exists.
    pub production_cutover_enabled: bool,
    /// Always `false` in this patch. The runtime session gate
    /// (`session_controller.rs`) does not read this seam.
    pub runtime_uses_session_v2: bool,
    /// Always `false` in this patch. No trading/risk/OMS path reads this seam.
    ...
    /// Candidate v2 session state computed only when `session_source_mode ==
    /// "v2_equity_shadow"`. `None` for `"legacy"` mode — the candidate is not
    /// evaluated at all when the operator has not selected it.
    ...
    /// Operator-facing reason the candidate fell back to legacy or could not
    /// activate. Always `None` for `"legacy"` mode (nothing to explain — it
    /// is the default). `Some(..)` for `"v2_equity_shadow"` mode whenever the
    /// candidate evaluation failed or the registry could not be loaded.
...
/// `"v2_equity_shadow"` mode degrades to a `fallback_reason`-carrying summary
/// rather than propagating an error to the parent `/system/status` or
/// `/system/preflight` response.
pub fn runtime_session_source_summary(
    ...
    match evaluate_runtime_session_source_from_registry_path(registry_path, now_utc) {
        Ok(eval) => RuntimeSessionSourceSummary {
            session_source_mode: RuntimeSessionSourceMode::V2EquityShadow
                .as_str()
                .to_string(),
            production_cutover_enabled: false,
            runtime_uses_session_v2: false,
...
mod tests {
    ...
    /// Serializes env-var mutation across this module's tests.
    ///
    /// `std::env::set_var`/`remove_var` are process-global; Rust runs
    /// `#[test]` functions concurrently by default. Every test below that
    /// touches `RUNTIME_SESSION_SOURCE_ENV` takes this lock for its whole
    /// body so the env var cannot be observed half-mutated by a concurrently
    /// running test in this same module.
