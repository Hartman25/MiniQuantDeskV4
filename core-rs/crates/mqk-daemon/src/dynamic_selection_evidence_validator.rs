...
fn candidate_record_to_result(
    ...
    Ok(SelectionCandidateResult {
        ...
        // DYNAMIC-SELECTION-CONFIG-ELIGIBILITY-CLOSURE-01: a `NULL` durable
        // value (a row persisted before migration 0067) reads back as
        // fail-closed `false` -- never optimistically treated as verified
        // just because the evidence predates this check's existence.
        config_identity_verified: rec.config_identity_verified.unwrap_or(false),
        durable_config_fingerprint: rec.durable_config_fingerprint.clone(),
        current_config_fingerprint: rec.current_config_fingerprint.clone(),
...
mod tests {
    ...
    fn reconstruction_recomputes_the_same_plan_id_as_the_original_build() {
        ...
        let candidate = SelectionCandidateResult {
            ...
            config_identity_verified: evidence.config_identity_verified,
            durable_config_fingerprint: evidence.durable_config_fingerprint.clone(),
            current_config_fingerprint: evidence.current_config_fingerprint.clone(),
    ...
    fn sample_candidate_record(
        ...
        DynamicSelectionPlanCandidateRecord {
            ...
            exact_fingerprint_v2_matches: true,
            config_identity_verified: Some(true),
