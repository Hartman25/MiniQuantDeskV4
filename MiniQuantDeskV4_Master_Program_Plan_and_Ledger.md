# MiniQuantDeskV4 — Master Program Plan, Completion Contract & Ledger

# MiniQuantDesk V4 — Project Definition of Done and Milestone Contract

**Status of this section:** AUTHORITATIVE PROJECT COMPLETION CONTRACT  
**Adopted:** 2026-09-07  
**Purpose:** Define a finite, non-moving finish line for MiniQuantDeskV4 before further audits, repairs, expansion, or operational validation.  
**Relationship to the ledger below:** This section defines what the project must become and when each milestone is finished. The existing Master Patch Ledger is retained below as the authoritative implementation/history record. A patch, audit finding, feature idea, or cleanup item may block the project only if it maps to a requirement or exit gate in this milestone contract.

## A. Final Definition of Fully Finished

MiniQuantDeskV4 is fully finished when it is a deterministic, fail-closed, auditable, multi-strategy and multi-asset trading platform that can:

1. run the complete Research → Backtest → Promotion → Paper → Live lifecycle without fabricated authority or evidence;
2. process multiple strategies, symbols, assets, market-data streams, and execution workflows concurrently without cross-contamination or unsafe global bottlenecks;
3. run Paper and Live at the same time as independent execution domains;
4. use Paper continuously to test, forward-validate, reject, and mature new strategies/alphas while separately-approved strategies operate in Live;
5. operate Paper and Live mostly/fully autonomously once required human authority has been granted;
6. maintain separate, truthful authority for orders, fills, positions, cash, P&L, risk, reconciliation, runtime ownership, deployment state, and broker/account identity in every execution domain;
7. support the frozen V4 asset matrix: US equities/ETFs, crypto, futures, FX, and options;
8. survive the defined restart, retry, disconnect, stale-data, broker, DB, reconciliation, and safety-halt failure cases without inventing state or silently weakening safety;
9. complete the finite integrated validation in Milestone 10; and
10. be released and frozen as the MiniQuantDeskV4 production baseline.

**Alpha profitability is not a software-completion gate.** MiniQuantDeskV4 must be capable of testing, rejecting, promoting, Paper-validating, and Live-operating strategies truthfully. The project is not held open because a particular hypothesis or campaign fails to discover profitable alpha.

## B. Multi-Threaded Means Concurrent Multi-Domain Execution

For this project, multi-threaded is an observable capability requirement, not a requirement to use one specific operating-system threading primitive.

The finished system must be able to concurrently process multiple strategies, symbols, asset classes, market-data streams, broker/execution workflows, Paper and Live domains, and independent risk/reconciliation/accounting flows without serial global bottlenecks that unnecessarily prevent safe concurrency and without races that create duplicate, missing, cross-strategy, cross-symbol, cross-asset, or cross-domain economic actions.

The implementation may use Rust threads, Tokio tasks, bounded worker pools, asynchronous I/O, actor-like ownership, or another architecture when that is safer. Observable deterministic behavior and isolation are authoritative; thread count is not.

## C. Permanent Paper + Live Architectural Contract

Paper is not a global mode that replaces Live. Paper and Live are independent execution domains that may operate concurrently.
...
```text
               ...
               +---------------+---------------+
               |                               |
          PAPER DOMAIN                    LIVE DOMAIN
               |                               |
   candidates / forward tests        separately-approved alphas
   experimental deployments          production deployments
               |                               |
       Paper broker/account             Live broker/account
```

Shared infrastructure is permitted only where authority remains unambiguous. Immutable/canonical market data, instrument metadata, calendars, research artifacts, strategy definitions, and observability may be shared when safe.

The following must remain execution-domain specific: broker account, deployment identity, run identity, arm/disarm state, runtime ownership, orders, fills, positions, cash, P&L, risk budgets/limits, reconciliation, broker cursors, capital allocation, strategy deployment authority, and evidence proving Paper/Live actions.

A Paper deployment must never submit to Live because of shared process state, configuration leakage, environment-variable ambiguity, retry/restart behavior, or identity collision. The inverse must also hold.

## D. Permanent Human-Authority Boundary

MiniQuantDeskV4 may become highly autonomous, but autonomy does not remove every human authorization boundary.

The system may eventually automate research campaigns, trial/evaluation scheduling, rejection of failed candidates, promotion toward Paper, Paper startup/trading/risk/reconciliation/recovery/shutdown, Live startup/trading/risk/reconciliation/recovery/shutdown after explicit Live deployment authorization, provider/data maintenance, bounded retry/recovery, evidence generation, alerting, session rollover, and normal operational maintenance.

Human authority remains required for initial approval of a strategy/deployment for Live capital, material increases to Live capital/risk outside an already-authorized policy, overriding/clearing a safety halt when safe recovery cannot be proved, accepting unresolved broker/reconciliation discrepancies, credential/account changes, production software deployment, and genuinely ambiguous or contradictory state.
...
# Milestone 1 — US Equity/ETF Paper Production

**Purpose:** Establish the first complete production path and trustworthy baseline before broadening architecture.

### Required capabilities

- Research → Backtest → OOS/robustness → Promotion is causally correct, cost-aware, execution-aware, and evidence-bound.
- Final holdout stays reserved unless explicitly authorized for consumption.
- Promoted US equity/ETF strategies deploy through the production Alpaca Paper path.
- Market-data provider/provenance/readiness gates are truthful.
- Risk, OMS, outbox/inbox, portfolio/accounting, reconciliation, runtime ownership, halt/recovery, scheduler, and autonomous-operation paths are production-capable.
- A genuine Paper trade lifecycle is observed end to end.
- A genuine no-trade lifecycle is observed end to end.
- Actual deployed Paper DB/config/provider/universe/scheduler/risk-state truth is verified.
- Autonomous Paper operation completes finite validation.

### Operational validation target

Milestone 1 requires at least **10 countable autonomous Paper market sessions** and at least **5 consecutive clean sessions after the final correctness repair**, if any repair occurs during validation.

A session counts only if the production path actually ran, required data was valid, strategy evaluation occurred, the trade/no-trade disposition is truthful, broker/MQD state agrees, and required evidence exists. A safety refusal before strategy evaluation is truthful behavior but is not a successful countable validation session.

### Finish line

Milestone 1 is COMPLETE when the aggressive M1 audit has no unresolved M1 V4 blockers; the actual Paper production state is ready; at least one genuine Paper order/fill/reconcile lifecycle and one genuine no-trade lifecycle are proven; the 10-session / 5-consecutive-clean gate is met; and no unresolved deterministic safety/correctness defect remains on the equity/ETF Paper path.

Once accepted, Milestone 1 is FROZEN and may be reopened only by a new deterministic contradiction or work explicitly required by a later milestone.

**Current project status: THIS IS THE CURRENT TARGET.**

# Milestone 2 — Concurrent Multi-Strategy / Multi-Symbol Engine

**Purpose:** Move from a narrow strategy/symbol operating model to deterministic concurrent portfolio operation.

### Required capabilities

- multiple strategy instances and symbols operate simultaneously;
- per-strategy/per-symbol state cannot contaminate another;
- same-symbol competing strategy intentions resolve through explicit portfolio/execution authority;
- strategies propose targets/opportunities rather than owning the account;
- portfolio-level capital allocation and risk aggregation span strategies;
- retries/restarts cannot create duplicate dispatches/orders;
- concurrent completed bars cannot be lost or double-consumed;
- conflict resolution/order is deterministic where economics depend on it;
- failures/panics are isolated according to explicit policy.

### Finish line

Milestone 2 is COMPLETE when deterministic concurrency tests and production-path proofs demonstrate multiple strategies × multiple symbols × concurrent events × conflicting signals × retry/restart without duplicate actions, missing required actions, nondeterministic portfolio results, or state contamination.

Then Milestone 2 is frozen.

# Milestone 3 — Concurrent Paper + Live Execution Domains

**Purpose:** Make Paper and Live independent, simultaneously-operable deployment domains.

Every deployed strategy must have durable/explicit identity including strategy artifact/version/semantic identity, execution domain, broker/account identity, instrument universe, risk policy, capital allocation, promotion lineage, and activation/authorization state.

Required negative controls prove Paper cannot submit to Live; Live cannot submit to Paper; duplicate/replayed deployment identity cannot cross domains; Paper failure/recovery cannot mutate Live economic truth; Live failure/recovery cannot rewrite Paper truth; and shared market-data/research artifacts cannot grant execution authority.

### Finish line

Milestone 3 is COMPLETE when Paper and Live run concurrently in a controlled harness/session with independent accounts, deployments, state, risk, reconciliation, and evidence and with no cross-domain contamination.

Then Milestone 3 is frozen.

# Milestone 4 — US Equity/ETF Live Production

**Purpose:** Prove controlled real-capital operation for the first asset family while Paper remains simultaneously usable for candidate validation.

Required path:

```text
Paper-proven candidate
        ->
LiveShadow
        ->
shadow capture
...
parity evidence
        ->
authoritative Live trust chain
        ->
explicit operator Live authorization
        ->
tiny-capital Live
        ->
controlled Live validation
        ->
normal authorized Live deployment
```

Required proof includes correct Live account/broker identity, genuine tiny-capital order/fill lifecycle, reconciliation, cancel, restart/recovery, disconnect/reconnect, safety halt, authorized flatten policy, no Paper/Live contamination, and continued Paper testing of separate candidates while Live operates approved strategies.

### Finish line

Milestone 4 is COMPLETE when the US equity/ETF Live path is independently accepted through the finite tiny-capital/controlled validation plan while concurrent Paper operation remains correct.

Then equity/ETF Paper+Live production is frozen.

# Milestone 5 — Asset-Neutral Production Contracts

**Purpose:** Generalize only seams that real non-equity assets require, without rewriting proven equity behavior for hypothetical future needs.

Generic concepts should remain asset-neutral where practical: research identity, signal/target/proposal, promotion, deployment, risk decision, order intent, broker event, evidence, and reconciliation contract.

Asset-specific adapters/specs may own instrument identity, tick/lot size, contract multiplier, calendar/session semantics, fees, margin/leverage, settlement, expiry, roll, corporate actions, exercise/assignment, financing/funding, and 24/7 market semantics.

### Finish line

Milestone 5 is COMPLETE when an explicit production-grade asset contract exists, current equity/ETF economic behavior remains unchanged, and the contract is proven sufficient by concrete later-asset requirements rather than speculative abstraction.

Then the generic contract is frozen except for deterministic contradictions exposed by later assets.

# Milestone 6 — Crypto Paper + Live

**Purpose:** Add the first non-equity production asset family and prove 24/7/fractional-market correctness.

Required capabilities include crypto instrument identity, asset-correct quantity/tick/fee semantics, 24/7 behavior, market-data provenance/readiness, Research → Backtest → Promotion, Paper/simulation validation, controlled Live execution, risk/accounting/reconciliation, autonomy, and simultaneous operation with equities without inheriting equity session assumptions.

### Finish line

Milestone 6 is COMPLETE when at least one canonical supported crypto market traverses the full Research → Paper → controlled Live lifecycle with asset-correct economics and operates concurrently with equity/ETF workloads without cross-asset semantic contamination.

Then crypto V4 scope is frozen.

# Milestone 7 — Futures + FX Paper + Live

**Purpose:** Add contract/expiry/margin-driven futures and pair/leverage-driven FX without collapsing their distinct semantics.

Futures requirements include executable contract identity separate from research convenience/continuous series, expiration, rollover, multiplier, margin, session breaks/nearly-24-hour calendar, and contract-aware data/accounting. A continuous research series must never silently become executable contract identity.

FX requirements include pair/base/quote identity, pip/tick semantics, leverage/margin, nearly continuous weekly sessions, rollover/financing, and FX-correct sizing/accounting.

### Finish line

Milestone 7 is COMPLETE when at least one supported futures market and one supported FX market each traverse Research → Backtest → Promotion → Paper → controlled Live with truthful executable identity, economics, risk, accounting, and reconciliation.

Then futures/FX V4 scope is frozen.

# Milestone 8 — Options Paper + Live

**Purpose:** Add options last because their instrument identity, lifecycle, historical-data requirements, and risk are materially more complex.

Required capabilities include underlying, expiration, strike, call/put, multiplier, executable contract selection, option-specific liquidity/spread modeling, exercise, assignment, expiration, option-correct accounting/risk, causal historical contract identity, Paper, and controlled Live.

The exact V4 strategy permission set must be frozen before implementation begins. V4 is not required to support unlimited-risk/naked strategies merely to claim options support.

### Finish line

Milestone 8 is COMPLETE when the frozen V4 options strategy set traverses historical research → causal backtest → promotion → Paper → controlled Live and the exercise/assignment/expiration lifecycle is truthful and tested.

Then options V4 scope is frozen.

# Milestone 9 — Mostly/Fully Autonomous Operations
...
Live autonomy, for already-authorized deployments, includes broker/account/data verification, startup, arm, trading, risk management, cancel, reconciliation, recovery, halt/flatten where policy authorizes it, safe restart, normal session/contract maintenance, notification, and escalation.

### Finish line

Milestone 9 is COMPLETE when routine operation no longer depends on an operator manually driving the normal daily lifecycle while ambiguity and safety boundaries still fail closed and escalate rather than guess.

Then the autonomy contract is frozen.

# Milestone 10 — Final Integrated Validation and V4 Release Freeze

**Purpose:** Prove the complete V4 platform as one integrated system and terminate the V4 build phase.

The integrated matrix covers concurrent Paper and Live, multiple strategies, multiple symbols, equities/ETFs, crypto, futures, FX, options, concurrent market-data/execution workflows, and asset/domain-specific accounting, risk, reconciliation, and evidence.

Required failure-path coverage includes simultaneous events, provider disconnect/staleness, broker disconnect/reconnect, daemon/process restart, crash, DB reconnect/recovery, rejected order, partial fill, cancellation, reconciliation drift, safety halt, domain isolation, asset session transitions, futures roll, option expiration/exercise/assignment, and continued Paper operation while authorized Live deployments operate where policy permits.

### Final operational validation target

The exact session matrix may be refined before M10 begins to reflect supported brokers/markets, but it may not become open-ended.

Default final gate:

- at least **20 countable integrated production sessions**;
- at least **10 sessions with Paper and Live operating concurrently**;
- at least **5 consecutive clean integrated sessions after the final production correctness repair**;
- at least one genuine controlled-Live fill lifecycle for every asset family V4 claims as Live-supported;
- zero unexplained broker/MQD state divergence;
- zero unresolved deterministic safety/correctness defect in frozen V4 production scope.

### Finish line

Milestone 10 is COMPLETE when the integrated validation gate passes, all milestone evidence is recorded, no required V4 blocker remains, deferred/post-V4 work is separated from V4 requirements, main/origin and release artifacts are verified, and the release is tagged/frozen as the final MiniQuantDeskV4 baseline.

At that point:

```text
MINIQUANTDESKV4 = FULLY FINISHED

Research -> Backtest -> Promotion -> Paper -> Live = production-capable
Concurrent multi-strategy / multi-symbol / multi-asset = production-capable
Paper + Live simultaneous operation = production-capable
Routine Paper + authorized Live operation = autonomous
V4 asset matrix = complete
Unresolved V4 engineering blockers = 0
```

After Milestone 10, new capability work belongs to a post-V4 release/version unless it corrects a deterministic defect in a claim made by frozen V4.

## F. Scope-Control Rules That Prevent Endless Development

### F1. Every proposed task must answer

**Which unfinished milestone requirement or exit gate does this task block?**

If the answer is none, the work is POST-V4, maintenance, or optional product work and cannot hold V4 completion open.

### F2. Closed audit classification

Every audit finding must be exactly one of:
...
Only `V4_BLOCKER` and `V4_REQUIRED` automatically create pre-release implementation work. `UNKNOWN_NEEDS_PROOF` is temporary and must resolve before the affected milestone audit closes.

### F3. Frozen milestones stay frozen

An accepted milestone is reopened only by a new deterministic contradiction, a later milestone exposing a real incompatibility that prevents its exit gate, or an explicit operator change to this completion contract.

### F4. Alpha results do not redefine infrastructure

A losing/rejected strategy is not an infrastructure defect. Infrastructure changes require a deterministic platform defect, evidence/identity violation, execution-model defect, or missing capability already required by this contract.

### F5. Do not build future assets early

Until a milestone requiring a new asset begins, preserve proven current behavior and add later asset behavior through explicit adapters/specifications. Do not rewrite working equity code for hypothetical future assets.

### F6. Finite validation beats indefinite soak

Every operational milestone has an explicit numeric/finite boundary. Passing it closes the milestone. Do not silently raise validation counts without a concrete defect/evidence reason and explicit contract update.

### F7. Audit once per bounded subsystem, then freeze
...
Do not intentionally use symptom-by-symptom implementation prompts when the whole bounded authority/data flow can be inspected first.

### F8. Implementation-agent efficiency rule

Claude/Codex/Fable are implementation agents, not acceptance authorities.

Before spending implementation-agent usage:

1. ChatGPT performs the bounded systems-level review.
2. The complete known defect set for that invariant/subsystem is assembled.
3. Related fixes are grouped into a controller/wave.
4. Each invariant still receives its own commit boundary.
5. Focused disproving tests/mutation controls are specified up front.

If the same bounded invariant fails implementation review twice, do not automatically spend a third agent cycle. Prefer direct/manual repair or reassessment.
...
| **M2 — Concurrent Multi-Strategy / Multi-Symbol Engine** | NOT STARTED AS A FORMAL MILESTONE | Existing multi-symbol/concurrency work is reusable evidence, but M2 has not been audited against this frozen exit gate. |
| **M3 — Concurrent Paper + Live Domains** | NOT STARTED | Some isolation scaffolding exists, but simultaneous independent domains are not yet accepted. |
| **M4 — US Equity/ETF Live Production** | NOT READY / GATED | LiveShadow/trust-chain work exists; real operational/live-capital validation remains separate and gated. |
| **M5 — Asset-Neutral Production Contracts** | NOT STARTED | Must be driven by concrete later-asset requirements, not speculative abstraction. |
| **M6 — Crypto Paper + Live** | NOT STARTED | Existing crypto/Kraken data work is partial input only; execution milestone not accepted. |
| **M7 — Futures + FX Paper + Live** | NOT STARTED | Foundation/scaffolding only. |
...
2. Aggressive deep audit: deeply audit Milestone 1 before implementation work, classify every finding under §F2, and group all `V4_BLOCKER`/`V4_REQUIRED` findings into efficient controller waves.
3. Repair/acceptance: implement only required M1 repairs, independently review them, and freeze repaired subsystems.
4. Production-state verification: confirm real Paper DB/config/provider/universe/scheduler/risk environment.
5. Real Paper lifecycle proof: genuine trade and no-trade paths.
6. Finite autonomous Paper validation: 10 countable sessions / 5 consecutive clean after final repair.
7. Accept and freeze Milestone 1.
8. Begin Milestone 2 only after M1 acceptance unless an M1 audit finding proves a narrow M2 prerequisite is already necessary for M1 correctness.

## I. Tracked Capability Backlog and Milestone Mapping

**Purpose:** Preserve worthwhile product, research, observability, and development ideas without allowing them to silently redefine the frozen M1–M10 completion contract.

This section is the authoritative place for ideas that are worth keeping but are not yet part of an active milestone implementation wave.

### I1. Backlog governance

Every tracked idea has one of these dispositions:

- `MILESTONE_REQUIRED` — explicitly part of a frozen milestone exit gate.
- `MILESTONE_CANDIDATE` — potentially useful inside a milestone, but does not block that milestone unless explicitly promoted or a deterministic audit proves it is required.
- `RESEARCH_HYPOTHESIS` — strategy/research idea; failure or rejection does not imply an infrastructure defect.
- `OPTIONAL_V4` — useful capability that may be implemented during V4 only if it does not delay required milestone work.
- `POST_V4` — intentionally outside the frozen V4 finish line.
- `DEV_WORKFLOW_ONLY` — improves how MiniQuantDesk is built/reviewed, not the trading product itself.
- `SUPERSEDED_BY_MILESTONE_CONTRACT` — the substance is already captured by M1–M10 and should not exist as a second roadmap.

Moving an item from `MILESTONE_CANDIDATE` or `OPTIONAL_V4` to `MILESTONE_REQUIRED` requires one of:
1. an explicit operator decision updating the relevant milestone exit gate; or
2. a deterministic audit finding proving the milestone cannot meet its existing contract without the capability.

### I2. Product / trading-system capability backlog

| ID | Capability | Primary milestone mapping | Disposition | Why it is worth retaining | Completion effect |
|---|---|---|---|---|---|
| `JOURNAL-THESIS-01` | Trade thesis journal / decision attribution | M9, supports M10 evidence | `MILESTONE_CANDIDATE` | Preserve why a trade was entered/exited, strategy identity, regime, confidence, and decision context for later review and auditability. | Does not block M1–M8 by itself. |
| `TRADE-REVIEW-01` | Automated weekly trade review | M9 | `MILESTONE_CANDIDATE` | Automate strategy/time-of-day/regime performance review, expectancy, largest losses, stop-out reasons, and operator-facing summaries. | Candidate for autonomy; not an M1 blocker. |
| `RISK-STOP-IMMUTABILITY-01` | Stop-loss immutability / no risk widening after entry | M4, M9 | `MILESTONE_CANDIDATE` | A useful live-risk invariant: unchanged/tighter stop or reduced exposure may be permitted while widening risk is refused. | Becomes required only if the frozen supported Live strategy/risk model depends on explicit stops. |
| `ANALYTICS-MAE-MFE-01` | Maximum Adverse/Favorable Excursion analytics | M9 | `MILESTONE_CANDIDATE` | Useful for post-trade diagnostics, stop/target calibration, and forward-validation analysis without changing execution authority. | Not a platform blocker unless promoted into M9. |
| `REGIME-ATTRIBUTION-01` | Performance attribution by market regime | M9 | `MILESTONE_CANDIDATE` | Allows deployed/research strategy performance to be analyzed by trending/ranging/high-vol/low-vol/panic/other defined regimes. | Not a completion gate unless promoted. |
| `DECAY-01` | Strategy decay detection | M9 | `MILESTONE_CANDIDATE` | Detect deterioration of a previously admitted strategy and support bounded actions such as alert, size reduction, Paper revalidation, or disablement under policy. | Strong candidate for M9 autonomy; not allowed to auto-authorize new Live risk. |
...
Its useful substance is now authoritative in:

- **M5 — Asset-Neutral Production Contracts**
- **M6 — Crypto Paper + Live**
- **M7 — Futures + FX Paper + Live**
- **M8 — Options Paper + Live**
- **M10 — Integrated Multi-Asset Validation**

The following concepts remain important but are no longer separate free-floating backlog items: instrument registry, multi-feed management, per-asset risk models, concurrent scheduling, portfolio aggregation, asset-correct contract identity, margin/leverage, session semantics, expiry/roll, exercise/assignment, and cross-asset accounting/reconciliation.

### I4. Development-workflow backlog

These items may improve engineering efficiency but are **not MiniQuantDeskV4 product requirements** and cannot block a milestone:

| ID | Idea | Disposition | Notes |
|---|---|---|---|
| `DEV-CODEX-VERIFY-01` | Codex as independent verifier | `DEV_WORKFLOW_ONLY` | Potential use for repo audits, diff validation, test verification, and failure analysis. ChatGPT remains acceptance authority unless the workflow is explicitly changed. |
| `DEV-AGENT-FLOW-01` | Agent Flow observability | `DEV_WORKFLOW_ONLY` | Potentially visualize implementation-agent actions, tool calls, branching, and timing. |
| `DEV-AI-STACK-01` | ChatGPT architecture + Claude implementation + Codex verification | `DEV_WORKFLOW_ONLY` | Efficiency pattern only; never substitutes for current repo truth or independent acceptance. |

### I5. Backlog items deliberately not promoted to current priority
...
This file is the **single master program-tracking document for MiniQuantDeskV4 from this point forward**.

It has four authoritative layers:

1. **Completion Contract / M1–M10** — defines what “fully finished” means.
2. **Current Milestone Scoreboard and Immediate Sequencing** — defines where the project is now and what comes next.
3. **Tracked Capability Backlog** — preserves worthwhile future ideas without making them blockers automatically.
4. **Historical Master Patch Ledger** — preserves patch IDs, implementation history, evidence, prior audits, superseded states, and accepted technical detail.

### J1. Source-of-truth precedence

```text
CURRENT repo code / tests / Git / DB / real proof artifacts
        >
CURRENT milestone contract + newest current-truth entry in THIS document
        >
older ledger/history sections in THIS document
        >
other historical audits/specs/trackers
        >
old chat claims or memory
```

### J2. How new work is recorded

Every new finding, patch, or feature must record:

- milestone mapping;
- classification/disposition;
- exact invariant or capability;
- dependency;
- acceptance/finish condition;
- implementation status;
- proof/commit when completed.

A new idea must not be inserted directly into the active implementation path without first being mapped through the milestone and backlog rules above.

### J3. Status vocabulary

Use these project statuses consistently:
...
For backlog disposition use the closed set in §I1 rather than inventing ad hoc urgency labels.

### J4. Audit workflow

The normal audit sequence is:

```text
bounded milestone/subsystem
    ->
...
Friday/deep audits should therefore produce a bounded defect set mapped to the relevant milestone, not an unstructured list of every possible improvement in the repository.

### J5. No silent finish-line changes

Neither an audit, an implementation agent, a README, a new strategy idea, nor a future feature may change the M1–M10 finish line implicitly.

Changing the final definition of MiniQuantDeskV4 requires an explicit operator decision and a deliberate edit to the completion contract at the top of this document.



### J6. Machine-readable current patch-status index

The bounded block below is the **only machine-readable current patch-status authority** for production scripts that need exact patch-state lookup. Historical ledger text, summaries, dependency diagrams, prior audit prose, and later narrative mentions must never be interpreted as current automation authority.

The index is intentionally limited to patch IDs currently consumed by production operational tooling. Each indexed patch ID appears exactly once. Production parsing must fail closed if the sentinels are missing, a row is malformed, an indexed ID is duplicated, or a requested ID is absent.

<!-- BEGIN MQK_CURRENT_PATCH_STATUS -->
LIVE-SECRETS-CONSOLIDATION-01 = CLOSED
LIVE-ACCOUNT-TRUTH-01 = CLOSED
LIVE-TINY-CAPITAL-SMOKE-01 = IMPLEMENTATION_COMPLETE_OPERATOR_VALIDATION_DEFERRED
LIVE-FLATTEN-PROOF-01 = CLOSED
LIVE-TRUST-CHAIN-SHADOW-CAPTURE-01 = BLOCKED_BY_DEFERRED_OPERATOR_VALIDATION
LIVE-TRUST-CHAIN-PARITY-SCORER-01 = BLOCKED_BY_DEFERRED_OPERATOR_VALIDATION
LIVE-TRUST-CHAIN-EVIDENCE-SIGNER-01 = BLOCKED_BY_DEFERRED_OPERATOR_VALIDATION
LIVE-CAPITAL-EXTERNAL-PROOF-01 = BLOCKED_BY_DEFERRED_OPERATOR_VALIDATION
<!-- END MQK_CURRENT_PATCH_STATUS -->

## K. Pre-Friday Aggressive M1 Audit — 2026-09-07

**Audit mode:** READ ONLY. No repository file, database, broker/provider account, Paper runtime, Live runtime, scheduler, secret, or `smoke_logs/` artifact was modified.  
**Authoritative GitHub baseline audited:** `origin/main` = `30e0e0ec391be7d7039b67516c77dde2eea54e33` (`docs: record liveshadow provenance independent acceptance`).  
**Milestone mapping:** Primarily M1 — US Equity/ETF Paper Production. Later-milestone findings are recorded only when they are necessary to prevent false M1 closure or preserve future scope.  
**Purpose:** Build the complete currently-known defect/cleanup set before the deeper Friday implementation audit so Claude usage is spent on grouped implementation rather than discovery.

### K1. Executive verdict

**M1 is NOT audit-clean yet.** The old implementation ledger's `ACTIVE_IMPLEMENTATION=0` / `PENDING_INDEPENDENT_REVIEW=0` state is historically accurate for the rows it knew about, but current repo/CI truth has exposed additional required work.

Most importantly:

- current `main` is **red in CI**;
- the new M1–M10 master program document has **not yet been committed to GitHub**;
- multiple CI failures are stale/brittle tests or test-fixture debt rather than production regressions;
- one real Paper configuration contract is incomplete (`MQK_RISK_MAX_DRAWDOWN` absent from canonical setup docs/example despite being required by production risk wiring);
- the core execution/outbox/broker authority search found no new direct broker-submit bypass;
- the current Paper-vs-Live credential/endpoint separation search found no new cross-domain leak;
- operational Paper truth is still unproven and remains downstream of engineering cleanup.

This audit does **not** authorize Paper soak, LiveShadow smoke, Live Capital, broker calls, provider calls, DB migration against the running Paper DB, or holdout consumption.

### K2. Current GitHub / CI truth

Current `main` remains:

`30e0e0ec391be7d7039b67516c77dde2eea54e33`

Latest CI run inspected: GitHub Actions run `34144316804`.

| CI job | Current result | Audit interpretation |
|---|---|---|
| Safety Guards | PASS | No current contradiction from the canonical shell safety guards. |
| Python Research | PASS | No current CI contradiction to the accepted Research Python layer. |
| GUI Truth Tests + Build + Daemon Contract Gate | FAIL | Stale daemon operator-catalog contract test: daemon has 10 catalog entries while test still hardcodes 9. |
| Windows Platform | FAIL | Five PowerShell guard suites fail; several are stale/false-positive test contracts rather than proven production defects. |
| DB Proof Lane | FAIL | Six broker-cursor restart scenarios fail during fixture cleanup because the accepted `runtime_leader_lease -> runs` FK is `RESTRICT`; cleanup order is stale. |
| Rust fmt + clippy + test | FAIL at fmt | `cargo fmt --check` reports formatting drift across 134 Rust files. Because fmt fails first, clippy and the full workspace Rust test step do not run in this CI job. |

**Consequence:** M1 cannot claim a green integrated code/test boundary until the failures are repaired and exact-head CI is green.

### K3. Confirmed M1 findings

#### AUDIT-M1-01 — Current main CI is red

**Classification:** `V4_REQUIRED`  
**Status:** OPEN  
**Milestone:** M1  
**Invariant:** The accepted M1 engineering baseline must have a reproducible green CI boundary.  
**Notes:** This is an umbrella acceptance condition. It is closed only by resolving the specific child findings below; do not create a patch whose only behavior is to suppress CI failures.

---

#### AUDIT-M1-02 — GUI/daemon operator catalog contract hardcodes 9 entries while production catalog has 10

**Classification:** `V4_REQUIRED`  
**Status:** OPEN  
**Milestone:** M1  
**Primary surfaces:** `core-rs/crates/mqk-daemon/tests/scenario_gui_daemon_contract_gate.rs`; adjacent `scenario_ops_control_oc01_oc02.rs`; GUI action mapping tests.

**Confirmed current truth:** Production operator catalog includes 10 actions, including `flatten-paper-positions`. The failing test still requires exactly 9.

**Required repair shape:** Update the tests to assert the current explicit action identity rather than a stale cardinality. Preserve the existing GUI contract that `flatten-paper-positions` is not automatically treated as an ordinary GUI-invokable action unless separately authorized by the relevant product contract.

**Do not:** Remove a real daemon action merely to make the old test pass.

---

#### AUDIT-M1-03 — Rust formatting baseline is not clean under the pinned canonical toolchain

**Classification:** `V4_REQUIRED`  
**Status:** OPEN  
**Milestone:** M1  
**Evidence:** Current CI reports `cargo fmt --check` drift in 134 Rust files under `core-rs/rust-toolchain.toml` channel `1.93.1`.

**Required repair shape:** One formatting-only commit generated by the canonical pinned toolchain. No semantic changes. Review the resulting diff mechanically before acceptance.

**Important:** After this repair, clippy/full Rust workspace tests may expose additional defects that current CI cannot reach. Those results become new findings rather than being assumed green.

---

#### AUDIT-M1-04 — PowerShell smoke/GUI relaunch source-style guard fails on non-ASCII bytes

**Classification:** `V4_REQUIRED` / test-contract hygiene  
**Status:** OPEN  
**Milestone:** M1  
**Primary surface:** `scripts/windows/Start-PaperTradingSmoke.ps1` and its guards.

**Confirmed current truth:** At least two Windows guard suites fail on the same non-ASCII bytes in the smoke script (a section-symbol comment). Functional relaunch assertions otherwise pass.

**Required repair shape:** Make the source conform to the repository's script-source policy or make the policy explicit if Unicode is intentionally supported. Do not mix behavioral changes into this patch.

---

#### AUDIT-M1-05 — Paper-smoke forbidden-trading-endpoint guard is likely matching harmless text rather than an executable broker route

**Classification:** `V4_REQUIRED` / test-guard repair  
**Status:** OPEN  
**Milestone:** M1  
**Evidence:** `test_start_paper_trading_smoke.ps1` flags a `submit.*order` pattern even though the launcher audit did not expose a direct broker submission call.

**Required proof before repair:** Identify the exact matched source span.  
**Required repair shape:** Preserve the prohibition on direct broker/order submission while eliminating false positives from comments/descriptive strings if that is the actual cause. Mutation-negative proof must still fail on a genuine direct submission pattern.

---

#### AUDIT-M1-06 — Discord secret-safety guard false-positives on `%error` as if it were `%err`

**Classification:** `V4_REQUIRED` / `TEST_WEAKNESS`  
**Status:** OPEN  
**Milestone:** M1  
**Primary surfaces:** `tests/script_guards/test_discord_secret_safety.ps1`, `core-rs/crates/mqk-daemon/src/notify.rs`.

**Confirmed current truth:** Production `notify.rs` uses `%error` and sanitized delivery summaries; the guard's `%\s*err` pattern also matches the prefix of `%error`.

**Required repair shape:** Make the forbidden-identifier match token/boundary aware. Preserve a mutation-negative case proving an actual raw `%err` logging path is still rejected.

---

#### AUDIT-M1-07 — Multi-symbol day-order-cap guard is bound to a superseded outbox API shape

**Classification:** `V4_REQUIRED` / stale guard  
**Status:** OPEN  
**Milestone:** M1  
**Primary surfaces:** `tests/script_guards/test_multi_symbol_day_order_cap.ps1`, `core-rs/crates/mqk-daemon/src/decision.rs`.

**Confirmed current truth:** Production uses `outbox_enqueue_for_running_run` and typed `OutboxEnqueueOutcome::{Enqueued,Duplicate,RunNotRunning}`. The symbol/day counter still increments only on a genuine `Enqueued` result. The guard expects the old boolean `outbox_enqueue` pattern.

**Required repair shape:** Rebind the guard to the actual typed production seam and retain a negative control that would catch counting duplicates/non-enqueues.

---

#### AUDIT-M1-08 — Multi-symbol OMS GUI guard contains a brittle historical-ledger/history assertion

**Classification:** `V4_REQUIRED` / obsolete guard governance  
**Status:** OPEN  
**Milestone:** M1  
**Primary surface:** `tests/script_guards/test_multi_symbol_oms_overview_gui.ps1`.

**Confirmed current truth:** The failing G14 assertion depends on historical commit/ledger-touch behavior rather than a current production invariant. It will also conflict with the new master program-document migration.

**Required repair shape:** Replace the historical assertion with a present-state invariant that actually protects the OMS/GUI contract, or retire that assertion if no live invariant remains. Do not weaken current GUI truth rendering.

---

#### AUDIT-M1-09 — PDT cross-symbol self-test can report green after fixture creation fails

**Classification:** `V4_REQUIRED` / high-value `TEST_WEAKNESS`  
**Status:** OPEN  
**Milestone:** M1  
**Confirmed current truth:** Clean Windows CI logs a PowerShell `Set-Content` failure while attempting to create a fixture under repo `smoke_logs/...`, yet the self-test still reports all 11 assertions passed.

**Why this matters:** A mutation/self-test that cannot prove its own fixture setup is invalid evidence.

**Required repair shape:** Use a dedicated test temp directory (prefer OS temp or a test-owned location, not protected `smoke_logs/`). Set `$ErrorActionPreference = "Stop"` or equivalent explicit setup checks so fixture-creation failure makes the test fail. Add a negative control proving setup failure cannot produce a green result.

---

#### AUDIT-M1-10 — Broker-cursor DB proofs use stale cleanup ordering after the accepted runtime-leader-lease FK hardening

**Classification:** `V4_REQUIRED` / DB test-fixture cleanup  
**Status:** OPEN  
**Milestone:** M1  
**Primary surface:** `core-rs/crates/mqk-testkit/tests/scenario_broker_cursor_restart.rs`.

**Confirmed current truth:** Six of seven scenarios fail at cleanup because they attempt to delete the parent `runs` row while a `runtime_leader_lease` row still references it. The FK is intentionally `RESTRICT`.

**Required repair shape:** Delete/release the runtime lease through the canonical DB seam before deleting the run fixture. Preserve the `RESTRICT` FK. Do not weaken production schema integrity to satisfy test cleanup.

**Acceptance implication:** Because `scripts/db_proof_bootstrap.sh` uses `set -e`, later DB proof lanes after this suite are not current-CI-proven until this blocker is fixed and the harness completes.

---

#### AUDIT-M1-11 — Canonical Paper setup omits required `MQK_RISK_MAX_DRAWDOWN`

**Classification:** `V4_REQUIRED`  
**Status:** OPEN  
**Milestone:** M1  
**Primary surfaces:** `.env.local.example`, `docs/runbooks/autonomous_paper_ops.md`, production contract in `core-rs/crates/mqk-daemon/src/state/orchestrator_build.rs`.

**Confirmed current truth:** Production now requires initial equity, daily-loss limit, and max-drawdown authority. `MQK_RISK_MAX_DRAWDOWN` is read by the canonical daemon and absence ultimately fails the runtime risk gate closed. The canonical env example and autonomous Paper runbook document only the first two values.

**Required repair shape:** Add/document the third required variable with a clearly labeled example value and update wording from “both” to the complete required set. Add a guard/test tying setup documentation to the production-required env contract if practical.

**Do not:** Make max drawdown optional merely to match stale docs.

---

#### AUDIT-M1-12 — GUI lockfile contains a current high-severity NanoID advisory

**Classification:** `V4_REQUIRED` / supply-chain maintenance  
**Status:** OPEN  
**Milestone:** M1  
**Primary surface:** `core-rs/mqk-gui/package-lock.json`.

**Confirmed current truth:** Current CI `npm ci` reports one high-severity vulnerability. The lockfile resolves `nanoid@3.3.16`. Current 2026 advisory evidence for GHSA-2v37-7h3g-55p8 / CVE-2026-67213 identifies the vulnerable 3.x line below `3.3.18`.

**Required repair shape:** Determine the exact dependency path with `npm audit --json` / `npm ls nanoid postcss vite --all`, then perform the narrowest compatible lockfile/dependency resolution to a patched NanoID 3.x floor (or a parent-package upgrade that resolves it). Run GUI tests/build and a fresh high/critical audit.

**Do not:** Use `npm audit fix --force` blindly or bundle unrelated package upgrades.

---

### K4. Unknown-needs-proof finding

#### AUDIT-M1-U01 — RuntimeRiskGate uses poison-panicking `Mutex::lock().expect(...)`

**Classification:** `UNKNOWN_NEEDS_PROOF`  
**Status:** OPEN FOR PROOF, NOT AN IMPLEMENTATION BLOCKER YET  
**Milestone mapping:** M1 availability/safety adjacency  
**Primary surface:** `core-rs/crates/mqk-runtime/src/runtime_risk.rs`.

Production risk-state lock sites call `lock().expect("runtime risk gate lock")`. A poisoned mutex would therefore panic subsequent risk calls.

However, the code executed while that mutex is held delegates to `mqk-risk::evaluate`, and the audited `mqk-risk/src` production implementation contains no explicit `unwrap`, `expect`, or `panic!` path. No deterministic production path poisoning this mutex was found in this pass.

**Current disposition:** Do not spend implementation time on this unless Friday's mutation/proof demonstrates a reachable poison path or establishes that the system's required fail-closed contract demands typed refusal even for externally-induced poison. This is not currently allowed to reopen accepted risk work on pattern-matching alone.

### K5. Confirmed non-findings / accepted paths not reopened

These searches found no new deterministic contradiction and therefore remain frozen:

1. **Execution/order authority:** production order writers are on `outbox_enqueue_for_running_run`; the compile-time broker invocation token remains owned by the execution router; no new direct production broker-submit bypass was found.
2. **Paper/Live endpoint and credential separation:** current daemon selects Paper Alpaca credentials + `paper-api.alpaca.markets` for Paper and Live credentials + `api.alpaca.markets` for LiveShadow/LiveCapital. No new cross-domain credential/URL leak was found.
3. **Research Python CI:** current exact-head Python Research job is green.
4. **Canonical safety shell guards:** current exact-head Safety Guards job is green.
5. **Old migration TODO for broker-map FK:** migration `0010` contains a stale “FK deferred” comment, but migration `0013_broker_map_outbox_fk.sql` actually implements the FK. This is historical-comment staleness, not an open schema defect. Do not mutate an applied historical migration merely to clean the comment.

### K6. Future/later-milestone capabilities intentionally not promoted into M1

The audit encountered truthful unsupported/future behavior. These do not block M1 unless the completion contract is explicitly changed:

- simultaneous independent Paper + Live runtime domains → M3;
- full Equity/ETF Live production → M4;
- non-equity execution → M5–M8;
- protective stop/bracket architecture → M4/M9 candidate depending on frozen strategy/risk policy;
- generalized hot mode-switching → not required by M1;
- broad code lean-out → maintenance unless a deterministic defect depends on it.

### K7. Repository cleanup disposition

No deletion is authorized by this audit. The classifications below are preparation for a separate exact-staging cleanup commit.

#### SAFE DELETE after the new master document is committed

| Path | Disposition | Evidence / rationale |
|---|---|---|
| `miniquantdesk_future_backlog_brainstorm.md` | SAFE DELETE | No current repo references found; useful ideas have been absorbed into this master document's capability backlog. |
| `package-lock.json` (repo root) | SAFE DELETE | There is no root `package.json`; the lock contains no meaningful package graph. The real GUI lockfile is `core-rs/mqk-gui/package-lock.json`. |
| `.cargo/config.toml` | SAFE DELETE | Zero-byte tracked placeholder; no current repo dependency found. |
| `APPLY_GUI_FUTURE_BUNDLES.ps1` | SAFE DELETE | Old patch-bundle applier references untracked/nonexistent `gui_future_bundle*.zip` inputs; no current repo caller found. Retaining it creates accidental stale-overwrite risk. |
| `core-rs/APPLY_BACKTEST_PATCHSET.ps1` | SAFE DELETE | Legacy one-shot source-overwrite/append script for old BKT patches, no current repo caller; current accepted backtest code has evolved far beyond this patchset. Accidental execution would be destructive. |

#### LIKELY SAFE DELETE / optional historical cleanup

| Path | Disposition | Notes |
|---|---|---|
| `docs_archive/` | LIKELY SAFE DELETE | 17 tracked files, ~49 KB; no living repo reference found. Git history preserves them. Deleting is reasonable if the goal is a lean repository, but it is historical cleanup rather than a correctness task. |

#### KEEP / NEEDS MIGRATION FIRST

| Path | Disposition | Reason |
|---|---|---|
| `MiniQuantDesk_Master_Patch_Ledger_v2_updated.md` | MIGRATE THEN DELETE | The new master document contains this history, but current launcher/README/runbooks/guards still reference this exact filename. `Start-MiniQuantDesk.ps1` actively reads it for Live blocker status. First migrate every living authority reference and prove parsing against the new master filename. |
| `MiniQuantDesk_Master_Patch_Ledger_v2.md` | KEEP HISTORICAL FOR NOW | Large old ledger, but many historical spec/audit links still point to it and it may contain detailed archival material not guaranteed to be duplicated byte-for-byte in the updated ledger. Do not casually remove it. |
| `backfill_daily_bars.ps1` | KEEP / RELOCATE CANDIDATE | Unreferenced by current automation, but it is a real historical-market-data backfill utility. Better move under `scripts/windows/` later if still desired. |
| `autonomous_reality_test_paper.ps1.ps1` | KEEP / RENAME LATER | Ugly duplicate extension, but current documentation references it. Renaming requires coordinated link/script updates. |
| `data/raw/tiingo/**`, `data/canonical/tiingo_top10_30Y_1D.csv` | KEEP PENDING DATA-PROVENANCE REVIEW | Large tracked research inputs may participate in reproducibility/identity. Size alone is not deletion authority. |
| `core-rs/crates/mqk-db/migrations/hold/0017_inbox_broker_fill_id_global_unique.sql` | KEEP | Explicitly held/deferred migration decision; not junk. |
| `smoke_logs/` | PROTECTED / DO NOT TOUCH | Standing repository rule. |

### K8. Master-document migration requirement

The new master document itself is not yet present on GitHub `main`.

Before Friday's implementation controller begins, perform one bounded governance patch:

1. add `MiniQuantDeskV4_Master_Program_Plan_and_Ledger.md`;
2. update **living** authority references such as `README.md`, `Start-MiniQuantDesk.ps1`, active runbooks, and relevant guards from `MiniQuantDesk_Master_Patch_Ledger_v2_updated.md` to the new filename;
3. prove the launcher's status parser still locates the required historical/live rows after the old ledger content is moved lower in the master document;
4. delete `MiniQuantDesk_Master_Patch_Ledger_v2_updated.md` only after that proof;
5. leave references inside immutable historical reports alone when they are merely statements about what filename existed at the time.

This is a governance migration, not a trading-behavior change.

### K9. Recommended implementation waves

To conserve implementation-agent usage, do not create one Claude prompt per finding.

#### WAVE A — Canonical formatting + CI reachability

- `FMT-BASELINE-01`: canonical `cargo fmt` only, one formatting-only commit.
- Run fmt check immediately.
- Then run clippy and targeted/full Rust tests far enough to discover any failures that were previously masked by fmt.
- Stop and report newly exposed deterministic findings before broader behavior edits.

#### WAVE B — CI contract / guard credibility

Suggested coherent patch sequence:

1. operator catalog contract tests: explicit current 10-action identity, preserve GUI flatten policy;
2. smoke/GUI-relaunch non-ASCII source-policy repair;
3. smoke direct-order guard exact-match repair with mutation negative control;
4. Discord `%err` token-boundary guard repair;
5. multi-symbol day-cap guard update to typed fenced outbox seam;
6. OMS GUI historical assertion replacement/removal;
7. PDT cross-symbol self-test fixture hardening.

Each invariant gets its own commit even if delivered in one controller.

#### WAVE C — DB test-fixture integrity

- broker-cursor restart cleanup must release/delete runtime lease before run;
- preserve `RESTRICT`;
- run the seven focused scenarios;
- then run the complete repo-native DB proof harness once.

#### WAVE D — Paper configuration contract + GUI supply-chain repair

1. add/document required `MQK_RISK_MAX_DRAWDOWN` in canonical example/runbook, with a contract guard if practical;
2. verify exact NanoID audit dependency path and make the narrowest patched dependency/lock resolution;
3. run GUI tests/build and `npm audit --audit-level=high` (or equivalent JSON proof).

#### WAVE E — Master tracker migration + obsolete-file cleanup

May be executed before or after A–D, but should be completed before the Friday controller relies on the new document as repo authority.

- add new master program document;
- migrate living `_v2_updated.md` references;
- prove launcher status lookup;
- delete superseded `_v2_updated.md`;
- exact-stage safe-delete files listed in K7;
- optionally delete `docs_archive/` in its own cleanup commit.

### K10. M1 audit status after this pass

Current known classification:

- `V4_REQUIRED`: 12 concrete findings/acceptance repairs (including the red-CI umbrella and its bounded child causes; the umbrella is not a separate behavior patch).
- `UNKNOWN_NEEDS_PROOF`: 1 runtime-risk mutex-poison adjacency item.
- `DEFERRED_REAL`: branch-protection/governance and other later-release hardening already known; does not block current Paper correctness.
- `FUTURE`: M2–M10 capability expansion remains governed by the frozen roadmap.
- confirmed core execution/order authority regressions discovered in this pass: **NONE**.
- confirmed Paper/Live credential/endpoint-crossing regressions discovered in this pass: **NONE**.

**M1 status: IN PROGRESS / ENGINEERING AUDIT REOPENED BY CURRENT REPO TRUTH.**

The next deep audit must use the specific open findings above as a starting set, re-trace each full bounded subsystem for adjacent defects, and then freeze the defect set before implementation. It must not re-open already-accepted systems merely because their code is complex.

# Existing Master Patch Ledger / Implementation History

**The ledger below is intentionally retained.** It remains the detailed source of implementation history, patch IDs, accepted invariants, deferred work, tests, commits, and prior audit evidence. Where historical status text conflicts with newer repo truth or the current milestone scoreboard above, current repo truth and the newest accepted current-truth entry govern.
...
### Runtime-Risk Correction Superseded (`POST-WAVE06-LEDGER-BURN-01` L0, 2026-09-06)

**The "Required current verdict" bullet immediately above — specifically its `RUNTIME-RISK-DYNAMIC-STATE-AUTHORITY-01` blocker claim — is itself now stale and superseded.** Direct verification against current HEAD (`84dcd14c`) shows this repair closed same-day, hours after the 2026-08-30 situational audit that raised it: commits `379e0b16`/`97097545`/`78e74b43` (08:53), `3f7bcedc` (10:34), `4a507818` (10:45), `e5e33d50` (11:04), `ddadf1c4` (11:10), `dab41b1c` (11:13) — all eight independently confirmed ancestors of this HEAD via `git merge-base --is-ancestor`. Point-by-point at current HEAD: (1) live-equity-drop denies daily-loss/max-drawdown — proven end-to-end by `mqk-runtime/tests/scenario_runtime_risk_dynamic_authority_e2e_01.rs` CASE 1/CASE 2 against the real `BrokerGateway`+`RuntimeRiskGate`; (2) reject-storm negative control denies — CASE 3 of the same file plus `mqk-testkit/tests/scenario_inbound_reject_storm_rr4_01.rs` against a real `ExecutionOrchestrator`; (3) day-rollover resets the daily-loss baseline — `runtime_risk.rs`'s `day_rollover_resets_day_start_equity_without_reconstruction` unit test, `day_id_for()`/`reject_window_id_for()` computed fresh per call; (4) PDT is explicitly disabled (`ddadf1c4`), not silently no-op'd, with a refuse-to-start guard if a run's config demands legacy enforcement; (5) `kill_switch: None` is documented as an intentional disposition (canonical halt authority lives in `StateIntegrityGate`, evaluated as an independent prior gate), not a silent no-op; (6) max-drawdown is threaded through the daemon's ordinary run-config seam (`orchestrator_build.rs::effective_run_config_for_risk` + `runtime_risk_config_from_run_config`), failing the whole gate closed if still absent after supplementation, never silently disabled. All six sub-conditions the situational audit itself named as the acceptance criterion are satisfied with direct production-path proof, not merely unit tests in isolation. **`RUNTIME-RISK-DYNAMIC-STATE-AUTHORITY-01` is REMOVED from the open critical path to Paper readiness** — it was never itself a Section-5 row and requires no new row. What remains genuinely open and unverified by this docs-only pass (operational, not a code defect): whether `MQK_RISK_MAX_DRAWDOWN`/`MQK_RISK_DAILY_LOSS_LIMIT` are actually set in the real running Paper deployment's environment, and migration `0067` confirmation against the running Paper DB — both require live-environment/DB access this reconciliation did not have and this mission does not authorize. **Per the controlling operator instruction for this wave (`MQK-LEDGER-BURN-WAVE-01`), this correction does not itself reopen or authorize any Paper soak, migration verification, or operational validation — the production ledger must reach zero active rows first.**
...
### Active Production-Ledger Baseline (`POST-WAVE06-LEDGER-BURN-01` L0, 2026-09-06)

**The historical "43 remaining" figure, referenced repeatedly elsewhere in this ledger (e.g. the Research/Backtest robustness-finding notes' "NOT PART OF ACTIVE 43-PATCH COUNT" annotations), is retired as an operational planning denominator.** Two independent reconciliation sessions attempted to reconstruct which specific subset of Section 5's rows produces exactly 43 and could not: the ledger's own "Active Patch Counts" line (READY 33 + BLOCKED 4 + DEFERRED 8 = 45, not 43; corrected DEFERRED-count arithmetic gives 44, still not 43) does not match it, no adjacent review bundle defines it, and the ledger's own text treats "43" purely as a pre-existing named figure without ever writing down the rule that derives it from Section 5's current rows. Per that reconciliation's own instruction — "if provenance cannot be established deterministically, do not publish a replacement authoritative remaining count; this is preferable to inventing a new baseline" — no replacement number is being backed into 43's old role. Historical "43" references elsewhere in this document are left as-is, as history only; their provenance is not fabricated by this correction.

**THIS IS A FRESH CURRENT-HEAD REBASELINE, NOT "43 MINUS N."** It answers a different question — not "what did 43 become," but "of Section 5's 67 rows as they stand at HEAD `84dcd14c`, how many are genuinely still open work" — established directly from the completed Section-5 reconciliation in `MQK-POST-WAVE06-LEDGER-RECONCILIATION-REPAIR.zip` (`10-row-reconciliation.csv`/`11-row-reconciliation.md`, SHA-256 `974d796f3dc409291540c7c752e02a21dde65588873b1e8e47a3a03ab38797ad`):

```text
Total Section 5 rows ......................................................... 67
Already ledger-status CLOSED (independently re-confirmed accurate) ..........  2
Non-CLOSED ("active") rows ................................................... 65
  ALREADY_CLOSED_LEDGER_STALE (reclassified by this L0 pass, see above) ..... 29
  BLOCKED ....................................................................  6
  DEPENDENCY_SATISFIED (blocker cleared, work not started) ..................  6
  OPEN ....................................................................... 24
  Sum check: 29 + 6 + 6 + 24 = 65   [MATCHES]

ACTIVE_PRODUCTION_LEDGER_BASELINE = 36   (BLOCKED 6 + DEPENDENCY_SATISFIED 6 + OPEN 24)
```
...
### MQK-LEDGER-BURN-CONTROLLER-02 — Phase W2 Results (2026-09-06)

| Row | Result | Commit |
|---|---|---|
| W2-A `DYNAMIC-SELECTION-E2E-SCENARIO-TEST-01` | CLOSED — new HTTP-route-read scenario test (real production seams throughout); dispatch-authority construction itself confirmed already covered in-crate | `889db5f0` |
| W2-B `PAPER-SOAK-RUST-TIMING-TEST-HARDENING-01` | Investigation only, no code change. **Reproduced 4/4** on this branch's current HEAD (materially different from the prior session's 0/4 on a different branch) — see the row's own updated text above. New defect row filed, not fixed here. | — (docs only) |
| W2-C `MULTI-SYMBOL-CAP1-TRUNCATE-SURFACE-01` | CLOSED — ALREADY_CLOSED_LEDGER_STALE; truncate-and-surface is already fully implemented and tested at `watchlist_intake.rs` (ancestor commits `45fe9557`/`99b9745a`/`3e5944e3`), the ledger's "Current Source Truth" pointer was simply stale | — (docs only) |
| W2-D `AUTONOMOUS-DATA-BLOCKER-AUTO-RECOVERY-01` | Investigated, not implemented. Real prerequisite gap found (typed-vs-string fault classification for `daily_data_readiness_blocked`) that must close first, without weakening the retry-policy module's own no-string-authority rule or building a second retry framework — genuine design/plumbing work, not a mechanical composition of two already-precise seams. Left OPEN, no code changed. | — (docs only) |
| W2-E `LIVE-TINY-CAPITAL-SMOKE-01` | Engineering portion CLOSED (`IMPLEMENTATION_COMPLETE_OPERATOR_VALIDATION_DEFERRED`) — `Start-LiveShadowSmoke.ps1` + 8/8 guard proof. Real-operational-smoke half genuinely blocked upstream (canonical launcher's Live path is itself still read-only/report-only today), not merely postponed. | `0e98a9ed` |

**New defect rows discovered this phase:** `AUTOFRESH-SCHEDULER-BARRIER-CALL-NEVER-STARTS-01` (W2-B).

**Recomputation** (best-effort tally against this session's own Phase-R baseline of 21; not an independent full re-verification of Section 5's 67-row list against this session's specific closures — see the caveat on `MULTI-SYMBOL-CAP1-TRUNCATE-SURFACE-01`/`LIVE-TINY-CAPITAL-SMOKE-01`/`DYNAMIC-SELECTION-E2E-SCENARIO-TEST-01` each being long-standing `READY` Section-5 rows presumed already counted in that baseline):

```text
ACTIVE_IMPLEMENTATION_LEDGER (start of W2) = 21

W2-A DYNAMIC-SELECTION-E2E-SCENARIO-TEST-01      READY -> CLOSED              -1
W2-B new row AUTOFRESH-SCHEDULER-BARRIER-CALL-NEVER-STARTS-01  discovered     +1
W2-C MULTI-SYMBOL-CAP1-TRUNCATE-SURFACE-01       READY -> CLOSED (stale)      -1
W2-D AUTONOMOUS-DATA-BLOCKER-AUTO-RECOVERY-01    OPEN -> OPEN (no change)      0
W2-E LIVE-TINY-CAPITAL-SMOKE-01     READY -> IMPLEMENTATION_COMPLETE /
                                    OPERATOR_VALIDATION_DEFERRED (removed)    -1
                                                            ---------------------
ACTIVE_IMPLEMENTATION_LEDGER (after W2) = 19
```

**W2_OPERATOR_VALIDATION_DEFERRED (new this phase):** `LIVE-TINY-CAPITAL-SMOKE-01` (real LiveShadow operational smoke — blocked upstream, not merely postponed).
...
### MQK-LEDGER-BURN-CONTROLLER-03 — Phase A/B Results and Count Sync (2026-09-06)

Independent review of Controller-02's final state (manifest SHA256
`5585e28b0b33008c89e900411922255b1a03d74cd98808358548d3b8bc9795f1`, verified
13/13 source hashes) disputed two of the 17 rows Controller-02 had removed
from `ACTIVE_IMPLEMENTATION_LEDGER`: `CI-TESTKIT-FEATURE-GUARD-VERIFY-01`
(the cross-crate feature-alias false negative below) and
`LIVE-TINY-CAPITAL-SMOKE-01` (the current-truth defects below) — giving a
provisional corrected starting count of 19, not authoritative until this
controller's own repair/reconciliation resolved each.

| Row | Result | Commit |
|---|---|---|
| A1 `CI-TESTKIT-FEATURE-GUARD-VERIFY-01-REPAIR-02` | CLOSED — guard rewritten with a true cross-crate Cargo feature graph (local-alias chains, `<crate>/<feature>`/`dep:<crate>` edges through `package="..."` renames and `workspace=true` inheritance, dependency default-feature propagation). The prior REPAIR-01 guard compared only literal requested-feature strings against `"testkit"` and missed `features=["full"]` where the target crate's own `full = ["testkit"]`. All 8 required `TKG-R2-01..08` two-crate mutation-negative cases pass against the real guard entrypoint, plus all 11 prior `TKG-*`/`TKG-R2-08` cases including a clean pass against the real repo. | `957908b7` |
| A2 `PAPER-SOAK-RUST-TIMING-TEST-HARDENING-01` | CLOSED — root cause was the test's real-calendar dependency, not a scheduler defect. Controller-02's reproduction ran on 2026-09-07 (NYSE Labor Day); today (2026-09-06) is a Sunday — both non-trading days, both hit the same `!schedule.is_trading_day` early return in `run_required_universe_cycle` before the injected provider is ever called. Added a narrow `AppState` test seam (`required_universe_calendar_override_for_test`) so the race test's immediate-cycle trading-day truth is deterministic while `now_a`/`now_b` stay real `Utc::now()` (background-loop wake-time scheduling still needs the real wall clock, unchanged). Negative control: reverting just the override reproduces the exact original failure against today's real calendar; with the fix, 10/10 consecutive passes plus the full 19-test scenario file green. | `40348e0c` |
| A2 `AUTOFRESH-SCHEDULER-BARRIER-CALL-NEVER-STARTS-01` | SUPERSEDED / DUPLICATE — same root cause as above, not an independent production scheduler defect. Disposed, not carried forward as a separate row. | `40348e0c` |
| A3A (new capability) | `-Mode LiveShadow` / `-DeploymentMode live-shadow` added to the canonical launcher (`Start-MiniQuantDesk.ps1` / `Launch-VeritasLedger.ps1`), reusing the existing daemon-bootstrap seam (`Ensure-DaemonBinary`/`Start-DaemonIfNeeded`/`Get-BackendProbe`, now deployment-mode-parameterized) rather than a second framework. Hard invariants (LiveShadow can never resolve to LiveCapital, fail-closed on missing `MQK_DATABASE_URL`/`ALPACA_API_KEY_LIVE`/`ALPACA_API_SECRET_LIVE`, no inherited `Confirm-LiveIntent` prompt) proven by 40 hermetic tests plus real `-CheckOnly` subprocess invocations. Zero regressions across 9 dependent test files. | `110392ec` |
| A3B `LIVE-TINY-CAPITAL-SMOKE-01` | Engineering now genuinely `IMPLEMENTATION_COMPLETE_OPERATOR_VALIDATION_DEFERRED` (not merely re-asserted) — `Start-LiveShadowSmoke.ps1` now delegates to the real `-Mode LiveShadow` (closing Controller-02 defect: it previously delegated to LiveCapital's inert `-Mode Live`, which structurally could not run a full smoke and inherited `Confirm-LiveIntent`'s prompt). `manifest.json` no longer hardcodes `real_daemon_start_performed`/`real_broker_call_performed`/`real_order_submitted` to a fabricated `$false`; it now distinguishes wrapper-static-contract facts (provably `$false`, unconditionally) from observed-runtime-evidence (`'not_run'`/`'observed_true'`/`'observed_false'`/`'not_observed'`, derived from the canonical launcher's own JSON log, never invented). 11/11 guard proofs, including one real `-CheckOnly` invocation. Only the real operational LiveShadow smoke remains deferred. **Controller-04 R1 note:** two further evidence-provenance defects found by independent review (stale-log "newest file" selection; `real_daemon_start_performed` conflating started-vs-attached) are now repaired — see the Controller-04 R1 section below. | `9d99b407` (repaired by `625b2eb6`) |
| Phase B `DOCS-TRACKER-RETIREMENT-01` | CLOSED — full 18-item audit of `ACTIVE_PATCH_LEDGER_20260425.md` §1 against current repo truth (see that row's own entry above for the per-item table); obsolete file deleted. | `5d3ec589` (audit), `b7b1f37b` (deletion) |
| Phase B new rows | `API-TYPES-RS-LEAN-OUT-01`, `HELD-MIGRATION-0017-DECISION-01`, `EXPORT-HANDOFF-SECRET-EXCLUSION-01` — all `DEFERRED`/Lane F, tracked only, not implemented (docs-only reconciliation pass). | `5d3ec589` |

**Recomputation** — a delta tally against the Controller-02-established chain (`25 → 23 → 21 → 19 → 17`), continuing the same methodology and the same caveat those sessions themselves stated: this is not an independent full re-verification of Section 5's entire row list, only of the rows this controller lineage has actually discovered/touched. `DEFERRED` rows (including all three new Phase B rows) have never contributed to this count, consistent with every prior recomputation in this chain.

```text
ACTIVE_IMPLEMENTATION_LEDGER (end of Controller-02 W3) = 17
Independent-review-corrected start (2 disputed rows reopened)    +2
                                                            ---------------------
PROVISIONAL_REVIEW_CORRECTED_ACTIVE (start of Controller-03) = 19

A1  CI-TESTKIT-FEATURE-GUARD-VERIFY-01-REPAIR-02   reopened -> CLOSED       -1
A2  PAPER-SOAK-RUST-TIMING-TEST-HARDENING-01       REPRODUCED -> CLOSED     -1
A2  AUTOFRESH-SCHEDULER-BARRIER-CALL-NEVER-STARTS-01  OPEN -> SUPERSEDED    -1
A3B LIVE-TINY-CAPITAL-SMOKE-01     reopened -> IMPLEMENTATION_COMPLETE /
                                    OPERATOR_VALIDATION_DEFERRED (removed)  -1
B   DOCS-TRACKER-RETIREMENT-01                      OPEN -> CLOSED          -1
B   3 new Lane F rows (all DEFERRED, never counted)                          0
                                                            ---------------------
ACTIVE_IMPLEMENTATION_LEDGER (after Controller-03 Phase A/B) = 14
```

**Categories, no row in more than one:**
- `IMPLEMENTATION_COMPLETE_OPERATOR_VALIDATION_DEFERRED` (this phase, added): `LIVE-TINY-CAPITAL-SMOKE-01` (joins the existing `PAPER-AUTOMATIC-PREOPEN-SCHEDULER-01`/`-REPAIR-01` from W3).
- `BLOCKED_BY_DEFERRED_OPERATOR_VALIDATION`: `LIVE-TRUST-CHAIN-SHADOW-CAPTURE-01` → `-PARITY-SCORER-01` → `-EVIDENCE-SIGNER-01` → `LIVE-CAPITAL-EXTERNAL-PROOF-01` (unchanged chain; `LIVE-TINY-CAPITAL-SMOKE-01` engineering completion does not unblock these — they require the real operational smoke, still deferred).
- `PARKED_PRODUCT_DECISION`: none newly added this phase.
- `DEFERRED_POST_LEDGER_AUDIT`: `LIVE-SHADOW-FLATTEN-ACTION-ROUTE-GAP-01` (unchanged) plus the three new Phase B Lane F rows.
- `RED_AUTHORIZATION_REQUIRED`: none newly added this phase (`DEADMAN-LEASE-TTL-RECONCILE-01` remains READY-but-RED from before, unchanged).
...
| `AUTONOMOUS-DATA-BLOCKER-AUTO-RECOVERY-01` PATCH 2 | CLOSED (automatic wiring) — see the row's own entry above for full detail. **Controller-04 R2 note:** independent review found a CAS-concurrency defect in this patch (the coordinator treated the DB's own `AlreadyApplied` outcome as "still blocked" instead of projecting the already-committed `preparing_data` truth) — repaired, see the Controller-04 R2 section below. | `2a7dad50` (repaired by `a775f4f9`) |
...
### MQK-LEDGER-BURN-CONTROLLER-04 — R1/R2 Independent-Review Repairs (2026-09-07)

Independent review of Controller-03's final state found two narrow deterministic
defects in otherwise-accepted work. Both are truth/correctness repairs on top
of already-`CLOSED`/`IMPLEMENTATION_COMPLETE` rows, not new backlog closures —
neither changes `ACTIVE_IMPLEMENTATION_LEDGER`.

| Row | Result | Commit |
|---|---|---|
| R1 `LIVE-TINY-CAPITAL-SMOKE-01` (A3B evidence-provenance repair) | CLOSED — `Start-LiveShadowSmoke.ps1` captured `$launcherLogBefore` as a bare count and never consulted it again, always selecting the newest `launch_*.json` under `smoke_logs\launcher\live-shadow` as this run's evidence; a stale prior LiveShadow log from a failed/no-op current invocation could be silently misattributed. `real_daemon_start_performed` was also inferred from `safety_guard.ok`, which only proves this invocation re-verified a reachable/correctly-postured daemon, not that it started one (the launcher can attach to an already-running daemon). Fixed: `Resolve-LiveShadowRunEvidence` derives evidence ONLY from the exact (post-invocation minus pre-invocation) new-log set difference (zero or >1 new logs fail closed to `not_observed`, never "newest file"); the canonical launcher (`Invoke-LiveShadowStartup`) now surfaces `daemon_started_by_this_invocation` (from `Start-DaemonIfNeeded`'s own deterministic stdout) as a field distinct from `daemon_reachable_and_verified` (the safety-guard fact). Manifest schema bumped to v3. 18/18 guard proofs, including 7 new hermetic fixture negative controls (LS-EV-01..07) proving stale-log rejection, exact-new-log selection, started-vs-attached distinction, wrong-mode rejection, and no secret leakage. **SUPERSEDED (`MQK-LIVESHADOW-R1B-FINAL`, 2026-09-07): this row's own "CLOSED" is incomplete/superseded — the set-difference resolver it describes proved only temporal novelty, not causal ownership; see the row's own §5 entry and the R1B table below.** | `625b2eb6` |
| R2 `AUTONOMOUS-DATA-BLOCKER-AUTO-RECOVERY-01` (CAS-concurrency repair) | CLOSED — the coordinator's automatic-recovery arm treated only `ManualInterventionRecoveryOutcome::Recovered(_)` as a successful recovery; `AlreadyApplied(updated)` fell through to the sticky `ManualInterventionRequired` re-projection alongside genuinely-still-blocked outcomes. But `mqk_db::transition_autonomous_daily_operation` returns `AlreadyApplied` ONLY when the durable row is already at exactly the requested `(state, state_version)` AND the event log proves that exact `manual_intervention_required -> preparing_data` transition already committed — i.e. a concurrent actor (an operator retry, or another coordinator tick) won the identical CAS this call attempted. Projecting `ManualInterventionRequired` in that case contradicted durable truth already at `preparing_data`. Fixed: the coordinator now treats `Recovered` and `AlreadyApplied` as equally truthful recovery outcomes; `StaleState` (a genuinely different concurrent transition) remains fail-closed, unchanged. Deterministic concurrent-CAS proof (no timing sleeps — both actors read the same fixture row, one applies the transition directly, the other goes through `dispatch_by_state` against the same stale version): POS4 proves the loser projects `PreparingData` from `AlreadyApplied` truth, exactly one transition event exists, `state_version` increments once, zero run_id/arm/halt side effects; NEG4 proves a genuinely different concurrent transition (`-> stopping`) still classifies `StaleState` and stays `ManualInterventionRequired`. Reverting the fix reproduces POS4's exact original failure (confirmed). 8/8 scenario tests plus 18/18 adjacent operator-retry regression, both against the disposable test DB. | `a775f4f9` |
...
- `MULTI-SYMBOL-CAPS-PREFLIGHT-WARNING-01` normalized
  `ACTIVE_IMPLEMENTATION -> CLOSED` — the `mscw01..05` preflight
  cap-warning test matrix (5/5) confirms the row's L0 truth-up.
- `DB-OUTBOX-SCHEMA-VERSION-01` normalized `ACTIVE_IMPLEMENTATION -> CLOSED`
  — `order_json_schema_version_tests` (13/13) confirms additive,
  backward-readable `schema_version` stamping on both outbox write paths.
- `GUI-OPERATOR-ACTION-409-BODY-SURFACE-01` normalized
  `ACTIVE_IMPLEMENTATION -> CLOSED` — `gui_ops_409_body_surface.test.ts`'s
...
- `CLI-DAEMON-CONTROL-PASSTHROUGH-01`, `BROKER-ALPACA-RATE-LIMIT-RETRY-AFTER-01`,
  ...
  `DISCORD-DATA-STALENESS-ALERT-01`, `DISCORD-DAILY-SUMMARY-PUSH-01` all
  normalized `ACTIVE_IMPLEMENTATION -> CLOSED` — each row's own L0 truth-up
  commit re-confirmed as an ancestor of HEAD, and each row's specific named
  test suite (15/15, 53/53, 11/11, 14/14, 8/8, 3/3 respectively) re-run
  green this session (the last against the disposable test DB).
- `CI-UNSAFE-PATTERNS-SYSTEMTIME-TESTFIXTURE-01` normalized
  `ACTIVE_IMPLEMENTATION -> CLOSED` — this row had no prior L0 truth-up (it
...
### MQK-LEDGER-FINAL-CLOSEOUT-01 — Final Canonical Ledger Synchronization (2026-09-07)

**This is a docs-only closeout syncing the ledger to a completed independent review, not a reconciliation or new implementation pass.** No production code, test code, scripts, or migrations were touched. No Paper/Live/smoke/soak/provider/broker activity occurred. Per this controller's own mechanical discipline, every commit hash the independent review cited was directly checked with `git merge-base --is-ancestor <commit> HEAD` before being recorded as accepted below — none is taken on the review's word alone.

**Eight rows independently accepted** (all cited commits confirmed ancestors of this HEAD):

1. `MARKET-DATA-PROVIDER-PROVENANCE-01-REPAIR-01` — `4bc78c70`. See the row's own entry (Lane A) for full acceptance detail.
2. `BKT-PROMOTION-ARTIFACT-AUTHORITY-01` — `08a292cd`.
3. `PROMOTION-STRESS-SUITE-AUTHORITY-01` — `8bed1b6c` (+ `7f8b0cdb` repair).
4. `PROMOTION-BACKTEST-EVIDENCE-SEAM-01` — `e56f94fb` (+ `7f8b0cdb` repair).
5. `PROMOTION-WALKFORWARD-GATE-WIRING-01` — `f8e9edf4` plus later real production E2E proof `37649200`.
6. `OFFICIAL-DUAL-MODE-LAUNCHER-01` — `e4f9eb92`; current integrated launcher truth accepted (not the superseded intermediate refresh-child architecture).
7. `PAPER-OPS-AUTOFRESH-LAUNCHER-INTEGRATION-01` — `1a9c4b8f`.
8. `PAPER-OPS-AUTOFRESH-LAUNCHER-INTEGRATION-01-REPAIR-01` — `cd0f1a00`.

All eight normalized `PENDING_INDEPENDENT_REVIEW -> CLOSED`. See each row's own entry (§5) for the full per-row acceptance record.

**`DEADMAN-LEASE-TTL-RECONCILE-01` normalized `RED_AUTHORIZATION_REQUIRED -> CLOSED`.** This row's own acceptance criterion permits closure by either (A) aligning the TTLs or (B) documenting why they intentionally differ plus defensive reconciliation/fencing with halt-path proof. Current repo implements (B) via `0f9606c6`/`104953e6` (both confirmed ancestors of HEAD) — run-identity-bound leases, same-run/different-run disposition rules, migration 0069 legacy reconciliation, RESTRICT (not CASCADE) lease FK, and negative controls cr07/cr08/cr10/cr10b. No runtime code changed by this pass — see the row's own entry (Lane D) for full detail.

**`INSTRUMENT-UNIVERSE-REFRESH-01` normalized `ACTIVE_IMPLEMENTATION -> DEFERRED_POST_LEDGER_AUDIT`.** This row has no finite In Scope/Acceptance Criteria/Exact CLOSED End State — it is an unscoped future architecture question, not a bounded patch. Per CLAUDE.md's no-fabricated-closure/no-invented-scope rules, this pass does not invent one. It is not CLOSED and not SUPERSEDED — the operator has ordered the current implementation ledger finished first, then a fresh aggressive post-ledger audit to scope any real work here.

**Final canonical bucket table — supersedes the bucket counts in Phase N above for current-truth purposes (Phase N's own text is retained unedited, for history):**
...
| `PENDING_INTEGRATION` | 0 |
| `RED_AUTHORIZATION_REQUIRED` | 0 |
| `CLOSED` | 54 |
| `IMPLEMENTATION_COMPLETE_OPERATOR_VALIDATION_DEFERRED` | 3 |
| `BLOCKED_BY_DEFERRED_OPERATOR_VALIDATION` | 4 |
| `DEFERRED_POST_LEDGER_AUDIT` | 5 |
| `DEFERRED_FUTURE_MAINTENANCE` | 7 |
| **TOTAL** | **73** |

**Mechanical check:** 54 + 3 + 4 + 5 + 7 = 73. `CLOSED` = Phase N's 45 + the 8 independently-accepted rows above + `DEADMAN-LEASE-TTL-RECONCILE-01` = 54. `DEFERRED_POST_LEDGER_AUDIT` = Phase N's 4 + `INSTRUMENT-UNIVERSE-REFRESH-01` = 5. Every other bucket is unchanged from Phase N. MATCHES.

**ACTIVE PRODUCTION IMPLEMENTATION LEDGER = ZERO.** This means no known current production implementation patch remains executable — it does **not** mean operational validation is complete. Paper/Live/smoke/soak/scheduler-activation/holdout-consumption proof remains deliberately deferred per standing operator instruction (§1 Paper-Soak Protection Rule, and the `MQK-LEDGER-BURN-WAVE-01` sequencing rule above). Future maintenance/feature rows (`DEFERRED_FUTURE_MAINTENANCE`, `DEFERRED_POST_LEDGER_AUDIT`) remain explicitly deferred rather than falsely closed.

**Side effects this phase:** docs-only; no code changed. Paper orders = 0. Live orders = 0. Real broker calls = 0. Paper validation = NOT RUN. Paper smoke = NOT RUN. Paper soak = NOT RUN. Scheduler activation = NOT RUN. Holdout consumption = 0. `smoke_logs/` untouched. No push to `origin`.

### MQK-LIVESHADOW-R1B-FINAL — LiveShadow Evidence-Provenance Invocation-Identity Repair (2026-09-07)

**This is a single-patch engineering repair plus its ledger sync, not a broad reconciliation.** It repairs one deterministic defect in `LIVE-TINY-CAPITAL-SMOKE-01`'s evidence-provenance tooling, found by independent review of the `MQK-LEDGER-BURN-CONTROLLER-04` R1 repair (`625b2eb6`) recorded CLOSED above.

**Defect found:** R1's `Resolve-LiveShadowRunEvidence` selected "this run's own log" as the exact (post-invocation minus pre-invocation) new-file set difference under the launcher's log directory. That proves only TEMPORAL novelty (a log new since a pre-run snapshot) — not CAUSAL ownership (that the log was written by THIS invocation's own child process, not some concurrent foreign invocation). Deterministic failure case: this wrapper's own child produces no evidence, while a concurrent foreign LiveShadow invocation writes one otherwise-valid new `launch_*.json` log; R1's resolver accepted that foreign log as this run's own evidence. Reproduced via a pre-fix RED run: a fixture harness invoked the unmodified R1 `Resolve-LiveShadowRunEvidence` with an empty pre-existing-log snapshot and one foreign fixture log — the resolver returned that foreign log as `source_log` with its `observed_true`/`observed_true` daemon fields, confirming the exact failure mode the mission described.

**Repair (`71ac7d21`):** `Start-LiveShadowSmoke.ps1` now generates an opaque GUID before delegating and passes it to `Start-MiniQuantDesk.ps1` via the new non-secret `-InvocationId` parameter. `Invoke-LiveShadowStartup` writes that exact value into its own launch-log entry's `invocation_id` field, unconditionally (CheckOnly and full-run both). `Resolve-LiveShadowRunEvidence` no longer computes a pre/post set difference at all — it scans every `launch_*.json` under the launcher log directory and accepts ONLY a log whose `invocation_id` field exactly equals this invocation's own GUID; zero exact matches or more than one exact match both fail closed to `not_observed` with an explicit reason. Foreign invocations' logs, file timestamps, and "newest file"/new-file-count heuristics are never consulted. Manifest schema bumped to v4, with the invocation GUID recorded in the manifest itself for audit.

**Test proof:** `tests\script_guards\test_live_shadow_smoke.ps1` — 21/21 guard proofs, including `LS-EV-01..05` (updated to the ID-based API, same guarantees as before: foreign/stale logs never consumed, exact-match log selected among distractors, started-vs-attached distinction, wrong-mode rejection) and three new negative controls: `LS-EV-08` (FOREIGN-ONLY — expected id A, no log claims A, a foreign log claims id B — stays `not_observed`; this is the exact pre-fix RED scenario, now GREEN), `LS-EV-09` (foreign B + exact A both present — only A is authoritative), `LS-EV-10` (two logs both claim exact id A — fails closed as ambiguous). `scripts\windows\tests\test_official_dual_mode_launcher.ps1` re-run against the changed canonical launcher — 0 violations (the additive `-InvocationId` parameter does not touch Paper/Live/broker/order/provider dispatch).

**Ledger update:** `LIVE-TINY-CAPITAL-SMOKE-01` moved `IMPLEMENTATION_COMPLETE_OPERATOR_VALIDATION_DEFERRED -> PENDING_INDEPENDENT_REVIEW` (see its own §5 entry for full detail). `625b2eb6` (R1) is recorded incomplete/superseded, not CLOSED-and-forgotten — see the R1 table row above. `DEADMAN-LEASE-TTL-RECONCILE-01` and `INSTRUMENT-UNIVERSE-REFRESH-01` are unchanged by this pass (remain `CLOSED` and `DEFERRED_POST_LEDGER_AUDIT` respectively, per `MQK-LEDGER-FINAL-CLOSEOUT-01` above).

**Final canonical bucket table — supersedes the `MQK-LEDGER-FINAL-CLOSEOUT-01` table above for current-truth purposes (that table's own text is retained unedited, for history):**
...
| `RED_AUTHORIZATION_REQUIRED` | 0 |
| `CLOSED` | 54 |
| `IMPLEMENTATION_COMPLETE_OPERATOR_VALIDATION_DEFERRED` | 2 |
| `BLOCKED_BY_DEFERRED_OPERATOR_VALIDATION` | 4 |
| `DEFERRED_POST_LEDGER_AUDIT` | 5 |
| `DEFERRED_FUTURE_MAINTENANCE` | 7 |
| **TOTAL** | **73** |

**Mechanical check:** 0 + 1 + 0 + 0 + 54 + 2 + 4 + 5 + 7 = **73**. `PENDING_INDEPENDENT_REVIEW` = 0 (per `MQK-LEDGER-FINAL-CLOSEOUT-01`) + `LIVE-TINY-CAPITAL-SMOKE-01` (moved in) = 1. The sole row in that bucket is `LIVE-TINY-CAPITAL-SMOKE-01`. `IMPLEMENTATION_COMPLETE_OPERATOR_VALIDATION_DEFERRED` = 3 (per `MQK-LEDGER-FINAL-CLOSEOUT-01`) − `LIVE-TINY-CAPITAL-SMOKE-01` (moved out) = 2 (`PAPER-AUTOMATIC-PREOPEN-SCHEDULER-01` and `-REPAIR-01` remain). `CLOSED` (54), `BLOCKED_BY_DEFERRED_OPERATOR_VALIDATION` (4), `DEFERRED_POST_LEDGER_AUDIT` (5), `DEFERRED_FUTURE_MAINTENANCE` (7), `ACTIVE_IMPLEMENTATION`/`PENDING_INTEGRATION`/`RED_AUTHORIZATION_REQUIRED` (0 each) are unchanged from `MQK-LEDGER-FINAL-CLOSEOUT-01`. TOTAL unchanged at 73 (a status move, not a row added or removed). MATCHES.

**Side effects this phase:** docs (this file) + `scripts/windows/Start-LiveShadowSmoke.ps1` + `scripts/windows/Start-MiniQuantDesk.ps1` + `tests/script_guards/test_live_shadow_smoke.ps1` changed; no other production code, migrations, or Paper/Live/runtime/broker/order/provider behavior touched. Paper orders = 0. Live orders = 0. Real broker calls = 0. Real LiveShadow smoke = NOT RUN. Paper validation = NOT RUN. Holdout consumption = 0. `smoke_logs/` untouched. No push to `origin`.
...
**Ledger Truth-Up (`POST-WAVE06-LEDGER-BURN-01` L0, 2026-09-06):** Reclassified `ALREADY_CLOSED_LEDGER_STALE` — current HEAD `84dcd14c` already contains this invariant via ancestor commit `4d7aafca` ("merge: integrate accepted paper soak repair chain"), verified `git merge-base --is-ancestor`. This is a documentation-drift correction only; not promoted to literal `CLOSED` pending a fresh harness pass per `audit_repo_truth_rules.md`. Full citation: `MQK-POST-WAVE06-LEDGER-RECONCILIATION-REPAIR.zip` row reconciliation.

**Current Source Truth (superseded by the Ledger Truth-Up above — retained for history):** Implemented in isolated worktree `C:\Users\Zacha\Desktop\MiniQuantDeskV4-data`, branch `fix-market-data-provider-provenance`, base `54082a448c84b6429713a429bfb9403da8822131` (`origin/main`). Not merged into the primary worktree/branch as of this writing.
...
**Status:** CLOSED — INDEPENDENTLY ACCEPTED (`MQK-LEDGER-FINAL-CLOSEOUT-01`, 2026-09-07) — independent review accepted this row's implementation (ancestor commit `4bc78c70`, confirmed `git merge-base --is-ancestor` against this HEAD) as current production truth: raw provider symbol is the exact requested provider symbol; registry provider operations are scoped by enabled+asset_class=equity+provider+authorized timeframe; wrong provider/timeframe fails closed; AAPL automatic authorization is alpaca+5m only; provider provenance is persisted truthfully; automatic Windows refresh derives provider from the registry rather than pinning stale TwelveData truth. See §0 Final Closeout for the full acceptance record.
**Priority:** P0
**Paper Impact:** RED
**Subsystem:** mqk-cli market-data ingest / instrument registry / Windows premarket+intraday scripts

**Ledger Truth-Up (`POST-WAVE06-LEDGER-BURN-01` L0, 2026-09-06):** Reclassified `ALREADY_CLOSED_LEDGER_STALE` — current HEAD `84dcd14c` already contains this invariant via ancestor commit `4d7aafca`; `config/instruments/equities.json`'s AAPL entry independently confirmed to match the described end-state. Documentation-drift correction only; not promoted to literal `CLOSED` pending a fresh harness pass per `audit_repo_truth_rules.md`.

**Current Source Truth (superseded by the Ledger Truth-Up above — retained for history):** Implemented in isolated worktree `C:\Users\Zacha\Desktop\MiniQuantDeskV4-data`, branch `fix-market-data-provider-provenance`, on top of `dae446b337b77245417a4cc982ff7fa22736b098`. Not merged.
...
**Ledger Truth-Up (`POST-WAVE06-LEDGER-BURN-01` L0, 2026-09-06):** Reclassified `ALREADY_CLOSED_LEDGER_STALE` — current HEAD `84dcd14c` already contains this invariant via ancestor commits `e109c9db` ("fix: allow safe operator retry after legacy freshness refusal") and `54daa588` ("fix: distinguish prestart observation from retry-unsafe activity"), both verified `git merge-base --is-ancestor`. These are two undocumented follow-up commits on the same route this ledger row never recorded — a ledger completeness gap, not a functional defect. Documentation-drift correction only; not promoted to literal `CLOSED` pending a fresh harness pass per `audit_repo_truth_rules.md`.
**Priority:** P0
...
**Ledger Truth-Up (`POST-WAVE06-LEDGER-BURN-01` L0, 2026-09-06):** Reclassified `ALREADY_CLOSED_LEDGER_STALE` — current HEAD `84dcd14c` already contains this invariant, along with both REPAIR patches below, merged as one lineage via ancestor commit `4d7aafca`, verified `git merge-base --is-ancestor`. Documentation-drift correction only; not promoted to literal `CLOSED` pending a fresh harness pass per `audit_repo_truth_rules.md`.
...
**Ledger Truth-Up (`POST-WAVE06-LEDGER-BURN-01` L0, 2026-09-06):** Reclassified `ALREADY_CLOSED_LEDGER_STALE` — current HEAD `84dcd14c` already contains this invariant via ancestor commit `fde6e227`, confirmed ancestor of HEAD via `git merge-base --is-ancestor`. Documentation-drift correction only; not promoted to literal `CLOSED` pending a fresh harness pass per `audit_repo_truth_rules.md`.
...
**Ledger Truth-Up (`POST-WAVE06-LEDGER-BURN-01` L0, 2026-09-06):** Reclassified `ALREADY_CLOSED_LEDGER_STALE` — current HEAD `84dcd14c` already contains this invariant (same `4d7aafca` merged lineage as the parent patch and REPAIR-01); the `generation` field is independently confirmed present at `required_market_data_autofresh.rs:1538` (fully-formed race-fence doc comment + field, module has since grown but the mechanism is unchanged). Documentation-drift correction only; not promoted to literal `CLOSED` pending a fresh harness pass per `audit_repo_truth_rules.md`.
...
**Ledger Truth-Up (`POST-WAVE06-LEDGER-BURN-01` L0, 2026-09-06):** Reclassified `ALREADY_CLOSED_LEDGER_STALE` — current HEAD `84dcd14c` already contains this invariant (same `4d7aafca` merged lineage); the described deterministic-time helper functions are independently confirmed present verbatim in the test file. The DB-backed suite itself was not re-run as part of this L0 pass (out of scope for a docs-only rebaseline). Documentation-drift correction only; not promoted to literal `CLOSED` pending a fresh harness pass per `audit_repo_truth_rules.md`.
...
**Status:** DEFERRED_POST_LEDGER_AUDIT (`MQK-LEDGER-FINAL-CLOSEOUT-01`, 2026-09-07) — not an actionable current implementation patch. This entry has no finite In Scope, no complete Acceptance Criteria, and no Exact CLOSED End State (unlike every other row in this inventory); it concerns a bulk review of seeded instruments beyond the currently-approved AAPL path, undecided provider/timeframe assignments for those future symbols, and an unresolved longer-term architecture question about whether one registry `provider` field should eventually become separate historical/intraday/streaming/execution provider concepts. Per operator instruction, the current implementation ledger is finished first; a future aggressive post-ledger audit will scope any real patch here. Not marked CLOSED and not marked SUPERSEDED — no work performed. Was `OPEN` · **Priority:** P3 · **Paper Impact:** GREEN · **Subsystem:** Instrument registry
...
**Status:** CLOSED — reviewed, no deterministic defect found (`POST-WAVE06-LEDGER-BURN-01` W1-03, 2026-09-06) · **Priority:** P2 · **Paper Impact:** GREEN (review produces no code change by itself) · **Subsystem:** mqk-portfolio
**Findings (W1-03 targeted review, current HEAD `84dcd14c`):** File is now 3,947 lines (grew from 3,680 since this row was authored), of which ~2,160 are production logic and ~1,780 are tests (100 `#[test]` functions). Targeted read covered: the closed reason-code vocabulary and its exhaustive `code()`/`parse_code()` round-trip (fails closed on any unrecognized persisted string); `evaluate_fingerprint_gate` and its three call sites (legacy, v2, and config-identity fingerprint pairs) — each requires both values present, exactly-64-lowercase-hex, byte-equal, and the caller's claimed-match boolean coherent with the derived equality, fails closed on any of missing/malformed/incoherent/mismatch; `evaluate_evidence_gate`'s full ordered gate chain (identity → promotion state/effective/expired → evidence lineage → review state → both fingerprint gates → config-identity gate → plugin/timeframe/data-ready → score presence); `resolve_symbol_group`'s full tie-break cascade (exact-decimal-string max score → positive-rank tie-break → watchlist-assignment tie-break → canonical-strategy-id-ascending final tie-break, each step explicitly refusing/not-selecting losers with a distinct typed reason, never silently dropping a candidate); `compute_dynamic_selection_plan`'s caller-contract boundary checks (empty/over-limit eligible set, blank/duplicate eligible symbol, over-limit candidate slice, per-symbol universe cap independent of the total cap, candidate-outside-eligible-set) — every violation fails the whole plan closed with a distinct `TRUTH_STATE_*`, never silently drops or truncates; `candidate_result`'s pre-gate-diagnosis refinement (a caller-supplied `exact_reason` is only ever trusted after `gate_reason_accepts_pregate_refinement` proves it agrees with the gate's own finding — an inconsistent claim maps to its own distinct `PreGateReasonInconsistent` reason, never silently accepted); and `canonical_plan_identity_material`'s serialization primitives (`push_len_prefixed` et al.) — correctly length-prefixed, ruling out delimiter-injection ambiguity in the caller-hashed plan-identity material. No `.unwrap()`/`.expect()` found outside logically-guarded invariants already proven true by the preceding control flow (e.g. `.expect("gated Some")` on a field the evidence gate already required `Some`). Every code comment referencing "Defect 1"/"Defect 2"/"Defect F"/R1-R8/IR7-IR8 corresponds to a control-flow branch that is actually present and exercised, not aspirational documentation. **Classification: no deterministic defect found. No follow-up patch spawned.**
**Original entry (retained for history):** `mqk-portfolio/src/dynamic_selection.rs` is 3,680 lines and was the one file in the portfolio/P&L cluster not fully read during this audit pass (out of that pass's read budget). Everything else in the fill/P&L accounting path was proven complete.
**Problem (original, resolved by the findings above):** Unknown whether this file contains any gap; classified UNKNOWN / REQUIRES EXTERNAL PROOF pending a dedicated read.
**Why This Matters:** It's the largest unreviewed file touching portfolio/selection accounting.
**Dependencies:** NONE. **Unlocks:** N/A — no follow-up patch spawned.
...
**Acceptance Criteria:** 1) File fully read. 2) Classification recorded in this ledger with citations. — **MET.**
**Exact CLOSED End State:** CLOSED when the classification is recorded, whether or not it spawns a follow-up patch. — **MET, see Findings above.**
**Acceptance History:** Read: DONE / Classification: DONE (no defect) / Follow-up: N/A / Ledger recorded: DONE.
...
**Status:** CLOSED — INDEPENDENTLY ACCEPTED (`MQK-LEDGER-FINAL-CLOSEOUT-01`, 2026-09-07, implementation `08a292cd`, confirmed ancestor of HEAD). Was `CLOSED_LOCAL — PENDING INDEPENDENT REVIEW` (2026-08-21, `RESEARCH-BACKTEST-V1-CLOSURE-CONTROLLER-01`) · **Priority:** P1 · **Paper Impact:** GREEN (backtest artifact writer only; no execution/portfolio/broker path) · **Subsystem:** mqk-artifacts / mqk-backtest / mqk-promotion
**Ledger Truth-Up (`POST-WAVE06-LEDGER-BURN-01` L0, 2026-09-06):** Reclassified `ALREADY_CLOSED_LEDGER_STALE` — current HEAD `84dcd14c` already contains this invariant via ancestor commit `08a292cd`, verified `git merge-base --is-ancestor`; `backtest_report_artifact.rs` is confirmed present at HEAD. Consistency pass only, no code change — the accepted `fbddeb3d..HEAD` chain already carries this commit.
...
**Status:** CLOSED — INDEPENDENTLY ACCEPTED (`MQK-LEDGER-FINAL-CLOSEOUT-01`, 2026-09-07, implementation `8bed1b6c` plus the `7f8b0cdb` protocol-identity repair, both confirmed ancestors of HEAD). Was `CLOSED_LOCAL — PENDING INDEPENDENT REVIEW` (2026-08-21, `RESEARCH-BACKTEST-V1-CLOSURE-CONTROLLER-01`) · **Priority:** P1 · **Paper Impact:** GREEN (research/promotion evidence only) · **Subsystem:** mqk-backtest / mqk-artifacts
**Ledger Truth-Up (`POST-WAVE06-LEDGER-BURN-01` L0, 2026-09-06):** Reclassified `ALREADY_CLOSED_LEDGER_STALE` — current HEAD `84dcd14c` already contains this invariant via ancestor commits `8bed1b6c` and follow-on `7f8b0cdb`, both verified `git merge-base --is-ancestor`, both inside the accepted range. Consistency pass only, no code change.
...
**Status:** CLOSED — INDEPENDENTLY ACCEPTED (`MQK-LEDGER-FINAL-CLOSEOUT-01`, 2026-09-07, implementation `e56f94fb` plus the `7f8b0cdb` protocol-identity repair, both confirmed ancestors of HEAD). Was `CLOSED_LOCAL — PENDING INDEPENDENT REVIEW` (2026-08-21, `RESEARCH-BACKTEST-V1-CLOSURE-CONTROLLER-01`) · **Priority:** P1 · **Paper Impact:** GREEN (research/promotion evidence only; no execution/portfolio/broker path) · **Subsystem:** mqk-promotion / mqk-daemon
**Ledger Truth-Up (`POST-WAVE06-LEDGER-BURN-01` L0, 2026-09-06):** Reclassified `ALREADY_CLOSED_LEDGER_STALE` — current HEAD `84dcd14c` already contains this invariant via ancestor commit `e56f94fb`, a confirmed-pushed ancestor per this ledger's own text and verified `git merge-base --is-ancestor`. Consistency pass only, no code change.
...
**Status:** CLOSED (`MQK-LEDGER-BURN-CONTROLLER-02` W2-A, 2026-09-06, commit `889db5f0`) · **Priority:** P2 · **Paper Impact:** GREEN (adds a test only) · **Subsystem:** mqk-daemon dynamic selection
**W2-A finding:** `build_dynamic_paper_enforced_dispatch_authority` and the evidence-writer helpers are `pub(crate)` -- genuinely unreachable from an external `tests/*.rs` binary -- and that exact chain (host pool -> dispatch authority -> the real `advance_run_to_active` start sequencer, which performs the real evidence write) is already proven extensively in-crate by `state/lifecycle.rs`'s own `#[cfg(test)]` suite (30+ tests driving `drive_production_start_effects_with_dispatch_authority_for_test`). The one segment with zero coverage anywhere in the repo (confirmed by grepping every test file for the route names) was the read side: `GET /api/v1/dynamic-selection/plans` and `/plans/:plan_id` had never been exercised by any test, embedded or external. New `scenario_dynamic_selection_end_to_end_paper_dispatch_01.rs` closes exactly that gap: real pure selector (`compute_dynamic_selection_plan`) with controlled evidence -> real host-pool constructor -> real DB-layer writer (`insert_dynamic_selection_plan`) -> real Axum router over real HTTP, with `plan_id` derived via the same `canonical_plan_identity_material` + namespace seed the production dispatch-authority module uses so the read-side validator genuinely reports `valid`. 3/3 tests pass against the local disposable test DB (round-trip, unknown-plan_id negative control, DB-tamper negative control proving the validator is wired into the routes, not bypassed).
...
**Status:** CLOSED — catalog recorded, coverage confirmed sufficient (`POST-WAVE06-LEDGER-BURN-01` W1-06, 2026-09-06) · **Priority:** P3 · **Paper Impact:** GREEN (review only) · **Subsystem:** mqk-daemon dynamic selection
**Coverage catalog (W1-06, current HEAD `84dcd14c`):** Both files now have real embedded `#[cfg(test)]` modules (contradicting the original "not yet confirmed read" framing) — `dynamic_selection_dispatch_authority.rs` (844 lines) has 16 tests covering `build_dynamic_paper_enforced_dispatch_authority`'s complete error surface (empty plan, no-selected-candidates, unknown DB timeframe label, missing/extra host-pool entry, duplicate binding, invalid/mismatched/matching plan `run_id`) plus `derive_dynamic_selection_plan_id`'s determinism, sensitivity-to-result-affecting-change, and independence from host-pool/construction order, plus `Legacy`/`DynamicPaperEnforced` discrimination and a `Debug`-never-panics/hides-internals proof. `dynamic_selection_host_pool.rs` (393 lines) has 10 tests covering `DynamicSelectionHostPool::build`'s complete error surface (duplicate key refused, unknown strategy id refused, wrong timeframe refused) plus its key invariants (two symbols get independent isolated hosts, same strategy independently instantiated per symbol, a differing timeframe on the same symbol+strategy is correctly NOT treated as a duplicate key, build never partially populates the pool on a downstream failure, input order never changes the resulting pool keys, and a structural proof the build touches no outbox/dispatch side effect). **Classification: embedded unit coverage is sufficient for both files' own error/invariant surface.** This does not make `DYNAMIC-SELECTION-E2E-SCENARIO-TEST-01` redundant — that row tests a categorically different thing (the live integration chain: plan build → host-pool selection → selected-host dispatch → evidence write → evidence-route read as one continuous path), which these unit-level tests do not and cannot exercise on their own. `DYNAMIC-SELECTION-E2E-SCENARIO-TEST-01` remains open in W2, now confirmed non-duplicative.
**Original entry (retained for history):** `dynamic_selection_dispatch_authority.rs` (837 lines) and `dynamic_selection_host_pool.rs` (393 lines) have no dedicated `scenario_*` file by name; coverage may live in embedded `#[cfg(test)]` modules not yet confirmed read.
...
**Required Validation:** None (read-only).
**Acceptance Criteria:** Coverage catalog recorded in this ledger entry. — **MET, see catalog above.**
**Exact CLOSED End State:** CLOSED when the catalog is recorded, regardless of outcome. — **MET.**
**Acceptance History:** Read: DONE / Catalog: DONE (sufficient) / Ledger recorded: DONE.
...
**Status:** CLOSED (`MQK-LEDGER-BURN-CONTROLLER-04` burn phase, 2026-09-07) — this session's own fresh harness pass confirms the row's L0 truth-up: `878ba8ef` is an ancestor of HEAD, `mqk-cli/src/commands/daemon.rs` implements `daemon_status`/`daemon_arm`/`daemon_disarm`/`daemon_halt`/`daemon_clear_halted_run` against the real daemon control-plane routes, and `cargo test -p mqk-cli --bin mqk-cli commands::daemon` is 15/15 green (httpmock-based: 409-blocker propagation, unreachable/malformed-body/unauthorized truthful-failure controls, confirm-before-mutate "-yes"-gated no-HTTP-request controls, missing-token refusal, positive arm/disarm/halt/clear-halted-run paths). No code changed by this session. · **Priority:** P1 · **Paper Impact:** GREEN (pure HTTP passthrough, zero new daemon logic) · **Subsystem:** mqk-cli
**Ledger Truth-Up (`POST-WAVE06-LEDGER-BURN-01` L0, 2026-09-06):** Reclassified `ALREADY_CLOSED_LEDGER_STALE` — current HEAD `84dcd14c` already contains this invariant via ancestor commit `878ba8ef`, verified `git merge-base --is-ancestor`; a full httpmock-based test suite including confirm-before-mutate controls is present. Documentation-drift correction only; not promoted to literal `CLOSED` pending a fresh harness pass per `audit_repo_truth_rules.md`.
**Current Source Truth (superseded by the Ledger Truth-Up above — retained for history):** `mqk-cli/src/main.rs:44-88` has no `Daemon`/`Control` command. `RunCmd` (`main.rs:846-972`) operates on the generic `mqk-db` `runs` table directly, not via the live daemon's HTTP control-plane routes (`/v1/run/start`, `/v1/run/stop`, `/v1/run/halt`, `/v1/integrity/arm`, `/v1/integrity/disarm`, `/api/v1/ops/action`, `routes.rs:781-791`). An operator cannot arm/halt/clear the actual running autonomous daemon from the CLI — only via the HTTP API (GUI or curl).
...
**Status:** CLOSED — help text and doc comments disambiguated (`POST-WAVE06-LEDGER-BURN-01` W1-12, 2026-09-06) · **Priority:** P3 · **Paper Impact:** GREEN · **Subsystem:** mqk-cli
**Resolution (W1-12, sequenced after W1-10's `LIVE-CLI-ARM-RECONCILE-01` determination as this row required):** `CLI-DAEMON-CONTROL-PASSTHROUGH-01`'s `mqk daemon <cmd>` passthrough already exists (closed in this wave's L0), so the correct-alternative pointer this row needed is real, not aspirational. Added: (1) a top-level doc comment on the `Run { }` clap variant (`main.rs`) stating plainly that every `RunCmd` subcommand touches only the generic `runs` DB table's bookkeeping status, is NOT the live daemon control plane, and pointing to `mqk daemon <cmd>` for live-process control; (2) a specific doc comment on `RunCmd::Halt` — the exact incident-confusion scenario this row named ("an operator could mistake `mqk run halt` for a live-daemon-halt action") — stating it does not halt a live daemon process and pointing to `mqk daemon halt` (kill-switch) for that. `RunCmd::Arm` was already disambiguated in W1-10. No behavior change to `RunCmd` (doc-only, per this row's own out-of-scope boundary).
**Regression:** `cargo build -p mqk-cli` clean.
**Original entry (retained for history):** `mqk-cli/src/main.rs:846-972` (`RunCmd::Start/Arm/Stop/Halt/...`) operates directly on the `mqk-db` `runs` table, which is a different code path from the live daemon's HTTP control-plane routes.
**Problem (original, resolved above):** An operator could mistake `mqk run halt` for a live-daemon-halt action during an incident, when it is not.
**Dependencies:** Best done alongside or after `CLI-DAEMON-CONTROL-PASSTHROUGH-01` so the doc can point to the correct alternative. — **Satisfied.**
...
**Required Validation:** `cargo build -p mqk-cli`. — **DONE.**
**Acceptance Criteria:** Help text and doc comments are unambiguous about scope. — **MET.**
**Exact CLOSED End State:** CLOSED when committed. — **MET.**
**Acceptance History:** Doc comments added: DONE / Regression: DONE (clean build).

#### CI-TESTKIT-FEATURE-GUARD-VERIFY-01 — Verify testkit feature never ships in release builds

**Status:** CLOSED — repaired twice since original closure; see A1 finding below for the current, correct state (`MQK-LEDGER-BURN-CONTROLLER-03` A1, 2026-09-06, commit `957908b7`) · **Priority:** P3 · **Paper Impact:** GREEN · **Subsystem:** CI / mqk-cli
**A1 finding (`MQK-LEDGER-BURN-CONTROLLER-03`, 2026-09-06) — REPAIR-02, closing the REPAIR-01 gap independent review found:** REPAIR-01 (`14ec4096`, below) rewrote the guard as a Cargo/TOML-structural parser but its dependency-table checker still compared only *literal* requested-feature strings against `"testkit"` — a dependency edge like `b = { features = ["full"] }` where `b`'s own `full = ["testkit"]` silently bypassed it, since the guard never expanded a cross-crate feature name into the *target* crate's own `[features]` table. Rewrote it as a true workspace-wide feature graph (`feature_reaches_testkit`/`dependency_activation_reaches_testkit`): local alias chains, cross-crate `<crate>/<feature>`/`<crate>?/<feature>`/`dep:<crate>` edges (resolved through `package="..."` renames and `workspace=true` inheritance), and a dependency's own default-feature propagation when an edge activates it — with a visited-set cycle guard. Added the required `TKG-R2-01..08` two-crate mutation-negative cases (all invoking the real guard against isolated fixture repos); all 8 plus the prior 11 `TKG-*` cases (including a clean pass against the real repo) pass. This is now the authoritative closure; REPAIR-01's `14ec4096` closure (recorded in the independent-review table above and the Resolution paragraph below) is superseded, not reopened as a separate defect.
**Resolution (W1-07, original closure — REPAIR-01/REPAIR-02 above are the current truth):** Verified first: `testkit` is `mqk-cli`'s own optional feature (`Cargo.toml`), propagating to `mqk-execution/testkit` and `mqk-runtime/testkit`; neither is in any crate's `default = [...]` list workspace-wide (none exists at all, confirmed by grep), and a workspace-wide scan of every crate's production `[dependencies]` section (excluding `[dev-dependencies]`) found zero lines enabling any dependency's `testkit` feature — the existing `check_disposable_db_not_in_production.sh` already proved this narrowly for `mqk-db`; this row generalizes it. No equivalent CI-wired guard existed, so a new one was added: `scripts/guards/check_testkit_feature_not_default.sh` (workspace-wide default-feature-list check + production-dependency check) plus `tests/script_guards/test_testkit_feature_not_default.sh` (5-case mutation-negative proof: TKG01 clean baseline passes, TKG02 catches `testkit` added to a `default` list, TKG03 catches `testkit` moved into production `[dependencies]`, TKG04 catches both simultaneously without short-circuiting, TKG05 proves the correct dev-dependency-only pattern is never false-flagged). Both wired into `.github/workflows/ci.yml`'s `guards` job. No change to any stub/testkit-gated code itself.
**New finding surfaced while validating (NOT fixed in this patch, per W1 discipline — see `CI-UNSAFE-PATTERNS-SYSTEMTIME-TESTFIXTURE-01` immediately below):** running the full existing `guards` job locally (`check_unsafe_patterns.sh`) to confirm no regression surfaced a pre-existing, unrelated failure: `core-rs/crates/mqk-cli/src/commands/research_replay.rs:953` uses `SystemTime::now()` inside its own `#[cfg(test)] mod tests` helper (`unique_dir`, a temp-directory-name generator), which the guard's grep-based check does not distinguish from production code. This predates this session (confirmed: this session never touched `research_replay.rs`) and is independent of the testkit-feature invariant this row tracks.
**Original entry (retained for history):** `mqk-cli/src/commands/run.rs:405,464,485` and `main.rs:27` document real, intentional test-only stub wiring (`NullBroker`, always-pass gate stub) gated by `#[cfg(feature = "testkit")]`. No existing CI job was confirmed in this audit pass to explicitly assert `testkit` is absent from release builds.
**Problem (original, resolved by the resolution above):** If the feature gate were ever misconfigured, a stub broker could theoretically ship in a production build. Currently believed safe, not confirmed via an explicit guard.
...
**Required Validation:** Run the relevant guard script locally if one exists; otherwise add one and run it. — **DONE, see Resolution above.**
**Acceptance Criteria:** A CI guard fails if `testkit` feature is enabled in a release-profile build. — **MET.**
**Exact CLOSED End State:** CLOSED when the guard exists (or is confirmed to already exist) and demonstrably fails on a deliberately-misconfigured build. — **MET.**
**Acceptance History:** Guard: DONE (new) / Mutation-negative proof: DONE (5/5 pass) / CI wiring: DONE / Regression check: DONE (existing guards suite re-run, one unrelated pre-existing finding surfaced and tracked separately).

#### CI-UNSAFE-PATTERNS-SYSTEMTIME-TESTFIXTURE-01 — `SystemTime::now()` inside a test-only helper flagged by the unsafe-patterns guard

**Status:** CLOSED (`MQK-LEDGER-BURN-CONTROLLER-04` burn phase, 2026-09-07) — already implemented at this session's HEAD: `research_replay.rs::unique_dir` (commit `e5e3d44c`, "test: remove wall-clock uniqueness from replay fixture") took option (b) from this row's own in-scope choices — it now derives uniqueness from `std::process::id()` + a per-process `AtomicU64` counter, with a comment explicitly citing this row by name, never `SystemTime::now()`. Zero `SystemTime::now` occurrences remain anywhere in `research_replay.rs`, and a repo-wide `grep -rn "SystemTime::now" core-rs/crates/*/src/` (excluding allow-listed/comment lines) finds none. A fresh run of `bash scripts/guards/check_unsafe_patterns.sh` this session exits 0 with `ALL GUARDS PASSED` — the row's own literal acceptance criterion. No code changed by this session. · **Priority:** P3 · **Paper Impact:** GREEN (test-only code; no production/trading path) · **Subsystem:** mqk-cli / CI guards
**Current Source Truth:** `core-rs/crates/mqk-cli/src/commands/research_replay.rs:953`, inside `#[cfg(test)] mod tests`, `fn unique_dir`: `SystemTime::now().duration_since(UNIX_EPOCH)...` generates a unique temp-directory suffix for test fixtures. `scripts/guards/check_unsafe_patterns.sh`'s `[S] SystemTime::now in production src/` check is a plain grep over `src/` that does not exclude `#[cfg(test)]` modules, so it flags this test-only helper as if it were a production determinism violation, currently making the `guards` CI job fail (confirmed via local run, 2026-09-06, HEAD includes all commits through this wave).
**Problem:** Either (a) the guard is over-broad (should exclude `#[cfg(test)]`-gated code, since test-fixture uniqueness has no bearing on production determinism), or (b) the test helper should use a deterministic/non-wall-clock unique-id mechanism (e.g. an atomic counter or a UUID) as a matter of house style — a real decision either way, not yet made.
**Why This Matters:** The `guards` CI job is currently red on this branch because of this single finding, independent of anything in `MQK-LEDGER-BURN-WAVE-01`.
**Dependencies:** NONE.
**In Scope (future patch):** Either narrow the guard's `[S]` check to exclude `#[cfg(test)]` blocks, or replace `SystemTime::now()` in `unique_dir` with a deterministic unique-id source, whichever an operator decides is the correct fix. **Out of Scope for this discovery entry:** Implementing either fix now.
**Likely Files:** `scripts/guards/check_unsafe_patterns.sh`, `core-rs/crates/mqk-cli/src/commands/research_replay.rs`.
**Acceptance Criteria:** `bash scripts/guards/check_unsafe_patterns.sh` exits 0 against current HEAD plus whichever fix is chosen.
**Exact CLOSED End State:** CLOSED when the guard's `guards` CI job passes clean again, with the chosen fix's rationale recorded here.

#### CLI-RUN-STUB-TRACKING-01 — Convert untracked "replace stubs before LIVE" comment into a tracked patch

**Status:** CLOSED — obsolete reminder replaced with the real current fact (`POST-WAVE06-LEDGER-BURN-01` W1-11, 2026-09-06) · **Priority:** P3 · **Paper Impact:** GREEN · **Subsystem:** mqk-cli
**Resolution (W1-11, using `LIVE-CLI-ARM-RECONCILE-01`'s W1-10 determination):** `run_execute` (the function this comment sits in) is entirely `#[cfg(feature = "testkit")]`-gated, and `CI-TESTKIT-FEATURE-GUARD-VERIFY-01` (this same wave, W1-07) both verified and CI-guards that `testkit` is neither a default feature anywhere in the workspace nor enabled by any production `[dependencies]` section — this function structurally cannot compile into a release/LIVE-deployed binary at all. The old "replace stubs before LIVE deployment" comment implied a future action item that will never actually need doing (there is no scenario where this code ships live to replace stubs in). Per this row's own in-scope options, the bare reminder was removed and replaced with the real current fact: a doc comment naming both this row and `LIVE-CLI-ARM-RECONCILE-01`, explaining the `testkit`-gate + CI-guard mechanism that makes the stubs unreachable by construction, not by developer discipline. No stub behavior changed (doc-only, per this row's own out-of-scope boundary).
**Regression:** `cargo check -p mqk-cli --features testkit` clean (proves the doc-only change didn't affect the `testkit`-gated compilation path either).
**Original entry (retained for history):** `mqk-cli/src/commands/run.rs:485` contains a standing comment "Replace stubs with real implementations before LIVE deployment" with no tracking issue/patch ID.
**Problem (original, resolved above):** An untracked reminder comment is easy to miss before an eventual live cutover decision.
**Dependencies:** Related to `LIVE-CLI-ARM-RECONCILE-01` (Lane C) which investigates whether this CLI path is even the live-relevant one. — **Satisfied: W1-10 closed first, as sequenced.**
...
**Required Validation:** `cargo fmt --check`. — **DONE (`cargo check` clean; no logic touched).**
**Acceptance Criteria:** Comment now references a real ledger entry instead of being a dangling reminder. — **MET.**
**Exact CLOSED End State:** CLOSED when committed. — **MET.**
**Acceptance History:** Comment rewritten: DONE / Regression: DONE (0 failed, compiles with and without `testkit`).

#### GUI-OPERATOR-ACTION-409-BODY-SURFACE-01 — Surface real daemon conflict reasons to the operator

**Status:** CLOSED (`MQK-LEDGER-BURN-CONTROLLER-04` burn phase, 2026-09-07) — this session's own fresh harness pass confirms the row's pre-recorded L0 truth-up: `de17635d` is an ancestor of HEAD, `http.ts` parses the response body on `!response.ok` and `actions.ts::failedOperatorActionReceipt` prefers `payload.blockers` (`blocking_failures: operatorPayload.blockers ?? []`) over the generic message. The dedicated `features/system/gui_ops_409_body_surface.test.ts` carries exactly the required controls (B1 real-blockers-surfaced, B2 malformed-JSON-does-not-crash, B3 no-body-stays-truthful, B4 non-JSON error body never parsed as JSON, B5 unrecognized JSON shape never fabricates a blocker, plus the 200-success positive control). `npm test -- --run` is 1048/1048 green. No code changed by this session. · **Priority:** P1 · **Paper Impact:** GREEN (GUI transport-layer fix only; does not touch the Rust backend/trading path) · **Subsystem:** mqk-gui
**Ledger Truth-Up (`POST-WAVE06-LEDGER-BURN-01` L0, 2026-09-06):** Reclassified `ALREADY_CLOSED_LEDGER_STALE` — current HEAD `84dcd14c` already contains this invariant via ancestor commit `de17635d`, verified `git merge-base --is-ancestor`; the full transport→receipt→UI render chain plus a dedicated test file are confirmed present. Documentation-drift correction only; not promoted to literal `CLOSED` pending a fresh harness pass per `audit_repo_truth_rules.md`.
**Current Source Truth (superseded by the Ledger Truth-Up above — retained for history):** `core-rs/mqk-gui/src/features/system/http.ts:110-117` (`postJson`) discards the HTTP response body on any non-2xx status, setting only `error: "HTTP ${status}"`. The daemon (`control_plane.rs:783-805,906-908` and 9 other `StatusCode::CONFLICT` sites) returns a structured `OperatorActionResponse` on 409 with real `blockers: [...]` explanation text, but `actions.ts::failedOperatorActionReceipt` (`actions.ts:116-151`) can only synthesize a generic message because the body was never parsed. `ActionReceiptBanner.tsx:13` never displays the real reason.
...
**Status:** CLOSED — INDEPENDENTLY ACCEPTED (`MQK-LEDGER-FINAL-CLOSEOUT-01`, 2026-09-07, accepted implementation lineage includes `e4f9eb92`, confirmed ancestor of HEAD; current integrated launcher truth accepted, not the superseded intermediate refresh-child architecture). Was `IMPLEMENTED_PENDING_REVIEW` (repaired, still not accepted) · **Priority:** P2 · **Paper Impact:** GREEN (orchestration script + one narrow `-SkipGui` addition to `Launch-VeritasLedger.ps1`; zero Rust/Python trading code touched) · **Subsystem:** Ops tooling / operator launcher
**Ledger Truth-Up (`POST-WAVE06-LEDGER-BURN-01` L0, 2026-09-06):** Reclassified `ALREADY_CLOSED_LEDGER_STALE` — the "isolated worktree, not merged" claim below is stale. Current HEAD `84dcd14c` directly contains `scripts/windows/Start-MiniQuantDesk.ps1` as linear history on this branch, contradicting the ledger's own text; git ancestry proves this. Documentation-drift correction only — **explicitly not promoted to literal `CLOSED`**: per the input reconciliation's own hard-stop, this family's Pester/PowerShell assertion counts were confirmed present by line-grep only, not by execution, so a real harness pass against current HEAD is still required before `CLOSED` would be honest (`audit_repo_truth_rules.md`).
**Current Source Truth (superseded in part by the Ledger Truth-Up above — retained for history):** Built entirely in the isolated worktree `C:\Users\Zacha\Desktop\MiniQuantDeskV4-ops` on branch `ops-official-launcher`, forked from the protected paper-soak baseline `54082a448c84b6429713a429bfb9403da8822131`. Original implementation landed at commit `aead3420974ba1bdf493344f57e9c519ee764c0e`.
...
**Status:** CLOSED — INDEPENDENTLY ACCEPTED (`MQK-LEDGER-FINAL-CLOSEOUT-01`, 2026-09-07, implementation `1a9c4b8f`, confirmed ancestor of HEAD; current official Paper startup accepted as using the daemon required-universe scheduler as the sole ongoing market-data maintenance authority). Was `IMPLEMENTED_PENDING_REVIEW` · **Priority:** P1 · **Paper Impact:** GREEN (orchestration-script-only; removes a redundant PowerShell-owned refresh-child subsystem, zero Rust/Python trading code touched) · **Subsystem:** Ops tooling / operator launcher

**Ledger Truth-Up (`POST-WAVE06-LEDGER-BURN-01` L0, 2026-09-06):** Reclassified `ALREADY_CLOSED_LEDGER_STALE` — current HEAD `84dcd14c` already contains this invariant via ancestor commit `1a9c4b8f` ("integrate: unify paper launcher data authority"), verified `git merge-base --is-ancestor`; the old PowerShell-owned ownership functions are confirmed removed. Documentation-drift correction only — not promoted to literal `CLOSED` pending a fresh harness pass per `audit_repo_truth_rules.md`.

**Current Source Truth (superseded by the Ledger Truth-Up above — retained for history):** Built in the isolated worktree `C:\Users\Zacha\Desktop\MiniQuantDeskV4-integration`, branch `integrate-paper-autofresh-launcher`, created from the accepted `MARKET-DATA-AUTOFRESH-REQUIRED-UNIVERSE-01` chain HEAD `f6e769728cfe6e4febd249c0c9db97d52a509d9d` with `origin/ops-official-launcher` (`OFFICIAL-DUAL-MODE-LAUNCHER-01` chain, parent `e4f9eb92ade32e2f7d7e5cc7c45a0c6dea18c8ba`) merged in via `git merge --no-ff`. Not merged to `main`.
...
**Status:** CLOSED — INDEPENDENTLY ACCEPTED (`MQK-LEDGER-FINAL-CLOSEOUT-01`, 2026-09-07, implementation `cd0f1a00`, confirmed ancestor of HEAD; current closed-set required-universe semantics accepted — ready→accepted, blocked→refused, not_applicable+non-trading-day→legitimate no-work, not_applicable+trading-day→refused, missing/unknown state→refused; no optimistic "anything except blocked" fallthrough). Was `IMPLEMENTED_PENDING_REVIEW` · **Priority:** P0 (final integration blocker) · **Paper Impact:** GREEN (orchestration-script-only, zero Rust/Python touched) · **Subsystem:** Ops tooling / operator launcher

**Ledger Truth-Up (`POST-WAVE06-LEDGER-BURN-01` L0, 2026-09-06):** Reclassified `ALREADY_CLOSED_LEDGER_STALE` — current HEAD `84dcd14c` already contains this invariant via ancestor commit `cd0f1a00` ("fix: fail closed on invalid required universe"), verified `git merge-base --is-ancestor`; the `Test-RequiredUniverseReportAcceptable` helper is confirmed present and wired. Documentation-drift correction only — not promoted to literal `CLOSED` pending a fresh harness pass per `audit_repo_truth_rules.md`.

**Current Source Truth (superseded by the Ledger Truth-Up above — retained for history):** Built in the same isolated worktree/branch as the parent patch, `C:\Users\Zacha\Desktop\MiniQuantDeskV4-integration`, branch `integrate-paper-autofresh-launcher`, starting from the pushed integration commit `1a9c4b8f150e728675d3aa996c4cef844da10c2e`. Not merged to `main`, not pushed.
...
**Status:** CLOSED (`MQK-LEDGER-BURN-CONTROLLER-03` A2, 2026-09-06, commit `40348e0c`) — the W2-B 4/4 reproduction below was real, but its root cause is test/real-calendar coupling, not a scheduler defect; see the A2 finding immediately below the W2-B one. · **Priority:** P2 · **Paper Impact:** GREEN (test-only fix; adds an `AppState` test seam, zero production behavior change) · **Subsystem:** `mqk-daemon` test suite

**A2 finding (`MQK-LEDGER-BURN-CONTROLLER-03`, 2026-09-06):** Root-caused the W2-B 4/4 failure: Controller-02's reproduction ran on 2026-09-07 (NYSE Labor Day per `mqk-integrity`'s holiday table); the test deliberately uses real `Utc::now()` for `now_a`/`now_b` (documented in its own source comment — the background loop's wake-time scheduling needs the real wall clock), and `run_required_universe_cycle`'s `!schedule.is_trading_day` early return short-circuits before the injected `BarrierDelayedProvider` is ever called on a non-trading day, so `call_started.notify_one()` never fires. Confirmed by negative control: today (2026-09-06) is also a non-trading day (Sunday), and reverting just the fix reproduces the exact original failure (`A's provider call must start within 10s: Elapsed(())`) against today's real calendar. Fix: added a narrow `required_universe_calendar_override_for_test` seam on `AppState` so `run_and_record_cycle` can take an injected `MarketCalendarProvider` (defaulting to the real `active_calendar_provider_from_env()` in production, unchanged); the race test now injects the file's own existing `trading_day_calendar()` fixture, while `now_a`/`now_b` remain real `Utc::now()` exactly as before. Result: 10/10 consecutive passes (~0.1s each), full 19-test scenario file green including `non_trading_day_makes_zero_provider_calls` (holiday production behavior unchanged). This **disproves** `AUTOFRESH-SCHEDULER-BARRIER-CALL-NEVER-STARTS-01` as an independent production defect (see that row).

**W2-B finding (`MQK-LEDGER-BURN-CONTROLLER-02`, 2026-09-06, prior session — root cause identified above, preserved for history):** Ran `stop_start_generation_race_old_cycle_cannot_overwrite_new_owner` in isolation (exact-name filter, single test per process, no cross-test contamination) 4 times — 1 initial + 3 confirmation runs, mixed default/`--test-threads=1` — and it **failed 4/4**, every time with the identical signature: `A's provider call must start within 10s: Elapsed(())` at the test's very first barrier (`call_started.notified()`, before generation-race logic is even reached), each run taking ~10.1s (a real timeout, not a fast crash). A sibling test in the same file that also drives `start_required_universe_scheduler` (`concurrent_scheduler_start_admits_exactly_one_starter`, no barrier-delayed provider) passed cleanly in 0.04s in the same session, ruling out "the scheduler function itself is broken" as the cause — the defect is specific to this test's `BarrierDelayedProvider`-driven concurrent-start path. This is a **materially different result** from the prior session's investigation immediately below (0/4 reproductions there, on a different branch/commit) — not a re-confirmation of "no defect", a genuine current-HEAD regression or branch-specific defect. Per this row's own controller instruction, no diagnosis deeper than isolating the sibling-test contrast above was performed, and no fix was attempted at that time — see the A2 finding above for the actual root cause.
...
**Exact CLOSED End State:** CLOSED (`MQK-LEDGER-BURN-CONTROLLER-03` A2) — met: real cause identified (test/real-calendar coupling), deterministic fix applied, 10/10 consecutive passes plus full scenario-file regression green, negative control proves the root cause.
**Expected Handoff:** Start HEAD `3d2894d39a184b1740faa1f20694dcba5b498f78` (original investigation); superseded by Controller-03 A2's fix on `post-wave06-ledger-burn-01` at commit `40348e0c`.

#### AUTOFRESH-SCHEDULER-BARRIER-CALL-NEVER-STARTS-01 — `stop_start_generation_race_old_cycle_cannot_overwrite_new_owner` deterministically times out on current HEAD

**Status:** SUPERSEDED / DUPLICATE of `PAPER-SOAK-RUST-TIMING-TEST-HARDENING-01`'s A2 root cause (`MQK-LEDGER-BURN-CONTROLLER-03`, 2026-09-06) — not an independent production defect; disposed, not carried forward. · **Priority:** P2 (was) · **Paper Impact:** GREEN (test-only; no production behavior change made or proposed) · **Subsystem:** `mqk-daemon` test suite / `required_universe_scheduler`
**A2 resolution:** Root cause was (c)-adjacent but not DB drift: the test's real-calendar dependency. Controller-02's reproduction ran on 2026-09-07 (NYSE Labor Day); `run_required_universe_cycle`'s `!schedule.is_trading_day` early return short-circuits before `BarrierDelayedProvider` is ever called on a non-trading day, so `call_started.notify_one()` never fires — not (a) a scheduler regression, not (b) a lost `Notify` permit. Confirmed by negative control (today, 2026-09-06, is also non-trading — reverting the fix reproduces the exact original failure). See `PAPER-SOAK-RUST-TIMING-TEST-HARDENING-01`'s A2 finding for the full fix (a narrow injectable-calendar `AppState` test seam) and proof (10/10 consecutive passes, full scenario file green). This row's own three hypotheses (a)/(b)/(c) are now resolved: none of them was the actual cause.
**Current Source Truth (as filed, preserved for history):** `core-rs/crates/mqk-daemon/tests/scenario_market_data_autofresh_required_universe_01.rs`, `stop_start_generation_race_old_cycle_cannot_overwrite_new_owner` — fails 4/4 in isolation on branch `post-wave06-ledger-burn-01` at HEAD `889db5f0`, always at the same line (`call_started.notified()` timeout, `A's provider call must start within 10s: Elapsed(())`), ~10.1s per run. The sibling test `concurrent_scheduler_start_admits_exactly_one_starter` (same file, same `start_required_universe_scheduler` entrypoint, no `BarrierDelayedProvider`) passes in 0.04s in the same session — isolates the defect to this test's barrier-delayed-provider concurrent-start path, not the scheduler function generally.
**Dependencies:** None. References `PAPER-SOAK-RUST-TIMING-TEST-HARDENING-01` (the investigation that discovered this, and whose A2 fix resolved it) and `PAPER-OPS-AUTOFRESH-LAUNCHER-INTEGRATION-01` (original discrepancy record, different branch).
**Exact CLOSED End State:** N/A — disposed as SUPERSEDED/DUPLICATE, not independently closed (no distinct fix required beyond `PAPER-SOAK-RUST-TIMING-TEST-HARDENING-01`'s A2).
**Acceptance History:** Superseded, not tracked separately (`MQK-LEDGER-BURN-CONTROLLER-03` A2, commit `40348e0c`).
...
**Status:** IMPLEMENTED_PENDING_REVIEW — code+tests done, independent paper-baseline review outstanding (`POST-WAVE06-LEDGER-BURN-01` W1-08, 2026-09-06) · **Priority:** P1 · **Paper Impact:** YELLOW (touches `routes/portfolio.rs`, shared paper+live code path) · **Subsystem:** Live account truth / mqk-broker-alpaca / mqk-schemas
**Resolution (W1-08):** `AlpacaAccountRaw` (`types.rs`) and `BrokerAccount` (`mqk-schemas/src/lib.rs`) both gained `buying_power: Option<String>` / `daytrading_buying_power: Option<String>` with `#[serde(default)]` — an unexpectedly-missing field deserializes to `None`, never a fabricated value or a hard parse failure on the rest of the payload. `normalize_account` (`snapshot.rs`) copies both through unchanged. Two production call sites read `snapshot.account.buying_power` and parse it fail-closed (`.as_deref().and_then(|s| s.parse::<f64>().ok())` — missing or malformed both yield `None`, never `Some(0.0)`, never a `cash` fallback): `routes/portfolio.rs::portfolio_summary` (the originally-cited site) and a second site with the identical bug found during this fix, `routes/oms_metrics.rs::metrics_dashboards` (`GET /api/v1/metrics/dashboards`'s `buying_power` field was also `Some(cash_val)`) — both corrected, since leaving one aliased site while claiming this row closed would not honestly satisfy "stop aliasing cash." The internal synthetic Paper broker's own `BrokerAccount` constructions (`state/snapshot.rs`, two sites) correctly set `None` for both fields — a synthetic broker has no real margin/buying-power concept, so `None` is the honest value, not a regression. **Proof:** `scenario_daemon_routes.rs`'s pre-existing `api_portfolio_and_risk_summary_derive_from_snapshot` test previously asserted the OLD buggy behavior (`buying_power == cash`) — corrected to assert `null` (the fixture supplies no real `buying_power`); two new tests added: `api_portfolio_summary_propagates_real_buying_power_not_cash` (positive control: a fixture with `buying_power="2500.75"` distinct from `cash="500.25"` propagates the real value verbatim) and `api_portfolio_summary_malformed_buying_power_is_null_not_zero` (negative control: a non-decimal `buying_power` string yields `null`, never a fabricated `0.0`). `scenario_metrics_dashboards_cc05.rs`'s existing assertions (`is_null() || is_number()`) already tolerated the fix without modification. 22 other test-fixture call sites across the workspace needed only the two new `None` fields added (mechanical, zero behavior change) to keep compiling. **Regression:** `cargo check --workspace --all-targets` clean; `mqk-broker-alpaca`/`mqk-schemas` full suites green (53+7+29+23+32=... all passed, 0 failed); `mqk-daemon --lib` 883 passed / 31 failed — all 31 failures independently confirmed to share the identical pre-existing `mqk-db/src/test_support.rs:755` `MQK_DATABASE_URL`-required panic (no `MQK_DATABASE_URL` set in this environment), unrelated to this change; `scenario_daemon_routes.rs` 76/76 passed (73 pre-existing + 1 fixed + 2 new). `cargo clippy --workspace -D warnings` could not run: blocked workspace-wide by a pre-existing, unrelated `mqk-db` doc-lint failure (toolchain-version drift, `autonomous_daily_operation.rs`, a file this patch never touched) — not fixed here, out of scope.
**Original entry (retained for history):** `mqk-broker-alpaca/src/types.rs:323-330` (`AlpacaAccountRaw`) only deserializes `equity`, `cash`, `currency` from Alpaca's `GET /v2/account` — no `buying_power`/`daytrading_buying_power`/`pattern_day_trader` fields captured at all. `mqk-schemas/src/lib.rs:63-67` (`BrokerAccount`) mirrors this omission. `routes/portfolio.rs:440` aliases `buying_power: Some(cash)` — silently wrong for any margin account.
...
**Acceptance Criteria:** 1) `BrokerAccount` carries real `buying_power`/`daytrading_buying_power`. — **MET.** 2) `portfolio.rs` no longer aliases cash. — **MET (and `oms_metrics.rs`'s identical bug fixed alongside it).** 3) Regression tests green. — **MET (see Resolution above; 31 pre-existing unrelated DB-required failures excluded).** 4) Change reviewed against paper baseline before any merge to `main`. — **PENDING** (independent review, not performed by this same session per the paper-soak protection rule).
**Exact CLOSED End State:** CLOSED when buying-power truth is sourced from Alpaca's real field end-to-end, proven by a test fixture with `buying_power != cash`, and the change has passed regression review against the paper baseline. — **Code+test MET; independent regression review against the paper baseline is still PENDING before merge to `main`, per this row's own YELLOW/Paper-Soak-Protection classification.**
**Acceptance History:** Implementation: DONE / Focused tests: DONE (3/3 new+corrected assertions pass) / Full regression: DONE (0 new failures) / Independent paper-baseline review: PENDING.

#### LIVE-SECRETS-CONSOLIDATION-01 — Route live credential resolution through mqk_config::secrets

**Status:** IMPLEMENTED_PENDING_REVIEW — code+tests done, independent paper-baseline review outstanding (`POST-WAVE06-LEDGER-BURN-01` W1-09, 2026-09-06) · **Priority:** P2 · **Paper Impact:** YELLOW (same `DaemonBroker` construction path used by paper) · **Subsystem:** mqk-config / mqk-daemon
**Resolution (W1-09):** Investigation found routing `build_daemon_broker` through the full `resolve_secrets_for_mode()` bundle (as originally scoped) would have been a real behavior change forbidden by this row's own "Out of Scope": that function also requires a TwelveData API key to be present in `LIVE` mode (it resolves the whole per-mode secrets bundle — broker + TwelveData + Discord — for one-time startup preflight), which has nothing to do with broker-adapter construction and would make live broker construction newly fail on a missing `TWELVEDATA_API_KEY` even when the daemon's price feed is Alpaca. Instead, `secrets.rs`'s own already-tested single-var primitive, `resolve_env` (previously private), was made `pub` — this is the literal function `resolve_secrets_for_mode` itself calls for each of `broker_api_key`/`broker_api_secret`/`twelvedata_api_key`, so `build_daemon_broker` now shares the exact same, already-covered resolution mechanism (`scenario_secrets_mode_enforcement.rs`'s existing SECRETS_MISSING/present/empty-string tests) without inheriting the wider per-mode bundle's unrelated requirements. `build_daemon_broker`'s two `std::env::var(key_env)`/`std::env::var(secret_env)` calls became `mqk_config::secrets::resolve_env(key_env)`/`resolve_env(secret_env)`, with identical error messages/codes and identical env-var names (`ALPACA_API_KEY_PAPER`/`LIVE`, `ALPACA_API_SECRET_PAPER`/`LIVE`, unchanged). **One intentional, narrow strengthening, noted honestly rather than silently:** `std::env::var` treats a set-but-empty-string var as `Ok("")` (a valid, if empty, credential); `resolve_env` treats it as absent (`None`), matching `secrets.rs`'s own documented contract. This is a fail-closed direction change only (an empty credential string was never a legitimate broker credential in practice) — not a new failure mode.
**Regression:** `cargo check --workspace --all-targets` clean; `mqk-config` full suite green (6+6+13+5 passed, 0 failed); `mqk-daemon --lib` 883 passed / 31 failed — identical count and identical pre-existing `mqk-db/src/test_support.rs:755` `MQK_DATABASE_URL`-required panic class as before this change, confirming zero new failures. No new env-mutating test was added directly to `mqk-daemon`'s shared `--lib` test binary — the missing-secret negative control and present-secret positive control are already proven at the shared primitive's own definition site (`mqk-config`'s existing suite), which this row's change now provably shares rather than duplicates.
**Current Source Truth (retained for history):** `mqk-config/src/secrets.rs:14` documents that `LIVE` mode requires key+secret+TwelveData key via `resolve_secrets_for_mode()`, but `mqk-daemon/src/state/broker.rs::build_daemon_broker` reads `std::env::var` directly rather than through that function — a documented-vs-actual mismatch against `secrets.rs`'s own stated contract ("never scatter `std::env::var` calls").
...
**Acceptance Criteria:** 1) `build_daemon_broker` no longer calls `std::env::var` directly for secrets. — **MET.** 2) Paper-mode behavior provably unchanged. — **MET modulo the one intentional empty-string fail-closed strengthening noted above.** 3) Regression-reviewed before merge. — **PENDING.**
**Exact CLOSED End State:** CLOSED when secret resolution has exactly one source of truth (`mqk_config::secrets`), proven unchanged for paper mode by regression tests. — **Code+test MET; independent regression review against the paper baseline is still PENDING before merge to `main`, per this row's own YELLOW classification.**
**Acceptance History:** Implementation: DONE / mqk-config regression: DONE (0 failed) / mqk-daemon --lib regression: DONE (883/31 split unchanged, 0 new failures) / Independent paper-baseline review: PENDING.

#### LIVE-CLI-ARM-RECONCILE-01 — Determine whether legacy `mqk run arm` CLI path is still operationally reachable

**Status:** CLOSED — determined and deprecated (`POST-WAVE06-LEDGER-BURN-01` W1-10, 2026-09-06) · **Priority:** P2 · **Paper Impact:** GREEN · **Subsystem:** mqk-cli
**Determination (W1-10, using the already-completed investigation in the repair ZIP's `21-live-cli-arm-current-truth.md`, not re-investigated from scratch):** `mqk run arm` is reachable and performs a real, CAS-guarded DB mutation (`runs.status` CREATED/STOPPED → ARMED via `arm_preflight`'s tail call to `arm_run`) — not a no-op, not a false success. It is **NOT** a proven live-dispatch safety bypass: the daemon's actual order-dispatch gate (`start_execution_runtime`, `state/lifecycle.rs`) unconditionally checks its own `IntegrityState` (armed only via `mqk daemon arm` / `POST /v1/integrity/arm`, disarmed by default on every daemon boot) and never consults `runs.status` at all — the two are separate authorities checking different things. The residual real risk is lifecycle-parity confusion (a `runs` row can read `ARMED`/`RUNNING` while the daemon never actually started executing it), not a safety bypass. **Chose Option A (deprecate, per this row's own "prefer A if there is no real current operator workflow depending on it"):** `CLI-DAEMON-CONTROL-PASSTHROUGH-01` (already closed this same wave, see L0) already ships the correct, daemon-integrated `mqk daemon arm` command as the live operator safety surface, so no real workflow depends on the legacy DB-direct path continuing to exist unmarked. `RunCmd::Arm`'s clap doc comment (`main.rs`) now states `[DEPRECATED — see 'mqk daemon arm']` plus the full authority-boundary explanation; `run_arm` (`commands/run.rs`) gained a matching function doc comment and now prints a runtime `eprintln!` warning on every invocation pointing operators to `mqk daemon arm`. No behavior change to the underlying DB mutation itself (out of scope, per this row).
**Regression:** `cargo check -p mqk-cli` clean; `scenario_cli_arm_requires_confirmation.rs` still passes (its `stderr` assertion is `contains(...)`, unaffected by the added warning line); full `mqk-cli` suite green (all test binaries, 0 failed).
**Original entry (retained for history):** `mqk-cli/src/commands/run.rs:100-111` (`run_arm`) calls `enforce_manual_confirmation_if_required` before `mqk_db::arm_preflight`, tested by `scenario_cli_arm_requires_confirmation.rs` ("PATCH 16") against a hand-built `NewRun` via `mqk_config::load_layered_yaml`. This looks like an older, pre-daemon CLI architecture parallel to (not integrated with) the current daemon's `/v1/integrity/arm` HTTP surface (proven in the arm-gate audit, §5 Lane A / core-exec cluster).
**Problem (original, resolved by the determination above):** Unclear whether operators actually use this path for live arm, or whether it's vestigial.
**Dependencies:** NONE. **Unlocks:** `CLI-RUN-STUB-TRACKING-01`'s resolution — see that row (W1-11) immediately below.
...
**Required Validation:** Investigation only in the first phase; if code changes result, standard `cargo test -p mqk-cli`. — **DONE.**
**Acceptance Criteria:** A definitive determination recorded in this ledger: "still live-relevant, now wired to `check_arm_safety`" or "deprecated, marked legacy in CLI help text." — **MET: deprecated, marked legacy in CLI help text.**
**Exact CLOSED End State:** CLOSED when the determination is recorded and, if action was needed, implemented and tested. — **MET.**
**Acceptance History:** Determination: DONE / Deprecation markers implemented: DONE / Regression: DONE (0 failed).

#### LIVE-FLATTEN-PROOF-01 — LiveShadow-mode flatten-on-halt scenario test

**Status:** CLOSED — proof test written, both cases pass (`POST-WAVE06-LEDGER-BURN-01` W1-13, 2026-09-06) · **Priority:** P2 · **Paper Impact:** GREEN (test-only, LiveShadow has zero capital at risk) · **Subsystem:** mqk-risk / mqk-daemon
**Resolution (W1-13, PROOF-ONLY):** Investigation found `RiskRequestContext.is_risk_reducing`/`BrokerGateway::submit_with_context` (the actual `RISK-FLATTEN-ON-HALT-01` mechanism, living in `mqk-execution`/`mqk-runtime`) and `flatten-paper-positions` (the `POST /api/v1/ops/action` route `scenario_paper_flatten_psf01.rs` exercises) are two entirely separate, unconnected mechanisms — the HTTP route submits a normal outbox order and never sets `is_risk_reducing` at all (confirmed: `scenario_paper_flatten_psf01.rs:503` asserts `is_risk_reducing` is absent from its outbox row). New file `scenario_live_shadow_flatten_on_halt_01.rs` (2 tests, both pass, no DB required): **LSF-01** proves the operator `flatten-paper-positions` HTTP action is hardcoded Paper-only — `DeploymentMode::LiveShadow` gets the identical `403 not_paper_mode` refusal as any other non-Paper mode; this is a genuine, deterministic, documented finding, tracked as a new follow-on row (`LIVE-SHADOW-FLATTEN-ACTION-ROUTE-GAP-01`, immediately below) rather than silently repaired in this proof-only patch. **LSF-02** — the load-bearing positive proof — drives the REAL `BrokerGateway` wrapping the REAL `RuntimeRiskGate` through the REAL `mqk_broker_alpaca::AlpacaBrokerAdapter` (hermetic httpmock standing in for LiveShadow's live-base-URL Alpaca connection, zero real network) into a genuine sticky max-drawdown breach (mirroring `mqk-runtime`'s own CASE 2 e2e proof): a normal `submit()` is denied by the risk gate before the broker is ever invoked (`mock.assert_hits(0)`), while `submit_with_context` marked `is_risk_reducing: true` bypasses the halt and reaches the mocked Alpaca order-submit endpoint successfully (`mock.assert_hits(1)`, response `broker_order_id` verified). **This proves flatten-on-halt is genuinely broker-kind-generic and works end to end through the actual Alpaca adapter type — the real production seam does NOT fail; no defect found in the mechanism itself.** Required a small dev-dependency-only Cargo.toml addition to `mqk-daemon` (`mqk-execution` with `testkit` feature, `httpmock`) — mirrors the existing `mqk-db`-with-testkit dev-dependency pattern already in this file, cannot reach a production build (CI-guarded by `CI-TESTKIT-FEATURE-GUARD-VERIFY-01`, this same wave).
**Regression:** `cargo check --workspace --all-targets` clean; new test file 2/2 pass; `mqk-daemon --lib` 883/31 split unchanged (0 new failures).
**Original entry (retained for history):** `RISK-FLATTEN-ON-HALT-01` (memory: CLOSED) implements `RiskRequestContext`/`evaluate_gate_for_request`, generic to broker kind. Only `scenario_paper_flatten_psf01.rs` exists; no LiveShadow-mode flatten test.
**Problem (original, resolved above):** Flatten-on-halt has never been exercised against the live Alpaca endpoint (even in LiveShadow, which requires no capital risk and already has `start_allowed: true`).
**Dependencies:** NONE.
...
**Likely Files:** `core-rs/crates/mqk-daemon/tests/scenario_live_shadow_flatten_on_halt_01.rs` (new). — **DONE.**
**Required Validation:** `cargo test -p mqk-daemon --test scenario_live_shadow_flatten_on_halt_01 -- --include-ignored` (if DB-gated). — **DONE, no DB required, 2/2 passed.**
**Acceptance Criteria:** Test exists, runs against LiveShadow mode, and passes or clearly documents a found defect. — **MET: exists, passes, and LSF-01 documents a real found gap (tracked separately below).**
**Exact CLOSED End State:** CLOSED when the test exists and passes (or a follow-up defect patch is filed if it fails). — **MET.**
**Acceptance History:** Test written: DONE (2 cases) / Both pass: DONE / Follow-on gap tracked: DONE / Regression: DONE (0 new failures).

#### LIVE-SHADOW-FLATTEN-ACTION-ROUTE-GAP-01 — No operator-triggered flatten action exists for LiveShadow mode

**Status:** OPEN — newly discovered while proving W1-13, not fixed in this wave · **Priority:** P2 · **Paper Impact:** GREEN (LiveShadow has zero capital at risk; this is a capability gap, not a safety defect) · **Subsystem:** mqk-daemon
**Current Source Truth:** `routes/control_plane.rs`'s `flatten-paper-positions` `action_key` handler (`~line 1709`) is hardcoded to refuse with `disposition: "not_paper_mode"` for any `DeploymentMode` other than `Paper`, confirmed for `LiveShadow` specifically by `scenario_live_shadow_flatten_on_halt_01.rs`'s LSF-01 (this wave, W1-13). There is no `flatten-live-shadow-positions` (or mode-generic) operator action anywhere in the daemon.
**Problem:** An operator running LiveShadow (real market data + a real Alpaca live-mode adapter, zero capital risk by construction) has no HTTP-level way to trigger a flatten of any positions LiveShadow might accumulate — only the lower-level `is_risk_reducing`/`submit_with_context` library mechanism (proven generic and working, W1-13 LSF-02) is broker-kind-generic; the operator-facing route is not.
**Why This Matters:** `LIVE-TINY-CAPITAL-SMOKE-01`/`LIVE-TRUST-CHAIN-SHADOW-CAPTURE-01` (both still open) will accumulate real LiveShadow evidence; an operator may need to manually flatten a LiveShadow position without restarting the daemon.
**Dependencies:** NONE.
**In Scope (future patch):** Either generalize `flatten-paper-positions`'s mode gate to accept `LiveShadow` too (reusing the exact same outbox/OMS path, since LiveShadow's broker adapter is real Alpaca and its orders are genuinely tracked), or add a dedicated `flatten-live-shadow-positions` action — an operator/design decision not made by this discovery entry. **Out of Scope for this discovery entry:** Implementing either fix now.
**Likely Files:** `core-rs/crates/mqk-daemon/src/routes/control_plane.rs`.
**Acceptance Criteria:** An operator can trigger a flatten for LiveShadow-mode positions through the standard `/api/v1/ops/action` surface, with the same arm/reconcile/run/position gating `flatten-paper-positions` already proves for Paper.
**Exact CLOSED End State:** CLOSED when that action exists and is scenario-tested for LiveShadow mode, mirroring `scenario_paper_flatten_psf01.rs`'s PSF-01..13 proof matrix.

#### LIVE-TINY-CAPITAL-SMOKE-01 — Build live-shadow smoke automation script

**Status:** IMPLEMENTATION_COMPLETE_OPERATOR_VALIDATION_DEFERRED (MQK-LIVESHADOW-PROVENANCE-FINAL-01, 2026-09-07, commit b2a8caa4, INDEPENDENTLY ACCEPTED) - engineering is independently accepted; real operational LiveShadow smoke remains NOT RUN and is the sole remaining requirement for this row's original full CLOSED end state. **Priority:** P1 **Paper Impact:** GREEN **Subsystem:** Ops tooling

**R1B finding (`MQK-LIVESHADOW-R1B-FINAL`, 2026-09-07):** Independent review found that Controller-04 R1's evidence resolver (`625b2eb6`) proved only TEMPORAL novelty (a log that is new since a pre-run snapshot), not CAUSAL ownership (that the log was written by THIS invocation, not a concurrent foreign one). Deterministic failure case: this wrapper's own child produces no evidence while a concurrent foreign LiveShadow invocation writes one otherwise-valid new `launch_*.json` log — R1's set-difference resolver accepted that foreign log as this run's own evidence. A pre-fix RED run (`LS-EV-08` fixture, run against the unmodified R1 resolver before any production change) reproduced exactly this failure. Fixed: `Start-LiveShadowSmoke.ps1` generates an opaque GUID and passes it to `Start-MiniQuantDesk.ps1` via the new non-secret `-InvocationId` parameter; `Invoke-LiveShadowStartup` writes that exact value into its launch log's `invocation_id` field, regardless of CheckOnly/full-run; `Resolve-LiveShadowRunEvidence` no longer looks at "new since a snapshot" at all — it accepts only a log whose `invocation_id` exactly matches this invocation's GUID, with zero or more-than-one exact matches both failing closed to `not_observed`. Three new hermetic negative controls: `LS-EV-08` (foreign-only fails closed — the exact pre-fix RED scenario, now GREEN), `LS-EV-09` (foreign + exact match present: only the exact match is authoritative), `LS-EV-10` (two logs both claiming the exact expected id: fails closed as ambiguous). `LS-EV-01..07` preserved with fixtures updated to the new ID-based API. 21/21 guard proofs (`tests\script_guards\test_live_shadow_smoke.ps1`); `scripts\windows\tests\test_official_dual_mode_launcher.ps1` also re-run, 0 violations (the `-InvocationId` param addition to `Start-MiniQuantDesk.ps1` is additive-only and does not touch Paper/Live dispatch). No daemon started, no broker call, no order submitted, no real LiveShadow smoke run — hermetic fixtures plus one real `-CheckOnly` invocation only.
**A3A/A3B finding (`MQK-LEDGER-BURN-CONTROLLER-03`, 2026-09-06):** Independent review of W2-E correctly disputed its `IMPLEMENTATION_COMPLETE_OPERATOR_VALIDATION_DEFERRED` claim: (1) `Start-LiveShadowSmoke.ps1` delegated to `-Mode Live`, which is LiveCapital's inert read-only preflight — no real LiveShadow daemon-bootstrap capability existed for it to delegate to, so "engineering complete" was not yet true; (2) the wrapper's non-`-CheckOnly` path therefore inherited `-Mode Live`'s interactive `Confirm-LiveIntent`/"Type LIVE" prompt, making `-IAcknowledgeLiveShadowOnly` not actually a noninteractive full-run switch; (3) `manifest.json` hardcoded `real_daemon_start_performed`/`real_broker_call_performed`/`real_order_submitted` to a literal `$false` regardless of what happened, validated by a circular guard. A3A built the missing capability: `-Mode LiveShadow` / `-DeploymentMode live-shadow` on the canonical launcher, reusing the existing daemon-bootstrap seam (not a second framework), with `Get-BackendProbe`'s identity-verification chain correctly made deployment-mode-aware (the paper-only `autonomous_readiness_applicable` block, which `routes/system.rs` defines as literally `is_paper_alpaca`, is now skipped for live-shadow rather than incorrectly required) — 40 hermetic proofs, zero regressions across 9 dependent test files. A3B then repointed `Start-LiveShadowSmoke.ps1` at the real `-Mode LiveShadow` (closing defects 1 and 2 for free, since that path never calls `Confirm-LiveIntent`) and replaced the fabricated manifest booleans with a truthful split: wrapper-static-contract facts (provably `$false`, unconditionally, about this script's own source) versus observed-runtime-evidence (`'not_run'`/`'observed_true'`/`'observed_false'`/`'not_observed'`, read from the canonical launcher's own JSON log, never invented) — 11/11 guard proofs. `DEFERRED_OPERATOR_VALIDATION` still applies to exactly one thing now: a real operational LiveShadow smoke run, which this controller did not perform (explicit mission constraint) — that is a genuinely real, correctly-labeled deferral, not a disguised incompleteness.
**Original entry (retained for history):** `scripts/` has 82 files matching "live" but all are paper-trading scripts or unrelated (Kraken, live Discord channel config). No live-capital smoke script exists. `docs/runbooks/live_shadow_operational_proof.md` is a manual proof-sequence document, not automation.
...
**Exact CLOSED End State:** Engineering: MET AND INDEPENDENTLY ACCEPTED through R1C b2a8caa4. Exact invocation identity, collision-proof launcher/bootstrap/wrapper artifacts, duplicate/replay fail-closed behavior, malformed-ID refusal, closed evidence vocabulary, and resolver read consistency are accepted. Full CLOSED still requires one successful real operational LiveShadow smoke, which remains DEFERRED_OPERATOR_VALIDATION.
**Acceptance History:** Engineering: MET / INDEPENDENTLY ACCEPTED through b2a8caa4 (MQK-LIVESHADOW-PROVENANCE-FINAL-01, 2026-09-07). Earlier R1 625b2eb6 and R1B 71ac7d21 are superseded intermediate repairs. Real operational smoke: PENDING / IMPLEMENTATION_COMPLETE_OPERATOR_VALIDATION_DEFERRED.
...
**Status:** CLOSED — ALREADY_CLOSED_LEDGER_STALE — INDEPENDENTLY ACCEPTED (`MQK-LEDGER-FINAL-CLOSEOUT-01`, 2026-09-07). This row's own acceptance criterion (§ below) offers two closure paths: (A) align the TTLs, or (B) explicitly document why they intentionally differ and add correct defensive reconciliation/fencing with halt-path proof. Current repo implements (B). Authoritative implementation lineage: `0f9606c6` ("fix: bind runtime leadership lease to run identity") and `104953e6` ("fix: fail closed on legacy unbound runtime leases") — both confirmed ancestors of this HEAD via `git merge-base --is-ancestor`. Current production truth: `RUNTIME_LEASE_TTL_SECS = 90`, `DEADMAN_TTL_SECS = 120`; the difference is documented as intentional (the deadman window must exceed the longest legitimate blocking orchestrator tick plus margin so a slow-but-live runtime is not falsely halted). Reconciliation: every current production lease is bound to exact `run_id`; same-run raw lease expiry requires same-run deadman truth before takeover; different-run lease disposition uses that other run's durable lifecycle state, never the new run's heartbeat; legacy `run_id=NULL` leases cannot be reclaimed from raw 90s expiry alone; migration 0069 reconciles legacy unbound state; the lease FK is RESTRICT, not silent CASCADE. Direct negative controls: cr07 (lease expired/deadman fresh → takeover refused), cr08 (lease expired/deadman expired → takeover permitted), cr10 (legacy unbound lease raw-expired/deadman fresh → takeover refused), cr10b (legacy unbound lease deadman stale → takeover permitted). No runtime code changed by this closeout pass. · **Priority:** P1 · **Paper Impact:** RED · **Subsystem:** mqk-daemon halt/deadman
**Prior Status (superseded by the closure above, retained for history):** READY (dependency closed; unblocked, but still RED — do not attempt during the active soak without explicit operator authorization).
**Current Source Truth:** Runtime lease TTL = 90s (`orchestrator.rs:50`); deadman TTL = 120s (`DEADMAN_TTL_SECONDS`). The 120s deadman interval can outlive the 90s runtime lease by approximately 30 seconds, so lease expiry alone cannot prove same-process task quiescence — this asymmetry is the root cause of the race that `PRE-SOAK-DAEMON-LOCAL-QUIESCENCE-AND-DEADMAN-SIDE-EFFECT-FENCE-01` (CLOSED at `e44e3ddd`) fences around rather than eliminates. The reconciliation above (option B) addresses the cause via run-identity binding rather than aligning the raw TTL values themselves.
**Problem:** The fence patch treats the symptom (a stale task might still be alive); this patch addresses the cause (why the windows don't align) via option B above.
...
**Acceptance Criteria:** Either the TTLs are aligned with a proof that no new race window opens, or a documented rationale for the intentional gap is added along with defensive fencing (which already exists via the Lane A patch) — MET via option B, see Status above.
**Exact CLOSED End State:** CLOSED when the TTL relationship is either aligned or explicitly documented as intentional, with full halt-path regression proof — MET.
**Acceptance History:** Implementation lineage `0f9606c6`/`104953e6` (ancestors of HEAD) / cr07/cr08/cr10/cr10b negative controls present / Independently accepted `MQK-LEDGER-FINAL-CLOSEOUT-01` 2026-09-07.

#### BROKER-ALPACA-RATE-LIMIT-RETRY-AFTER-01 — Parse Retry-After header on 429 responses

**Status:** CLOSED (`MQK-LEDGER-BURN-CONTROLLER-04` burn phase, 2026-09-07) — this session's own fresh harness pass confirms the row's L0 truth-up: `edc5da02` is an ancestor of HEAD, `parse_retry_after_ms` (`mqk-broker-alpaca/src/lib.rs:1138`) is wired into every 429 classification site, and `cargo test -p mqk-broker-alpaca --lib` is 53/53 green including `br04`..`br09` (valid/missing/malformed `Retry-After` parsing across GET/cancel paths, permanent-4xx classification unchanged, 429 never retried in the adapter itself). No code changed by this session. · **Priority:** P2 · **Paper Impact:** YELLOW (live order submit/replace/cancel path) · **Subsystem:** mqk-broker-alpaca
...
**Status:** CLOSED (`MQK-LEDGER-BURN-CONTROLLER-04` burn phase, 2026-09-07) — this session's own fresh harness pass confirms the row's pre-recorded L0 truth-up: `f4864e0c` is an ancestor of HEAD, `stamp_order_json_schema_version`/`validate_order_json_schema_version` (`mqk-db/src/orders.rs`) implement exactly the additive/backward-readable contract this row required (missing `schema_version` accepted as historical, current version accepted, future/malformed/zero/negative all refused, an explicit current value never silently overwritten), wired into both outbox write paths (`orders.rs:332,417`). `cargo test -p mqk-db --lib order_json_schema_version_tests` is 13/13 green, including `sv03` (legacy/missing-field backward-compat) and `sv09` (writer/reader round trip). No migration required — purely additive to the JSON payload, not the column schema. No code changed by this session. · **Priority:** P2 · **Paper Impact:** YELLOW (live write path; shape is stable/tested, low drift risk today) · **Subsystem:** mqk-db / mqk-execution
...
**Status:** CLOSED (`MQK-LEDGER-BURN-CONTROLLER-04` burn phase, 2026-09-07) — this session's own fresh harness pass confirms the row's L0 truth-up: `8868c214` is an ancestor of HEAD, `alpaca_provider.rs::fetch_bars` implements bounded retry with `Retry-After`/budget handling, and `cargo test -p mqk-md --lib alpaca_provider::tests::af` is 11/11 green (`af01`..`af11`: transient-429/5xx-then-success, bounded exhaustion, permanent-4xx never retried, malformed-200-body not retried, `Retry-After` budget boundary exact, within-budget honored, above-budget fails closed without retry). No code changed by this session. · **Priority:** P2 · **Paper Impact:** YELLOW (Alpaca is the live-gate equity data path; current behavior already fails safe — readiness gate correctly reports stale rather than fabricating data — so this is a resilience improvement, not a correctness fix) · **Subsystem:** mqk-md
...
**Status:** CLOSED — ALREADY_CLOSED_LEDGER_STALE (`MQK-LEDGER-BURN-CONTROLLER-02` W2-C, 2026-09-06) · **Priority:** P2 · **Paper Impact:** RED (no code touched — verification only) · **Subsystem:** mqk-daemon multi-symbol config
**W2-C finding:** This row's own "Current Source Truth" pointed at `multi_symbol_config.rs`'s narrow defense-in-depth check, which does still fail closed as documented -- but truncate-and-surface for cap #1 is already fully implemented one layer up, in `watchlist_intake.rs::evaluate_watchlist_intake` (ancestor commits `45fe9557`/`99b9745a`/`3e5944e3` "truncate and surface capped multi-symbol watchlists", all confirmed ancestors of this HEAD via `git merge-base --is-ancestor`), which truncates a v2 artifact's `symbols` to the first `max_symbols_to_trade` entries in artifact order and surfaces the dropped tail on `LoadedWatchlistArtifact::dropped_symbols` -- consumed by `build_multi_symbol_config_from_watchlist_artifact` (which therefore never sees an over-cap `artifact.symbols` on the real production path -- its own `ConcurrentLimitExceeded` check is unreachable defense-in-depth, exactly as its doc comment already says) and surfaced end-to-end on `WatchlistStatusResponse.symbols`/`.requested_symbols`/`.dropped_symbols` (`routes/watchlist.rs`, `api_types.rs`, both already citing this exact patch ID in their doc comments). Every required invariant is proven by existing tests, reconfirmed passing this session: deterministic first-N truncation in artifact order (`w35_truncation_uses_artifact_order_not_any_other_order`), an invalid cap value is never a truncation point (`w38_invalid_cap_value_is_never_a_truncation_point`), v1 still fully refuses on over-cap with no truncation (`w39_v1_exceeding_cap_stays_fully_refused_no_truncation`), a missing assignment on the dropped tail still refuses the whole artifact (`w42_missing_assignment_on_dropped_tail_symbol_still_refuses`), and the truncation is proven through the real runtime dispatch seam (`w43_first_n_truncation_proven_through_real_runtime_dispatch_seam`) -- `scenario_watchlist_readonly_handoff_01.rs` 43/43, `scenario_multi_symbol_runtime_config_01.rs` 18/18, `mqk-daemon --lib multi_symbol` 6/6 (1 unrelated ignored), all green. No code change was needed or made.
**Original entry (retained for history, now superseded by the finding above):** `mqk-daemon/src/state/multi_symbol_config.rs:45-59` explicitly defers cap #1 (`max_concurrent_symbols`) truncate/surface behavior — "remains open for a later patch" — currently fails closed instead of truncating.
...
**Status:** CLOSED (`MQK-LEDGER-BURN-CONTROLLER-04` burn phase, 2026-09-07) — this session's own fresh harness pass confirms the row's pre-recorded L0 truth-up: `57fbc1c2` is an ancestor of HEAD, `multi_symbol_capital_caps_preflight_warnings` (`routes/system.rs:671`) is wired into the preflight `warnings` list (`routes/system.rs:515`), and the full `mscw01..05` test matrix (all-set/no-warning, each of cap #2/#3/#5 individually missing names that exact cap, all-three-missing names all three) passes 5/5. Advisory-only, start-allowed determination unchanged (acceptance criterion 2 -- the function only ever appends warning strings, never touches the start-allowed verdict). No code changed by this session. · **Priority:** P1 · **Paper Impact:** RED (touches the autonomous session preflight path; conservatively classified RED even though it is advisory-only, since it changes what preflight reports during an active soak) · **Subsystem:** mqk-daemon preflight
...
**Status:** CLOSED (`MQK-LEDGER-BURN-CONTROLLER-04` burn phase, 2026-09-07) — this session's own fresh harness pass confirms the row's L0 truth-up: `73324b55` is an ancestor of HEAD, and `scenario_discord_channel_routing_01.rs` is 14/14 green (`cr01`..`cr12`: per-event-type channel targeting including the two-way live/paper split, unrecognized-environment routes nowhere, unconfigured-channel no-ops without blocking configured ones, canonical-vs-custom channel-name resolution from config). No code changed by this session. · **Priority:** P2 · **Paper Impact:** YELLOW (touches the already-running soak's live notifier construction) · **Subsystem:** mqk-daemon / mqk-config notify
...
**Status:** CLOSED (`MQK-LEDGER-BURN-CONTROLLER-04` burn phase, 2026-09-07) — this session's own fresh harness pass confirms the row's L0 truth-up: `b831c447` is an ancestor of HEAD, and `scenario_discord_data_staleness_alert_01.rs` is 8/8 green (`da01`..`da08`: stale/missing-bar fires critical alert, fresh bar fires none, repeated stale ticks dedup to one alert per run/symbol, Discord delivery failure never affects dispatch refusal, dedup resets at run start). No code changed by this session. · **Priority:** P2 · **Paper Impact:** YELLOW · **Subsystem:** mqk-daemon notify / market data readiness
...
**Status:** CLOSED (`MQK-LEDGER-BURN-CONTROLLER-04` burn phase, 2026-09-07) — this session's own fresh harness pass confirms the row's L0 truth-up: `ce64516c` is an ancestor of HEAD; `session_controller.rs::log_coordinator_outcome` carries a source-backed `autonomous_no_trade_diagnostics` summary derived via the same seam the read-only route uses. `MQK_DATABASE_URL=postgres://postgres:postgres@127.0.0.1:5434/mqk_test cargo test -p mqk-daemon --test scenario_autonomous_daily_outcome_coordinator_integration_01 p8_ -- --ignored --test-threads=1` is 3/3 green (`p8_01` source-backed summary present, `p8_02` no-rows states "none recorded" not fabricated, `p8_03` Discord delivery failure never alters finalization truth) against the disposable test DB. The row's own note about wall-clock-cron-vs-finalization-triggered timing remains an open question for a possible separate future row, not a defect in this row's own acceptance criteria. No code changed by this session. · **Priority:** P2 · **Paper Impact:** YELLOW · **Subsystem:** mqk-daemon notify
...
### MQK-LIVESHADOW-PROVENANCE-FINAL-01 - CURRENT TRUTH (2026-09-07)

Implementation commit b2a8caa40031b7f7b2f660ff247f1a54525291b0 is INDEPENDENTLY ACCEPTED.

Independent review inspected the actual committed production diff and complete bounded LiveShadow provenance chain. No remaining deterministic production defect was found in this bounded provenance invariant.

LIVE-TINY-CAPITAL-SMOKE-01 is now IMPLEMENTATION_COMPLETE_OPERATOR_VALIDATION_DEFERRED. Engineering is accepted; real operational LiveShadow smoke remains NOT RUN.

Current canonical buckets: ACTIVE_IMPLEMENTATION=0; PENDING_INDEPENDENT_REVIEW=0; PENDING_INTEGRATION=0; RED_AUTHORIZATION_REQUIRED=0; CLOSED=54; IMPLEMENTATION_COMPLETE_OPERATOR_VALIDATION_DEFERRED=3; BLOCKED_BY_DEFERRED_OPERATOR_VALIDATION=4; DEFERRED_POST_LEDGER_AUDIT=5; DEFERRED_FUTURE_MAINTENANCE=7; TOTAL=73.
